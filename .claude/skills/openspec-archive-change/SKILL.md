...
**qodo behavior on the archive commit (nearyou-id specific).** By the time this skill runs, the PR is non-draft (marked ready by `/opsx:apply` step 8 as the trigger point for qodo's canonical review on the implementation diff). The archive commit is a push to a non-draft PR, so qodo's `synchronize` event WILL fire and qodo WILL post a second review against the archive-commit diff (mv files + spec sync). **This skill does NOT poll or gate on that second review.** Rationale: the archive commit is mechanical (file moves + spec sync only — no production code), and qodo's structured-code-review heuristics produce low-signal output against it. If qodo posts blocking findings on the archive commit, they almost always belong to the implementation diff that qodo already reviewed in `/opsx:apply` step 8 — surface them to the user out-of-band before squash-merge if you happen to notice them, but do not insert a polling step here. Squash-merge proceeds when CI is green; qodo's archive-commit review is informational, not gating.
