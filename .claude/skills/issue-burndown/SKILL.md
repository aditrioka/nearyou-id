...
- **Gates before every push:** `./gradlew ktlintCheck detekt :backend:ktor:test :lint:detekt-rules:test` + (mobile-touching) `:mobile:app:testDevDebugUnitTest :mobile:app:testDevReleaseUnitTest`. CI runs BOTH lint frameworks — `detekt` alone is insufficient (precedent: PRs #31/#32 hit CI lint failures after skipping `ktlintCheck` locally). Don't run the gate while a local `:backend:ktor:run` is alive (Postgres connection budget — see verify-loop § Known blockers).
