...
**Cloud sandbox (no Claude_Preview / browser MCP):** the SessionStart hook already started Postgres :5433 + Redis :6379 and generated `dev/.env` (`KTOR_ENV=test` + throwaway boot and admin keys, see `dev/docs/cloud-environment.md`), so boot is `set -a; . dev/.env; set +a; ./gradlew --no-daemon :backend:ktor:run`. Drive the panel with the global Playwright + preinstalled Chromium: `NODE_PATH="$(npm root -g)" node script.js`, with `require('playwright').chromium.launch()` (no `executablePath` needed, `PLAYWRIGHT_BROWSERS_PATH` is preset). `oathtool` is installed by the environment setup script.
...
- **Every `native.cocoapods` module needs the custom-config → NativeBuildType map, not just :mobile:app:** a new cocoapods KMP module (e.g. `:infra:admob`, PR #422) without `xcodeConfigurationToNativeBuildType["Dev Debug"] = NativeBuildType.DEBUG` (+ Staging/Prod entries, mirroring mobile/app/build.gradle.kts:44-47) fails EVERY flavored-scheme xcodebuild with `Could not identify build type for Kotlin framework '<baseName>'` — Linux CI can't catch it; it surfaces only on a local iOS build. When adding a cocoapods module, copy the 4 mapping lines.
- **App link fails `Undefined symbols: _OBJC_CLASS_$__TtC6Sentry…` (or RevenueCat/GAD symbols) → the CHECKED-IN `mobile/app/app.podspec` is stale:** the Podfile consumes `pod 'app', :path => '../mobile/app'`, so the pod dependency list comes from the committed podspec, NOT live Gradle state — a cocoapods-block edit (new linkOnly pod) without regenerating leaves `pod install` reporting "8 total pods" (GoogleSignIn tree only) and the final `Ld NearYouID.debug.dylib` missing every newer pod. Fix: `./gradlew :mobile:app:podspec` (regenerates in place), re-run `dev/scripts/ios-pod-install.sh`, rebuild — and COMMIT the regenerated podspec whenever the cocoapods block changes.
...
- **iOS app terminates at launch with `GADInvalidInitializationException` → `GADApplicationIdentifier` missing from `iosApp/iosApp/Info.plist`:** the Google Mobile Ads SDK hard-crashes at launch without it. The key ships Google's official iOS TEST app id (`ca-app-pub-3940256099942544~1458002511`), mirroring the Android test id in `infra/admob`'s manifest — don't remove it; migrate both platforms together when real AdMob registrations land. iOS-only + launch-time, so Linux CI and the Android farm never see it.
- **Maestro iOS: system permission alerts (notifications etc.) are NOT tappable by text** — they live outside the app's a11y tree, so `tapOn: "Don't Allow"` fails the flow. Use `tapOn: point:` percentages from a screenshot. Also: the FIRST tap right after `simctl launch` (or right after an alert dismissal) gets eaten by the settling animation — lead flows with `waitForAnimationToEnd` and expect to re-tap once.
