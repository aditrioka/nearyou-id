...
**Action items:**
- [ ] Provision the `admin_app` Postgres role in staging Supabase per [`docs/08-Roadmap-Risk.md`](docs/08-Roadmap-Risk.md) Pre-Phase 1 #28: `CREATE ROLE admin_app LOGIN PASSWORD <secret>; GRANT <enumerated scoped CRUD perms per docs/07-Operations.md § Data Access Pattern>; REVOKE UPDATE, DELETE ON admin_actions_log FROM admin_app;`. Run [`dev/scripts/provision-admin-app-staging.sh`](dev/scripts/provision-admin-app-staging.sh) (lands in this same PR) — it adapts the `promote-staging-user.sh` Cloud-Run-Job pattern, is idempotent, and the 4 admin-app secret slots in GCP Secret Manager are already created (2026-05-17). Note: `LOGIN` (not `NOLOGIN` — earlier rev of this entry had a typo; application-use roles need LOGIN).
