...
**Cap note (2026-06-07):** `mobile-bottom-nav-sections-and-notifications` apply §13 added 5 spec-mandated deferral entries (`mobile-notifications-deep-link-targets`, `mobile-notifications-actor-username-enrichment`, `in-app-notifications-spec-wire-reconciliation`, `mobile-profile-section-screen`, `mobile-notifications-live-unread-badge`) + extended the existing `mobile-nearby-timeline-infinite-scroll` (net +5) → **37 open, 7 over the 30-entry hard cap**. All 5 are deferred-by-design negative-guard follow-ups (each MODIFY-anchors a spec requirement or is a bucket-b docs reconciliation), not rot. Per task 1.4 a sweep was due BEFORE adding; the user explicitly chose **add-now + triage in a dedicated `/triage-follow-ups` session** (fresh context — the sweep reads/triages all 37 and may spawn changes). **Triage is now firmly OVERDUE (7 over)** — the 2026-06-04 test-coverage bundle + these notifications entries (which collapse to deletions as #159 / the profile / live-badge changes ship) are the drawdown levers.
