...
**Add note (2026-06-08, `mobile-home-shell-redesign` apply §8.3).** This change adds **2** spec-obliged deferred-requirement entries below — `mobile-localization-language-switching` (the `mobile-design-system` § "Runtime user-selectable language switching is deferred" requirement's tracked owner) and `mobile-location-disambiguation-onboarding-hint` (the relocated disambiguation hint owner, per the removed `timeline_nearby_title` header / design D6) — taking the on-branch count to **37**. Both are mandated by the change's spec deltas (negative-guard requirement + docs amendment), not rot. The GitHub-Issues migration remains the structural drawdown lever.

---

## mobile-localization-language-switching

**Discovered during:** `mobile-home-shell-redesign` (the `mobile-design-system` substrate) — § "User-facing labels are single-language Bahasa Indonesia" normalizes the catalog copy to one language but deliberately does NOT introduce runtime language switching.
**Status:** open

**Finding:** The single-language Bahasa Indonesia rule (`openspec/specs/mobile-design-system/spec.md` § "User-facing labels are single-language Bahasa Indonesia") is satisfied by normalizing the catalog copy — there is **no** user-facing language selector, **no** `values-en`/`values-id` locale-variant split in [`shared/resources/src/commonMain/composeResources/values/strings.xml`](shared/resources/src/commonMain/composeResources/values/strings.xml), and **no** language-preference persistence. The companion negative-guard requirement `mobile-design-system` § "Runtime user-selectable language switching is deferred" names THIS entry as its tracked owner: the live capability (locale resource variants + a language-preference store + a settings toggle) will MODIFY that requirement.

**Specs at fault:** none — `openspec/specs/mobile-design-system/spec.md` § "Runtime user-selectable language switching is deferred" defers this deliberately.
**Code at fault:** none — single-language-by-construction is the intended v1 shape.
**Docs at fault:** none — `docs/03-UX-Design.md` § Material 3 Design System / Foundation records the deferral.

**Impact (if shipped):** the app is Bahasa-Indonesia-only with no in-app language switch. Acceptable pre-launch (Indonesia-only, 18+). A future English (or other) locale needs the locale-variant infrastructure + a preference store before any `values-en` copy is added.

**Action items:**
- [ ] File an OpenSpec change `mobile-localization-language-switching` introducing locale resource variants (`values-en` etc.), a language-preference store, and a Settings language toggle — MODIFYING `mobile-design-system` § "Runtime user-selectable language switching is deferred" to the live capability.
- [ ] Delete this entry once runtime language switching ships.

---

## mobile-location-disambiguation-onboarding-hint

**Discovered during:** `mobile-home-shell-redesign` design D6 — removing the redundant `timeline_nearby_title` "Post dari lokasi ini" Nearby header (it duplicated the Beranda section + Sekitar tab and re-applied the status-bar inset). The header had carried the posts-from-this-location vs people-around-you disambiguation, which moves to the one-time onboarding hint.
**Status:** open

**Finding:** The Nearby screen header is removed ([`NearbyTimelineScreen.kt`](mobile/app/src/commonMain/kotlin/id/nearyou/app/screens/timeline/NearbyTimelineScreen.kt) no longer renders `timeline_nearby_title`; the string is retained-but-unreferenced in [`strings.xml`](shared/resources/src/commonMain/composeResources/values/strings.xml)). The disambiguation it carried is relocated to (a) the per-card "Diposting dari {city}" context (already present on the post cards) and (b) the **one-time onboarding hint** "NearYouID menampilkan post berdasarkan lokasi saat post dibuat, bukan lokasi terkini penulis" (`docs/03-UX-Design.md` § UX Copy Strategy line 16) — which is **specified but not yet implemented** as a real surface. Until it ships, the relocation is documented (docs amended in this change) but the hint itself is not shown to users.

**Specs at fault:** none — `openspec/specs/mobile-nearby-timeline/spec.md` § "NearbyTimelineScreen renders the Nearby feed surface" (the header-removal requirement) cites the relocation; the hint's own implementation is out of this change's scope.
**Code at fault:** none — no onboarding-hint surface exists yet (it was never built; the header was the only disambiguation surface and it is now removed).
**Docs at fault:** none — `docs/02-Product.md` + `docs/03-UX-Design.md` § UX Copy Strategy are amended (2026-06-08) to record the relocation.

**Impact (if shipped):** between this change and the onboarding-hint change, the posts-from-this-location disambiguation rests solely on the per-card "Diposting dari {city}" context (the screen header is gone, the onboarding hint is not yet shown). Low risk pre-launch, but the explicit anti-misinterpretation hint the copy strategy mandates is temporarily absent.

**Action items:**
- [ ] File an OpenSpec change `mobile-location-disambiguation-onboarding-hint` implementing the one-time onboarding hint (shown once on first authenticated Nearby entry, dismiss-and-persist) carrying the canonical copy from `docs/03-UX-Design.md` § UX Copy Strategy line 16.
- [ ] Delete this entry once the onboarding hint ships.

---

## mobile-notifications-inset-free-substrate-migration

**Discovered during:** `mobile-home-shell-redesign` apply §10.3b — the new `mobile-design-system` substrate's § "The app shell owns a single Scaffold and window insets" requires that **all** section content (including the Notifikasi section) be inset-free (NO nested `Scaffold`/`TopAppBar`). The change's spec scenarios + file scope cover only the 4 Home-surface files; the Notifikasi section was out of scope.
**Status:** open

**Finding:** [`NotificationsScreen.kt`](mobile/app/src/commonMain/kotlin/id/nearyou/app/screens/notifications/NotificationsScreen.kt:117) (shipped by #162) still wraps its body in its own `Scaffold` + `TopAppBar` (the title + "Tandai semua dibaca" action). Under the new single-Scaffold shell this is **functionally harmless** — `AppShellScreen` now calls `Modifier.consumeWindowInsets(innerPadding)`, so the nested Notifications `Scaffold`'s `contentWindowInsets` resolves the system bars to zero (no double status-bar gap). But it violates the **letter** of the substrate requirement ("section content ... MUST NOT wrap its body in its own `Scaffold` or `TopAppBar`"). The Notifikasi `TopAppBar` is a real functional app bar (it hosts the mark-all-read action), not a redundant header, so the migration is a consistency/refactor item, not a live bug. The substrate source-scan guard (`ShellAndTimelineSourceGuardTest`) is therefore scoped to the 4 Home-surface files this change declares, per the `mobile-design-system` § "Only the shell declares a Scaffold" scenario.

**Specs at fault:** none functionally — `openspec/specs/mobile-design-system/spec.md` § "The app shell owns a single Scaffold and window insets" states the universal rule; this entry is the tracked owner for bringing the already-shipped Notifikasi section into letter-compliance.
**Code at fault:** `mobile/app/src/commonMain/kotlin/id/nearyou/app/screens/notifications/NotificationsScreen.kt:117` (nested `Scaffold` + `TopAppBar`).
**Docs at fault:** none.

**Impact (if shipped):** none today (the shell consumes insets, so no double-gap). The Notifikasi section's app bar stays an explicit `TopAppBar` until migrated; a future reader could read the substrate rule as already-universal when one section is not yet compliant.

**Action items:**
- [ ] Migrate `NotificationsScreen` to be inset-free (drop its `Scaffold`/`TopAppBar`; render the title + mark-all-read action inline under the shell's consumed padding, mirroring the Home feeds) — bundle with the next notifications-touching change or a small substrate-migration PR.
- [ ] Extend the `ShellAndTimelineSourceGuardTest` Scaffold-absence assertion to cover `NotificationsScreen` once migrated.
- [ ] Delete this entry once the Notifikasi section is inset-free.
