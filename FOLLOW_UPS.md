...
- **Hard limit: max 30 open entries.** When breached, force a triage sweep before adding new entries; entries open for >2 weeks are candidates for migration to GitHub Issues if the team grows beyond solo. Audit on 2026-05-08 (PM triage sweep, post-PR #74 merge) found 25 open + 0 triaged; below the hard limit. The PM sweep deleted 1 silently-resolved entry (`rate-limit-applies-to-health-endpoints` — addressed by Cloud Run probe-User-Agent bypass shipped in PR #74's `health-check` capability hardening), surfaced 1 OpenSpec promotion (`rate-limit-infrastructure-success-path-key-log-drift` → handed off to `/next-change`), and surfaced 2 regular-PR scope bundles (FCM-test bundle `fcm-payload-structural-tests` + `fcm-shutdown-drain-deterministic-tests` + `fcm-end-to-end-composite-test`; moderator-spy pair `reply-rate-limit-moderator-spy` + `chat-block-check-moderator-spy`). Earlier 2026-05-08 actions (see PRs #73 + #74) shipped: an AM sweep with 1 deletion (`fcm-firebase-import-boundary-detekt-rule` — covered by `VendorSdkLeakageScanTest`) + 1 migration (`grafana-otlp-token-scope-tightening` → `docs/08-Roadmap-Risk.md` Pre-Launch); and an atomic-with-PR-#74 replacement (`rate-limit-key-includes-raw-client-ip` deleted, `rate-limit-infrastructure-success-path-key-log-drift` added per §6.5 pre-archive verification). Of the 25 remaining entries: 10 deferred-by-trigger (Phase 3.5 schema, rule of three, user-growth signals, SDK upstream fixes), 9 OpenSpec-shaped pending promotion via `/next-change`, 6 regular-PR-shaped test/doc tightening work.
