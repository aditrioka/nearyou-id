...
/**
 * The first content-creation surface — the post composer, pushed onto the Voyager navigator by the
 * home-surface FAB. Renders a title, a multiline content field, a live Unicode-code-point `N/280`
 * counter, an outcome-driven error banner, and a "Posting" CTA, all under `NearYouTheme` (applied by
 * the host) with every string via `stringResource` (zero literals).
 *
 ...
 */
class PostCreationScreen : Screen {
    @Composable
    override fun Content() {
        ...
        // Navigate from an effect (never mutate the navigator during composition). On Success the
        // composer pops back to the home surface; Nearby auto-refresh on return is deferred
        // (mobile-post-creation-refresh-nearby-on-return) — NO Nearby reload is signalled here.
