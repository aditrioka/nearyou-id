...
/**
 * The first product surface — the authenticated Nearby feed. Injects [NearbyTimelineFlow] via Koin,
 * holds the [NearbyTimelineOutcome] + in-flight flag in `remember`, and (re)loads via a
 * `LaunchedEffect` keyed on a reload counter (so pull-to-refresh + error-retry both re-fetch the
 * first page). Renders the six states (loading / content / empty / error / rate-limit-hard /
 * rate-limit-soft) per the screen-state-mapping spec, all copy via `stringResource` (zero literals),
 * under `NearYouTheme` tokens. Holds NO navigation dependency (it does not push/replace screens), so
 * `HomeScreen` can embed `Content()` directly.
 */
class NearbyTimelineScreen : Screen {
    ...
    override fun Content() {
        ...
        // Incrementing this re-keys the LaunchedEffect → re-fetches page 1. Both pull-to-refresh
        // and the error-retry control increment it (shared reload path). `next_cursor` is retained
        // on Loaded but NOT consumed for load-more (deferred to mobile-nearby-timeline-infinite-scroll).
