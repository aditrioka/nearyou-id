...
/**
 * PII-discipline guard for `mobile-settings` (mirrors `ConsentSourceGuardTest`). Comments are stripped
 * first so a KDoc legitimately naming a term does not trip the scan:
 *
 ...
 *  3. mobile-data-export-entry (task 5.4): the data-export affordance resolves every user-visible string
 *     via `Res.string.*` (no hardcoded literal); the seam + the URL-handling screen log no token /
 *     `Authorization` / JWT `sub` / `downloadUrl`; and no call site persists the `downloadUrl`.
 *
...
class SettingsSourceGuardTest {
    ...
    // The seam sources PLUS the screen — `SettingsScreen.kt` is the one place the signed `downloadUrl` is
    // actually dereferenced (`uriHandler.openUri(ready.downloadUrl)`), so the log/persist guards MUST scan
    // it too (else a future `log(downloadUrl)` / DataStore write on the screen would slip past CI). The
    // file-level auth-header denylist deliberately does NOT include the screen (it legitimately imports
    // `TokenStore` for the logout path).
    private val dataExportUrlHandlingSources = dataExportSeamSources + settingsScreenPath
    ...
    fun dataExportSeam_logsNoTokenSubOrDownloadUrl() {
        // File-level: the seam neither handles nor references the auth header / bearer token at all (the
        // screen is excluded here — it legitimately imports TokenStore for logout).
        ...
        // Line-level: no logging call site in the seam OR the URL-handling screen echoes a forbidden value
        // (downloadUrl most of all). `sub` is word-boundary-matched so it can't false-trip on "subscription".
        val forbiddenSubstrings =
            listOf("downloadUrl", "downloadExpiresAt", "Authorization", "Bearer", "accessToken", "refreshToken")
        val jwtSub = Regex("""\bsub\b""")
        for (path in dataExportUrlHandlingSources) {
            stripped(path).lineSequence().forEach { line ->
                val isLogCall = line.contains("log(", ignoreCase = true) || line.contains("println")
                if (isLogCall) {
                    ...
                    assertTrue(
                        !jwtSub.containsMatchIn(line),
                        "$path logs the JWT 'sub' — the data-export diagnostics must be status/type-only (PII discipline)",
                    )
                }
            }
        }
    ...
    // mobile-data-export-entry § "The signed download URL is not persisted": no seam/VM/screen call site
    // writes the downloadUrl (or any export field) to DataStore / preferences / disk / any cache — it lives
    // only transiently in UI state for the open-URL hand-off (the screen hands it straight to `openUri`).
    @Test
    fun dataExportSeam_doesNotPersistDownloadUrl() {
        val persistenceApis =
            listOf("DataStore", "SharedPreferences", "putString", "preferencesKey", "writeText", "FileOutputStream")
        for (path in dataExportUrlHandlingSources) {
            val text = stripped(path)
