...
## Mobile-first to full-demo: COMPLETE — now Balanced priority (as of 2026-06-14)

The backend is MVP-ready (19 Flyway migrations, ~60 OpenSpec capabilities, Detekt-enforced invariants; Phase 1 + Phase 2 shipped). Two priority phases are now **complete**: the 2026-05-12 mobile + admin **scaffolding** phase (all 10 scaffold changes — 5 mobile, 5 admin — merged) and the 2026-06-07 **mobile-first** phase (finish the mobile app to a full-demo state ahead of admin). The mobile-first flip trigger fired **2026-06-13** — the mobile demo loop is complete (the 5 critical-path picks #245–#249 + post-detail #159 + bottom-nav/notifications #162 squash-merged). **New direction (operator decision, 2026-06-14): Balanced — no single priority.** `/next-change` and new OpenSpec work pick the **highest-value item by judgment each cycle** across three live lanes — **admin** (resume remaining surfaces), **Phase 4 / premium** (premium / billing / image upload — the revenue loop), and **mobile follow-ups** (polish + deferred items) — surveying in-flight PRs first to dedup. **The three lanes are prioritization buckets, not layer boundaries** — a single user-facing capability may span lanes and MUST ship its full vertical slice (or declare deferred layers as explicit requirements) per [`docs/12-Integration-Contracts.md`](docs/12-Integration-Contracts.md); don't force-split a feature to fit one lane. No lane is privileged; backend hardening stays blocker-only (security invariant gap, pre-launch test requirement, or a dependency for active work).

**Current state (descriptive):**

- **Mobile (`:mobile:app`)** — real Compose Multiplatform app (Navigation 3 + Koin DI + Material 3 light/dark theme + `:shared:resources` CMP Resources); **full demo loop shipped**: sign-in → age gate → Nearby/Following/Global → post creation → post detail → like/reply → profile + follow → notifications → 1:1 chat → premium-gated search → settings (the 5 critical-path picks #245–#249 + post-detail #159 + bottom-nav/notifications #162). Remaining mobile work is follow-up polish, not demo-blocking (in flight: follower/following lists #298, Sentry KMP #299).
- **Admin (`backend/ktor/.../admin`)** — well past scaffold (schema V16, Argon2id + TOTP login, audit-log viewer, suspend/unban, report-queue triage + in-row resolution actions, rejected-identifiers viewer, block registry, privacy-flip monitor; UI intentionally unstyled so far). **Active again** under the balanced cadence (no longer deferred); more surfaces in flight (#296 dashboard, #294 reserved-usernames, #290 chat redaction, #297 feature-flags). The full target surface (existing + planned) is frozen visually in the admin mockup board (`docs/11` § 3.6).

**Forward direction.** The mobile contracts in `docs/02-Product.md` / `docs/03-UX-Design.md` / `docs/04-Architecture.md` are the spec source for the remaining screens. The **canonical visual reference** is the mockup board set [`dev/mockups/`](dev/mockups/README.md) — every UI-affecting change (whatever the skill: `/next-change`, `/opsx:apply`, `mobile-ui-foundation`, …) MUST consult the matching frame before building: render the HTML (browser / preview panel / screenshot tool — agent's choice), generate the per-frame **measurement annex** for exact spacing/typography/tokens/animation (`dev/scripts/mockup-measure.sh <board> <frame-no>` — `dev/mockups/README.md` step 4; on-demand output, never committed), and translate to the surface's idioms — Compose Multiplatform per `docs/11` § 2.8 (mobile boards), **Pebble + HTMX + vendored CSS per `docs/11` § 3.6** (admin board `nearyou-admin-mockup.html`, 23 frames covering every admin feature, shipped + planned). On behavior conflicts, specs/docs win over mockups. The **priority section** — phase history (scaffold + mobile-first, both complete), the now-fired flip trigger (2026-06-13), and the current **Balanced** cadence across the admin / Phase 4-premium / mobile-follow-up lanes — lives at [`openspec/project.md`](openspec/project.md) § Mobile-First to Full-Demo Priority; read it and survey in-flight PRs before picking a lane.
...
- [`docs/12-Integration-Contracts.md`](docs/12-Integration-Contracts.md) — **cross-layer cohesion contract**: a user-facing capability ships its full backend+admin+mobile vertical slice, or declares each deferred layer as an explicit requirement. MUST-read for any multi-layer change.
- [`docs/13-Test-Matrix.md`](docs/13-Test-Matrix.md) — **test ground truth**: which test/check runs where (local / CI / staging) and at which stage, reconciled to `.github/workflows/`. The map skills follow for where/when to run tests.
...
- **Start-of-change preflight** (`/opsx:preflight`): `/next-change` runs it at Phase B.5 (after scaffold, before handoff to `/opsx:apply`) to surface operator/human-required tasks (credentials, GCP provisioning, store/dashboard config, physical-device verify), cross-layer cohesion gaps ([`docs/12-Integration-Contracts.md`](docs/12-Integration-Contracts.md)), and unmapped test scenarios — delegated to the operator at proposal time, not discovered mid-apply as follow-ups. `/opsx:apply` re-checks the Preflight **Human-required tasks** block as a precondition before implementing dependent tasks.
...
- **Pre-push verification**: before pushing a feat / fix / chore branch, run `./gradlew ktlintCheck detekt :backend:ktor:test :lint:detekt-rules:test` locally. CI runs **both** lint frameworks; passing only `detekt` is insufficient (precedent: PRs #31 + #32 hit CI lint failures after skipping `ktlintCheck` locally). **This gate is not the whole of CI** — three checks run only in CI (the Dockerfile↔`settings.gradle.kts` module-copy guard, the admin static-asset SHA256SUMS integrity check, and the supabase-parity Flyway migrate), and iOS is never CI-gated. For the full layer→environment→stage map + the CI-equivalent local recipe (fresh containers, the `!network` not `!database` tag), see [`docs/13-Test-Matrix.md`](docs/13-Test-Matrix.md).
...
## Cloud session parity (cloud sandbox)

A cloud session is made to behave like a local one by two layers (full runbook, the paste-in setup script, and the parity table per layer: [`dev/docs/cloud-environment.md`](dev/docs/cloud-environment.md)):

- **Environment setup script** (entered once in the claude.ai/code environment settings, snapshotted): runs [`scripts/setup_cloud_env.sh`](scripts/setup_cloud_env.sh). It installs PostGIS, oathtool, JDK 17, the Android SDK, the Google Cloud SDK, the CI-parity Docker images, the OpenSpec CLI, a migrated Postgres cluster and a warm Gradle cache.
- **SessionStart hook** (the only hook in [`.claude/settings.json`](.claude/settings.json)): [`scripts/session_start.sh`](scripts/session_start.sh). Cloud only: starts Postgres :5433 + Redis :6379 + `dockerd`, generates `dev/.env` ([`scripts/setup_dev_env.sh`](scripts/setup_dev_env.sh), `KTOR_ENV=test`, throwaway secrets), and exports `JAVA_HOME`/`ANDROID_HOME`/`DB_*`. Local: OpenSpec + banner + health checks only, never provisions. Hooks in one group run in parallel, so keep new session-start steps inside this script instead of adding hook entries.
- **Not closable in the cloud:** iOS (needs macOS; use `claude remote-control` on a Mac), and device runs reach only `api-staging`, not the sandbox backend.
...
> **Mobile-test routing rule (always-on):** to run / verify the mobile app, call **`scripts/test_android.sh`** (instrumented) or **`scripts/run_on_device.sh`** (see-it-run Robo) — NOT `gradlew connected…` / `gcloud firebase test` directly. They auto-route via [`scripts/_testing_context.sh`](scripts/_testing_context.sh): **local session → emulator/connected device; cloud sandbox → Firebase Test Lab.** Detection is capability-first (an attached adb device wins) + cloud-**container** markers (`CLAUDE_CODE_CONTAINER_ID` / `CLAUDE_CODE_REMOTE_ENVIRONMENT_TYPE`); it deliberately does NOT key on `CLAUDE_CODE_REMOTE` / `IS_SANDBOX` (those can be set locally by `claude --remote-control` / `--sandbox`). Override with `FARM=` or `FORCE_TESTING_CONTEXT=`. The `SessionStart` hook prints the active context every session.
...
- **Health check**: `scripts/verify_env.sh` — asserts `java`, `sdkmanager`, `adb`, `gradlew` + required SDK components; non-zero on any miss. Run by the `SessionStart` hook ([`scripts/session_start.sh`](scripts/session_start.sh)), so every session reports env health up front.
...
- **Run instrumented tests**: `scripts/test_android.sh` is the context-aware wrapper (see the routing rule above) — local+device → `connectedAndroidTest`; cloud → farm; local+no-device → guidance to start an emulator. Direct entry points: `scripts/test_firebase.sh` (Firebase Test Lab; needs `GCP_SA_KEY_JSON` or `GOOGLE_APPLICATION_CREDENTIALS` + `FIREBASE_PROJECT_ID`) or `scripts/test_browserstack.sh` (BrowserStack App Automate; needs `BROWSERSTACK_USERNAME` + `BROWSERSTACK_ACCESS_KEY`). Credentials are env-var-only — never hard-coded.
...
- **Provisioning**: the `SessionStart` hook ([`scripts/session_start.sh`](scripts/session_start.sh)) runs `scripts/setup_openspec.sh` every session. The container is ephemeral (global npm installs don't survive a reclaim) and the install is small + idempotent (~4 s, no-ops when the pinned version is already present), so it self-heals each session. Run it by hand any time with `bash scripts/setup_openspec.sh`.
