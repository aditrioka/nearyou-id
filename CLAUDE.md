...
- **Firebase Test Lab is the recommended device farm** (free tier for iteration, pay-per-use, same GCP/IAM as the rest of the stack); BrowserStack is the optional fallback. Full setup + the one-time operator step (enable APIs, service account, secrets) + cost: [`dev/docs/device-farm.md`](dev/docs/device-farm.md).
