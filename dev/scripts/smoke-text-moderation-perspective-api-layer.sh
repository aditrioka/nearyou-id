...
# deploy of Layer 3 (Google Perspective API toxicity classifier wired as an
...
#    for the structured `event=perspective_high_score_applied` log line that
...
# `event=perspective_high_score_applied` log line is emitted AFTER the
...
#
# Prerequisites:
#  - `staging-perspective-api-key` secret slot provisioned in GCP Secret Manager
#    AND wired into `.github/workflows/deploy-staging.yml --set-secrets` as
#    `PERSPECTIVE_API_KEY=staging-perspective-api-key:latest`. Without this,
#    the Cloud Run boot fails-fast at `event=perspective_init_failed
#    reason=missing_secret`.
#  - `perspective_api_enabled = TRUE` in staging Firebase Remote Config (Server
#    template). Already seeded per Pre-Phase 1 §18; idempotent verification.
...
# accommodates outliers (Cloud Logging ingestion lag, slow Perspective response).
...
# Smoke sentinel content — a phrase Google Perspective scores well above 0.8 on
# TOXICITY/THREAT but that's clearly a test (so operators reading the staging
...
# hostile phrase Perspective consistently scores >0.9. Operators MAY clean
# these test rows up periodically via the queue cleanup script.
HIGH_TOX_MESSAGE="SMOKE-TEST Perspective Layer 3: kill yourself you idiot, this is automated test content"
...
# ----------------------------------------------------------------------------
# Cloud Logging assertion: orchestrator emitted `event=perspective_high_score_applied`
# for the target. This proves: (a) Perspective returned a score >0.8;
...
# DefaultPerspectiveModerator.applyAutoHide).
# ----------------------------------------------------------------------------
echo "=== smoke: Cloud Logging — looking for perspective_high_score_applied for post $POST_ID ==="
LOGS_HIGH="$(gcloud logging read --project="$GCP_PROJECT" \
    "resource.type=\"cloud_run_revision\" AND timestamp>=\"$SINCE_TS\" AND textPayload:\"event=perspective_high_score_applied\" AND textPayload:\"target_id=$POST_ID\"" \
    --freshness=5m --limit=10 --format='value(textPayload)' 2>&1 || true)"

if [[ -z "$LOGS_HIGH" ]]; then
    echo "FAIL: no perspective_high_score_applied log entry found for target_id=$POST_ID since $SINCE_TS." >&2
    echo "      Possible causes:" >&2
    echo "        1. Perspective scored the content <= 0.8 (smoke phrase is no longer triggering)." >&2
    echo "        2. perspective_api_enabled = FALSE in staging Remote Config." >&2
    echo "        3. PERSPECTIVE_API_KEY missing or invalid — check for event=perspective_dispatch_failed." >&2
...
# ----------------------------------------------------------------------------
echo "=== smoke: Cloud Logging — verifying no perspective_init_failed / kill_switch_unavailable ERROR events ==="
LOGS_INIT_FAILED="$(gcloud logging read --project="$GCP_PROJECT" \
    "resource.type=\"cloud_run_revision\" AND timestamp>=\"$SINCE_TS\" AND textPayload:\"event=perspective_init_failed\"" \
    --freshness=5m --limit=5 --format='value(textPayload)' 2>&1 || true)"
if [[ -n "$LOGS_INIT_FAILED" ]]; then
    echo "FAIL: perspective_init_failed seen — Perspective API key is missing or invalid." >&2
    echo "$LOGS_INIT_FAILED" | head -3 | sed 's/^/      /' >&2
    exit 1
fi
echo "PASS: no perspective_init_failed entries (Cloud Run boot succeeded)"

LOGS_KILLSWITCH_ERR="$(gcloud logging read --project="$GCP_PROJECT" \
    "resource.type=\"cloud_run_revision\" AND timestamp>=\"$SINCE_TS\" AND textPayload:\"event=perspective_kill_switch_unavailable\"" \
    --freshness=5m --limit=5 --format='value(textPayload)' 2>&1 || true)"
if [[ -n "$LOGS_KILLSWITCH_ERR" ]]; then
    echo "WARN: perspective_kill_switch_unavailable seen — Firebase Remote Config read failed during this window." >&2
...
echo "     perspective_threshold_misconfigured / perspective_kill_switch_unavailable"
echo "     ERROR events from the smoke window. WARN-level perspective_dispatch_failed"
