...
- **Mobile larger refactors (deliberate, risk-bounded — each is a multi-file refactor with test migration, sized for its own change/PR rather than an audit drive-by):** 05-#5 (5 remember-only screens → entry-scoped ViewModels) is **DONE**: `PostCreationScreen`/`PostDetailScreen` shipped via sibling changes (both own VMs), and SignIn/AgeGate/Consent shipped via `mobile-auth-flow-viewmodels` (PR [#405](https://github.com/aditrioka/nearyou-id/pull/405) — 3 entry-scoped VMs on the single-`stateIn` shape, config-change state-survival + viewModelScope submits). `ui/components/` extraction (05-#11, the docs/11 §2.1 first structural move) — **post-card half SHIPPED** via `mobile-timeline-card-redesign` (PR #221: shared `ui/components/PostCard.kt` + `LetterAvatar.kt`, per-screen `NearbyPostCard`/`GlobalPostCard` deleted, `mobile-post-card` spec added); **list-state kit half ✔ SHIPPED via PR [#407](https://github.com/aditrioka/nearyou-id/pull/407)** (`ListStates.kt` = `ListScrollableState`/`ListLoadingState`/`ListCenteredMessageState`/`ListErrorState` + `SoftLimitBanner`; generic `PostFeedList<T>`; Nearby/Global/Following/Notifications migrated, ~9 near-verbatim private copies deleted, docs/11 §2.1 Pattern-Registry note). Still remaining: the `PostDetailScreen` `RepliesLoading`/`RepliesError`/`RepliesEmpty` partials + the `PostHeader` card-header near-copy (DEFERRED in #407 — the Replies\* states live in a different layout context: items inside the detail's shared `LazyColumn` with an `OutlinedButton` idiom + own test tags, so folding them risks drift on a heavily-tested screen), `screens/` package restructure (D6, not started). **05-#9 (shell unread VM) — ✔ SHIPPED via PR [#411](https://github.com/aditrioka/nearyou-id/pull/411)** (the Notifikasi unread-badge fetch moved off the `AppShellScreen` composable into a shell-scoped `AppShellViewModel`: one-shot `init` fetch + `refreshUnreadCount()` on leaving Notifikasi, single-`stateIn` `uiState`, fixing the docs/11 §2.2 data-in-composable violation + the silent rotation-refetch bug; the `AppShellBadgeOneShotScanTest` one-shot guard now follows the fetch into the VM, and a new `AppShellViewModelTest` pins it). **05-#16 (TokenRefresher follower-CE translation) — ✔ shipped via PR [#406](https://github.com/aditrioka/nearyou-id/pull/406)** (leader runs its refresh round-trip + in-flight cleanup under `NonCancellable`, so a leader-coroutine cancellation no longer poisons followers with a foreign CE / strands a dead deferred in `inFlight`; regression test proven to fail pre-fix). **05-#12 (LocationGate fold) — ✔ SHIPPED via PR [#412](https://github.com/aditrioka/nearyou-id/pull/412)** (the Nearby `LocationGate` moved off composition-scoped `remember { LocationGate(controller) }` into a `HomeRoute`-scoped `LocationGateViewModel` resolved via `viewModel { … }` at the top of `NearbyTimelineScreen`; the gate state now survives a swipe back to Nearby instead of rebuilding to `Loading` + re-querying the OS, removing the screen's third holder kind — the mixed VM + custom-holder + `remember` pattern the finding flagged. `LocationGate` is **kept** as the Compose-free orchestrator the VM owns as a VM-internal collaborator (mirroring `NearbyTimelineViewModel`'s `InlineLikeController`/`LoadMoreController`), so `openspec/specs/mobile-location` + `LocationGateTest` are undrifted; the `ON_RESUME` return-from-Settings re-check is preserved, the `viewModelScope.launch`es move off the composable, and a new `LocationGateViewModelTest` pins the delegation). **Re-scoped (stale-sketch correction, PR #405):** 05-#6/#7 are NO LONGER folded into 05-#5, and `viewModel { }` (not `koinViewModel()`) is the de-facto convention across ~17 VMs. **05-#6 = single-`stateIn` retrofit of the feed/list VMs — ✔ SHIPPED via PR [#409](https://github.com/aditrioka/nearyou-id/pull/409)** (`mobile-feed-viewmodels-statein`). Two stale-sketch corrections applied at ship: (a) it was **4 VMs, not 3** — `FollowingTimelineViewModel` post-dated the audit and copied the same multi-`StateFlow` shape, so the prior "new VMs already use single-`stateIn`" note was FALSE for the feed family (the auth/paywall/username/followlist VMs do; the 4 feed VMs did not) — so Nearby/Global/Following/Notifications were all folded to one `uiState` (`combine(_outcome, initialLoad).stateIn(WhileSubscribed(5000))` delegating to the unchanged pure projection; `isInitialLoad` made private; `outcome` retained as the raw-domain-state seam carrying cursor/anchor paging the PII-free `uiState` strips); (b) 05-#13 (`collectAsStateWithLifecycle`) was already shipped. The identical fork in `ConversationListViewModel`/`BlockedUsersViewModel` (chat/settings, out of audit-05 scope) is deferred to issue [#410](https://github.com/aditrioka/nearyou-id/issues/410). **05-#7 = the app-wide `koinViewModel()` + Koin VM-declaration conversion** of all ~17 `viewModel { }` call sites (an app-wide decision + a docs/11 §2.2-vs-code divergence to resolve, NOT a drive-by) — **still open**.
