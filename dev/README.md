...
- **Postgres binds host port 5433**, not the default 5432, to avoid colliding
  with a native Postgres install; `DB_URL` in `.env.example` matches. If
  nothing else is on 5432 you can change both back.
...
- **Container names are `nearyouid-dev-postgres` / `nearyouid-dev-redis`** —
  ...
  earlier prototypes. If `docker ps -a` shows old containers blocking
  creation, `docker rm <name>` (volumes are preserved separately).

- **`flywayMigrate` requires `--no-configuration-cache`** — reason in the
  Day-to-day block comment (`Task.project` at execution time, rejected by the
  config cache). Tracked as a TODO in
  `build-logic/src/main/kotlin/nearyou.ktor.gradle.kts`; revisit when the
  Flyway plugin fixes it, or migrate to invoking the Flyway Java API via a
  custom task.

- Plain Postgres has no `realtime` schema — the V2 migration's RLS policy
  block is a no-op locally; it applies when V2 runs against a Supabase
  instance.

- **Signup flow**: `POST /api/v1/auth/signup` (signup-flow change). Body:
  `{ provider, id_token, date_of_birth, device_fingerprint_hash? }`. Dev
  verification against a running Ktor needs a Google/Apple OAuth client that
  issues real ID tokens (proposal task 14.2: a live curl with a hand-rolled
  id_token isn't feasible without a mock JWKS). `SignupFlowTest` covers every
  scenario end-to-end against this same dev Postgres — run with
  ...
  `seed-test-user.sh` remains a shortcut to a usable user row.

- **Post creation**: `POST /api/v1/posts` (auth required). Body:
  `{ content, latitude, longitude }` — content ≤ 280 Unicode code points,
  coords inside the Indonesia envelope (`-11..6.5` lat, `94..142` lng). Happy
  path writes both `actual_location` and `display_location` in one INSERT via
  `ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography`; display is
  deterministically fuzzed via HMAC-SHA256(JITTER_SECRET, post_id). Business
  reads go through the `visible_posts` view (the Detekt rule
...
- **Detekt** runs on `main` sources only and is wired to `check` via the
  `nearyou.ktor` convention plugin. Custom ruleset: `:lint:detekt-rules`;
  config: `backend/ktor/config/detekt/detekt.yml`. Add a rule by extending
  `Rule` in `:lint:detekt-rules`, registering it in `NearYouRuleSetProvider`,
  and activating it in `detekt.yml`. Tests are excluded from scanning on
  purpose — they `SELECT FROM posts` for DB assertions, which is fine.
