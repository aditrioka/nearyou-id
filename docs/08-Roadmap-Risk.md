...
15. Nearby + Following + Global timeline spatial queries (using `display_location`) — **all shipped, trio complete**: Nearby (`nearby-timeline-with-blocks`, V5, `GET /api/v1/timeline/nearby`); Following (`following-timeline-with-follow-cascade`, V6, `GET /api/v1/timeline/following`); Global (`global-timeline-with-region-polygons`, V11 `admin_regions` schema + trigger + V12 552-row OSM polygon seed, `GET /api/v1/timeline/global`)
16. **Block user feature** (schema + endpoints + cascade follow removal + CI lint) — **all shipped**: via `nearby-timeline-with-blocks` (V5 `user_blocks` + `POST/DELETE/GET /api/v1/blocks` + Detekt `BlockExclusionJoinRule`); follow-cascade via `following-timeline-with-follow-cascade` (transactional `user_blocks` INSERT + bidirectional `follows` DELETE in `BlockService.block()`)
...
31. CI lint rules — all ship in Phase 1. The full enumerated list (15 rules, with the extra per-rule detail) is canonical under the **Development Tools** section below; this item exists to record that the whole suite lands in Phase 1.

### Phase 2: Social Features (Weeks 5-6)

1. Follow/unfollow system (using `follows` schema)
2. Following + Global timeline with polygon reverse geocoding (using `actual_location`) — **shipped** (`global-timeline-with-region-polygons`): V11 `posts_set_city_tg` BEFORE INSERT trigger + 4-step fallback ladder strict→buffered_10m→fuzzy_match→NULL; V12 552-row OSM polygon seed incl. 6 DKI kotamadya + 12nm maritime buffer on 48 coastal kabupaten
3. Like + reply (20/day Free limit, 280 chars max, flat structure, block-aware) — **Likes shipped V7 (`post-likes-v7`); reply shipped V8 (`post-replies-v8`)**. **Like rate limit shipped** (`like-rate-limit`): 10/day Free + 500/hour burst (both tiers) on `POST /api/v1/posts/{post_id}/like`, `premium_like_cap_override` Firebase Remote Config flag (Decision 28) for ops-side adjustment without a mobile release. **Reply rate limit shipped** (`reply-rate-limit`): 20/day Free, unlimited Premium on `POST /api/v1/posts/{post_id}/replies` — daily-only (no burst clause per `02-Product.md`; the asymmetry with likes is canonical, not an oversight — replies lack the velocity-fingerprint surface that anti-bot burst caps target), `premium_reply_cap_override` Remote Config flag (mirrors the like flag; canonical authority `05-Implementation.md:1416`), oversized-flag clamp at 10,000 (anti-typo guard), no `releaseMostRecent` escape hatch (every successful POST is a real new row — no idempotent re-action analogous to the like handler's INSERT-ON-CONFLICT no-op). DELETE / GET unaffected (V8 author-only idempotent-204 contract preserved; read-side throttling lives at the timeline session/hourly layer). Auto-hide trigger shipped via `reports` (V9).
...
16. Perspective API integration (stopgap text moderation, 500ms timeout fail-open, kill switch flag, writes to `moderation_queue` on high score) — **shipped** (`text-moderation-perspective-api-layer`): vendor pivoted mid-implementation from Google Perspective → OpenAI Moderation `omni-moderation-latest` after Perspective announced end-of-2026 sunset (OpenSpec change name + Firebase RC flag names + V9 SQL trigger value preserved as historical-artifact carve-outs per the proposal.md Vendor Swap Amendment). Final timeout budget: 3000ms regional baseline for asia-southeast1 (originally 500ms; bumped iteratively to cover empirical bimodal TTFB from Singapore → US OpenAI). Staging-side happy-path observability tracked separately in [#88](https://github.com/aditrioka/nearyou-id/issues/88) (Cloud Run cpu=1 scheduling characteristic; not a Layer 3 code defect; happy path covered by 98 local tests).
...
5. **Security review checklist**:
   ...
   - **Grafana OTLP token least-privilege**: before production tag-deploy, mint the `otel-grafana-otlp-token` slot from a custom Grafana Cloud Access Policy (`nearyou-prod-traces-write-only`, realm = stack `nearyouid`, scope = `traces:write` only) rather than the OTLP setup wizard's bundled-scope token (which over-grants `metrics:write` + `logs:write` + `profiles:write` + `stacks:read`). GCP Secret Manager IAM is the primary defense, but the wizard's token is unacceptable at production tag-deploy. Optional: rotate `staging-otel-grafana-otlp-token` to a custom-policy-generated token at next convenient maintenance window. Update [`docs/10-Setup-Checklist.md`](10-Setup-Checklist.md) § 3.7 to document the Access Policy path as canonical (current wizard path is convenient-but-over-privileged shortcut).
...
8. **Mobile CI lanes (iOS framework-link + Android instrumented encryption test)** — stand up the two mobile CI lanes deferred through the scaffold cycle: (a) a `macos-14` job invoking `./gradlew :mobile:app:linkDebugFrameworkIosSimulatorArm64`, path-filtered to `mobile/**` / `iosApp/**` / `gradle/libs.versions.toml` / `settings.gradle.kts` and wired into the `merge-gate` required checks (iOS build regressions are otherwise caught only by the author running it locally per the standing convention); and (b) a `SecureTokenStoreEncryptionTest` `androidInstrumentedTest` (Tink/DataStore raw-byte-leak + keyset-regeneration assertions) on a KVM-accelerated Linux Android-emulator lane (`reactivecircus/android-emulator-runner`, free Linux minutes), falling back to Firebase Test Lab if the emulator cannot faithfully exercise the Keystore-backed AEAD. A prior WIP attempt is the closed PR [#137](https://github.com/aditrioka/nearyou-id/pull/137). Migrated from `FOLLOW_UPS.md` (`mobile-ios-ci-link-task` + `mobile-auth-signin-android-instrumented-encryption-test`, 2026-06-08 triage).
9. **Per-admin destructive-action rate limit** — ship the ~20/hour-per-admin limiter gating `POST /admin/users/{id}/suspend` + `/unban` (and future admin writes, incl. the deferred `admin-rejected-identifiers-clear-action`), per [`docs/07-Operations.md`](07-Operations.md) § Security ("Rate limit destructive actions: 20/hour per admin"). Substrate (Redis sliding-window vs `admin_actions_log` COUNT) decided at design time — the COUNT cut is authoritative with no fail-soft hole. Motivating threat: a phished admin session (TOTP is phishable) mass-suspending at unbounded rate; interim mitigations (CSRF, idle/absolute session caps, IAP gate, immutable audit trail, reversibility) bound but do not eliminate it. The in-row report-resolution actions (PR [#160](https://github.com/aditrioka/nearyou-id/pull/160)) raise its urgency. Migrated from `FOLLOW_UPS.md` (`admin-destructive-action-rate-limit`, 2026-06-08 triage).
...
**Defer until user growth / feedback signals notification fatigue.** Today every successful chat send produces a `notifications` row AND fans out one FCM push per active recipient token (`chat-message-notification`, PR #65). What did NOT ship is per-conversation push batching: when a sender pumps multiple messages into one conversation in quick succession (a typing burst), the receiver gets one FCM push per message rather than a single coalesced push. At MVP scale this is acceptable noise; at scale it becomes a notification-fatigue + FCM-quota concern.

Batching strategy (decide at change time): (a) debounce-on-send — delay each push ~3s and merge incoming siblings; (b) per-conversation rate-limit — cap pushes per conversation per minute; (c) FCM-side notification grouping — Android `setGroup` + iOS `thread-id` keyed on `conversation_id`. Option (c) is purely client-display batching (no server change, cheapest) and is the likely correct first move; most likely shape is grouping shipped in `:infra:fcm`'s payload builders + a minimal `fcm-push-dispatch` spec amendment. If client-display grouping proves insufficient, add server-side debounce/coalesce in a separate change.

Trigger to act: user growth or feedback signals notification fatigue. _(Migrated from `FOLLOW_UPS.md` in the 2026-06-09 triage sweep — was `chat-message-notification-per-conversation-fcm-batching`.)_

### 35. Moderation-List Cache Invalidation Endpoint

**Defer until Month 3+ data shows the 5-min TTL is too stale.** The moderation list loader uses a 5-minute Redis TTL with NO push-based invalidation (`content-moderation-keyword-lists` design D4). When operators edit the Remote Config wordlist, the change propagates within 5 min max (TTL elapse + next loader call). This is acceptable for moderation — legal-advisor review is quarterly per `06-Security-Privacy.md`, so 5-min staleness is well within tolerance.

If operators report this is too stale (Month 3+ data), introduce an OIDC-authed `POST /internal/moderation-list-bust` endpoint that deletes the cache keys (open shape: clears all 3 keys, or per-list `?list=profanity`). Implement under a new change `text-moderation-cache-invalidation-endpoint` only if needed.

Trigger to act: Month 3+ operational data shows the 5-min TTL is too stale; otherwise the TTL stands indefinitely. _(Migrated from `FOLLOW_UPS.md` in the 2026-06-09 triage sweep — was `content-moderation-cache-invalidation-endpoint`.)_
