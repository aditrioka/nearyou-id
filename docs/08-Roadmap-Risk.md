...
15. Nearby + Following + Global timeline spatial queries (using `display_location`) — **all shipped, trio complete**: Nearby (`nearby-timeline-with-blocks`, V5); Following (`following-timeline-with-follow-cascade`, V6); Global (`global-timeline-with-region-polygons`, V11 + V12) — detail: [archive](archive/08-Shipped-Detail.md)
16. **Block user feature** (schema + endpoints + cascade follow removal + CI lint) — **all shipped**: `nearby-timeline-with-blocks` (V5) + follow-cascade via `following-timeline-with-follow-cascade` — detail: [archive](archive/08-Shipped-Detail.md)
...
21. Schema + Flyway migrations (deployed via Cloud Run Jobs `nearyou-migrate-staging` then `nearyou-migrate-prod` pre-deploy). **Canonical column-level DDL, indexes, CHECKs, triggers, and per-column rationale for every table below live in [`docs/05-Implementation.md`](05-Implementation.md) (per-table sections)** — this item is the Phase-1 ship manifest. Tables: `users`, `reserved_usernames`, `username_history`, `rejected_identifiers`, `posts` (+ `posts_timeline_cursor_idx`, `posts_nearby_cursor_idx`), `refresh_tokens`, `post_edits`, `follows` (**shipped** V6 in `following-timeline-with-follow-cascade`; column rename canonical `followed_id` → `followee_id`), `post_likes`, `post_replies`, `reports`, `moderation_queue`, `notifications`, `conversations` + `conversation_participants`, `chat_messages`, `subscription_events`, `referral_tickets` + `granted_entitlements`, `admin_regions`, `admin_users` + `admin_webauthn_credentials` + `admin_webauthn_challenges` + `admin_sessions`, `user_blocks` (**shipped** V5 in `nearby-timeline-with-blocks`), `user_fcm_tokens`, `csam_detection_archive`, `admin_actions_log`, `deletion_requests`.
...
24. 4-layer rate limiting + WIB stagger in Redis TTL from Day 1 + `computeTTLToNextReset(user_id)` shared function + CI lint + **hash tag key format standard** + `geocode:{geocell:<lat2dp>_<lng2dp>}` convention — **Layer 2 infrastructure shipped** in change `like-rate-limit` (`rate-limit-infrastructure` capability) — detail: [archive](archive/08-Shipped-Detail.md). Layer 1 (guest pre-issuance) + Layer 4 (per-area) remain deferred to future changes.
...
31. CI lint rules — the full enumerated list (15 rules, with per-rule detail) is canonical under **Development Tools** below; this item records that the whole suite ships in Phase 1.

### Phase 2: Social Features (Weeks 5-6)

1. Follow/unfollow system (using `follows` schema)
2. Following + Global timeline with polygon reverse geocoding (using `actual_location`) — **shipped** (`global-timeline-with-region-polygons`, V11 trigger + V12 polygon seed) — detail: [archive](archive/08-Shipped-Detail.md)
3. Like + reply (20/day Free limit, 280 chars max, flat structure, block-aware) — **Likes shipped V7 (`post-likes-v7`); reply shipped V8 (`post-replies-v8`)**. **Like rate limit shipped** (`like-rate-limit`); **Reply rate limit shipped** (`reply-rate-limit`) — detail: [archive](archive/08-Shipped-Detail.md). Auto-hide trigger shipped via `reports` (V9).
...
9. **Chat via Supabase Realtime Broadcast mode**:
   ...
   - **Chat send rate limit shipped** (`chat-rate-limit`): 50/day Free, unlimited Premium — detail: [archive](archive/08-Shipped-Detail.md)
...
16. Perspective API integration (stopgap text moderation, 500ms timeout fail-open, kill switch flag, writes to `moderation_queue` on high score) — **shipped** (`text-moderation-perspective-api-layer`; vendor pivoted to OpenAI Moderation `omni-moderation-latest`, final timeout 3000ms) — detail: [archive](archive/08-Shipped-Detail.md). Staging-side happy-path observability tracked separately in [#88](https://github.com/aditrioka/nearyou-id/issues/88) (Cloud Run cpu=1 scheduling characteristic; not a Layer 3 code defect; happy path covered by 98 local tests).
...
**Post-scaffold refinements (deferred mobile follow-ups, migrated from `FOLLOW_UPS.md` in the 2026-06-08 triage sweep)**:
- **Nearby radius slider** — replace the Free-tier fixed `NEARBY_RADIUS_M = 20_000` (`NearbyTimelineRepository`, the single generalization site) with the 10/20/50/100 km slider: Free bounce-back-to-20km + upsell, Premium pick. (was `mobile-nearby-radius-slider`)
- **Following timeline live feed** — replace `FollowingPlaceholderScreen` with `FollowingTimeline{ApiClient,Flow,Repository,Screen}` (mirror the shipped `GlobalTimeline*` seam) calling `GET /api/v1/timeline/following`; MODIFIES `mobile-home-tab-host` § "Following tab renders the deferred placeholder and issues no fetch". Likely bundled with a follow-action UI so the feed is non-empty. **On the mobile-first critical path** ([`openspec/project.md`](../openspec/project.md) live menu) — a near-term `/next-change` pick, listed here only as its tracked home, NOT deprioritized to post-MVP. (was `mobile-following-timeline-screen`)
- **Relative timestamps** — add a pure `relativeTime(createdAtIso, now)` formatter + localized Bahasa Indonesia unit strings (`_just_now` / `_minutes_ago` / `_hours_ago` / `_yesterday`) replacing `postDateLabel`'s ISO-date rendering on the timeline + post-detail cards; inject the clock for deterministic tests. (was `mobile-timeline-relative-timestamp`)
- **Nav3 adaptive multi-pane scenes** — when a tablet/foldable target is added, adopt a Nav3 `SceneStrategy` (`material3-adaptive-navigation3`) for list-detail layouts (the adaptive artifact is deliberately off the classpath today). (was `mobile-nav3-adaptive-scenes`)
- **iOS staging test-login (E2E auth bypass)** — the Android staging test-login (deep-link + `StagingTestLoginActivity` in `src/stagingDebug`, see [`mobile/app/maestro/PHASE-3-staging-test-login.md`](../mobile/app/maestro/PHASE-3-staging-test-login.md)) has no iOS equivalent: flavor source sets are Android-only. An iOS path needs a scheme/xcconfig-gated `iosMain` URL-scheme handler that writes the same `TokenPair`, minted via the same `dev/scripts/mint-staging-jwt.sh`. Deferred until Maestro must run on the iOS sim against staging.
...
11. **Premium username customization feature**:
    ...
    - Test coverage: the Premium-username test matrix under Pre-Launch item 5 (cooldown, release hold, reserved list, `username_flagged` queue entry, Free paywall, flag-OFF 503)
...
5. **Security review checklist**:
   ...
   - **Production Cloud Run Layer 3 flags gate**: the production deploy workflow MUST set `--cpu=2 --min-instances=1 --no-cpu-throttling --memory=512Mi` (mirroring staging's [`deploy-staging.yml`](../.github/workflows/deploy-staging.yml)). Without `--no-cpu-throttling`, request-based CPU billing throttles the fire-and-forget Layer 3 moderation dispatch (runs after the 201 response) and it fails-open ~100% — posts with toxicity >0.8 escape `is_auto_hidden` and reach the global timeline. Canonical requirement: [`docs/06-Security-Privacy.md`](06-Security-Privacy.md) § Cloud Run deployment requirement (empirically established in issue [#88](https://github.com/aditrioka/nearyou-id/issues/88))
   ...
   - **Privacy flip 72h test**: the full Phase 4 item 6 lifecycle (`EXPIRATION` webhook → `privacy_flip_scheduled_at` = NOW() + 72h + warning fired; re-subscribe within window clears the column; elapse → hourly worker flips `private_profile_opt_in = FALSE` + busts Redis cache); plus Free user without private profile → no flip scheduled; permanent cancellation mid-window → worker still flips unless re-subscribed (documented behavior).
   - **Apple S2S immediate delete test**: simulated `account-delete` exercises the full Phase 4 item 7 path (immediate `deletion_requests` row, synchronous tombstone+cascade before response, daily backstop pickup if the sync path fails); separately `consent-revoked` → `source = 'apple_s2s_consent_revoked'` + 30-day grace (user-cancellable).
   - **Admin session cookie test**: login sets `__Host-admin_session` with correct attributes (Secure, HttpOnly, SameSite=Strict, Path=/, and NO Domain attribute — the `__Host-` prefix is host-locked per RFC 6265bis §4.1.3.2); state-changing request without `X-CSRF-Token` returns 403; request with mismatched token returns 403 + audit log `admin_csrf_violation`; session cookie rotates on role escalation.
   ...
   - **Grafana OTLP token least-privilege**: before production tag-deploy, mint the `otel-grafana-otlp-token` slot from a custom Grafana Cloud Access Policy (`nearyou-prod-traces-write-only`, realm = stack `nearyouid`, scope = `traces:write` only) rather than the OTLP setup wizard's bundled-scope token (which over-grants `metrics:write` + `logs:write` + `profiles:write` + `stacks:read`). GCP Secret Manager IAM is the primary defense, but the wizard token is unacceptable at production tag-deploy. Optional: rotate `staging-otel-grafana-otlp-token` to a custom-policy token at the next maintenance window. Update [`docs/10-Setup-Checklist.md`](10-Setup-Checklist.md) § 3.7 to make the Access Policy path canonical (the wizard path is a convenient-but-over-privileged shortcut).
6. **Real device-location + permission flow (mobile)** — replace the scaffold `StubLocationProvider` (fixed Jakarta coordinate) with the platform device-location provider (Android fused / iOS `CLLocationManager`, swapped at the existing `LocationProvider` Koin seam), the runtime permission request, the UU-PDP consent modal, and the denial fallback per [`docs/03-UX-Design.md`](03-UX-Design.md) § Location Permission / § Permission Denial Fallback. Deferred from the `mobile-nearby-timeline-screen` change (Mobile #5); a location app cannot launch on a fixed coordinate.
7. **Stronger age assurance (mobile)** — layer the Apple Declared Age Range API (iOS 18+) + Google Play age-signal cross-checks on top of the shipped self-declared DOB age gate, per [`docs/06-Security-Privacy.md`](06-Security-Privacy.md) § Verification. Driven by **PP 17/2025 ("PP TUNAS", in effect ~March 2026)**, which pushes this app category toward age *assurance* rather than mere self-declaration. Deferred from the `mobile-age-gate-screen` change (Mobile #4).
8. **Mobile CI lanes (iOS framework-link + Android instrumented encryption test)** — the two lanes deferred through the scaffold cycle: (a) a `macos-14` job running `./gradlew :mobile:app:linkDebugFrameworkIosSimulatorArm64`, path-filtered to `mobile/**` / `iosApp/**` / `gradle/libs.versions.toml` / `settings.gradle.kts`, wired into the `merge-gate` required checks (otherwise iOS build regressions are caught only by the author's local run per the standing convention); (b) a `SecureTokenStoreEncryptionTest` `androidInstrumentedTest` (Tink/DataStore raw-byte-leak + keyset-regeneration assertions) on a KVM-accelerated Linux Android-emulator lane (`reactivecircus/android-emulator-runner`, free Linux minutes), falling back to Firebase Test Lab if the emulator cannot faithfully exercise the Keystore-backed AEAD. Prior WIP: closed PR [#137](https://github.com/aditrioka/nearyou-id/pull/137). Migrated from `FOLLOW_UPS.md` (`mobile-ios-ci-link-task` + `mobile-auth-signin-android-instrumented-encryption-test`, 2026-06-08 triage).
9. **Per-admin destructive-action rate limit** — ship the ~20/hour-per-admin limiter gating `POST /admin/users/{id}/suspend` + `/unban` (and future admin writes, incl. the deferred `admin-rejected-identifiers-clear-action`), per [`docs/07-Operations.md`](07-Operations.md) § Security ("Rate limit destructive actions"). Substrate (Redis sliding-window vs `admin_actions_log` COUNT) decided at design time — the COUNT cut is authoritative with no fail-soft hole. Motivating threat: a phished admin session (TOTP is phishable) mass-suspending at unbounded rate; interim mitigations (CSRF, idle/absolute session caps, IAP gate, immutable audit trail, reversibility) bound but do not eliminate it. The in-row report-resolution actions (PR [#160](https://github.com/aditrioka/nearyou-id/pull/160)) raise its urgency. Migrated from `FOLLOW_UPS.md` (`admin-destructive-action-rate-limit`, 2026-06-08 triage).
10. **Admin panel manual-verification evidence captures** — two verify-loop bring-ups whose behavior is already test-covered but whose docs/11 §5 screenshot evidence was never captured: (a) the admin user-management profile page (`/admin/users/{id}`, archived change `admin-user-management-profile`; migrated from follow-up [#281](https://github.com/aditrioka/nearyou-id/issues/281)); (b) the `/admin/csam` detection-log viewer + staging smoke (route test-covered by `AdminCsamRouteTest`; migrated from follow-up [#394](https://github.com/aditrioka/nearyou-id/issues/394)). No code delta — fold both into the pre-launch admin-panel verification pass.
...
- **CI lint rules** (canonical enumerated list — Phase 1 §31 ships all of these):
  ...
  - Detect `ST_DWithin` / `ST_Distance` using `actual_location` in non-admin paths (must use `display_location`)
  ...
  - Detect `UPDATE users SET username = ...` outside the signup flow and the single Premium username customization transaction (admin module exempted; signup is the only other allowed writer; lint recognizes annotation `// @allow-username-write: signup` or `// @allow-username-write: customization` on the enclosing function and fails CI on any other write site)
  ...
  - Detect direct `UPDATE users SET private_profile_opt_in = FALSE` outside the privacy flip worker and user-initiated Settings flip (annotation `// @allow-privacy-write: worker` or `// @allow-privacy-write: user_settings`)
...
**Resolved: OpenStreetMap** — shipped in `global-timeline-with-region-polygons` (V12 552-row seed: 38 provinces + 514 kabupaten/kota); ODbL attribution in the V12 migration header + app legal section. Full rationale (BPS rejection, ODbL analysis, import pipeline): [archive](archive/08-Shipped-Detail.md).
...
**Resolved 2026-05-07: Grafana Cloud Tempo via OTLP/HTTP** (PR [#66](https://github.com/aditrioka/nearyou-id/pull/66) `observability-otel-foundation`); `:infra:otel` owns the exporter wiring; staging is live and emitting traces. **Vendor swap remains a within-`:infra:otel` change** per its encapsulation contract. **Trigger to revisit**: free-tier ceiling breach (currently 50 GB traces/month + 14-day retention), regional latency complaint (Grafana stacks are not in Asia), or a compliance regime forcing a single-region data-residency story. Full evaluation rationale + credentials/rotation pointers: [archive](archive/08-Shipped-Detail.md).

### 13. IAP vs Cloud Armor Admin Panel — ✅ Resolved

**Resolved 2026-04-26: Identity-Aware Proxy (IAP)** for staging + production admin access; allowlist scoped to Gmail `nearyouid.founder@gmail.com`, expandable when a secondary admin is hired (Open Decision #9). **Trigger to revisit**: secondary admin hire OR formal compliance audit OR confirmed Google-account compromise vector. Full rationale (Cloud Armor + VPN rejection, defense-in-depth path): [archive](archive/08-Shipped-Detail.md).
...
**Resolved 2026-04-26: custom subdomain `img.nearyou.id` (production) + `img-staging.nearyou.id` (staging)**; `imagedelivery.net` retained as **emergency fallback** if subdomain provisioning breaks at launch (per Risk Register row "CF Images custom subdomain URL structure unverified"). Pre-Phase 1 gate: smoke-test upload + CSAM scan reaches scope via the custom subdomain (Pre-Phase 1 step 2). Full rejection rationale + cost detail: [archive](archive/08-Shipped-Detail.md).

### 33. CSAM Trigger Path — ✅ Resolved

**Resolved 2026-04-26: MVP = admin-triggered manual via Admin Panel; Phase 2+ = Cloudflare Worker auto-forward.** **Triggers to migrate to Phase 2+ Worker**: detection frequency ≥1/week, OR documented founder absence ≥48h regularly, OR compliance audit requires real-time enforcement, OR Phase 4+ scale traffic. Until any trigger fires, MVP path stays canonical and Worker is documented-not-built. Full mechanism + rejection rationale: [archive](archive/08-Shipped-Detail.md).

### 34. Per-Conversation Chat-Notification FCM Batching

**Defer until user growth / feedback signals notification fatigue** (that is also the trigger to act). Today every successful chat send produces a `notifications` row AND fans out one FCM push per active recipient token (`chat-message-notification`, PR #65); per-conversation push batching did NOT ship, so a sender's typing burst into one conversation gives the receiver one FCM push per message rather than a single coalesced push. Acceptable noise at MVP scale; a notification-fatigue + FCM-quota concern at scale.

Batching strategy (decide at change time): (a) debounce-on-send — delay each push ~3s and merge incoming siblings; (b) per-conversation rate-limit — cap pushes per conversation per minute; (c) FCM-side notification grouping — Android `setGroup` + iOS `thread-id` keyed on `conversation_id`. Option (c) is purely client-display batching (no server change, cheapest) and the likely correct first move; most likely shape: grouping shipped in `:infra:fcm`'s payload builders + a minimal `fcm-push-dispatch` spec amendment. If client-display grouping proves insufficient, add server-side debounce/coalesce in a separate change. _(Migrated from `FOLLOW_UPS.md` in the 2026-06-09 triage sweep — was `chat-message-notification-per-conversation-fcm-batching`.)_
