...
- **Report Queue** — **read-only viewer PARTIALLY SHIPPED** (Admin #6 `admin-report-queue`, `GET /admin/reports`): reads from `reports` + joins `moderation_queue` (single representative row via `LEFT JOIN LATERAL`), keyset-paginated newest-first, composable parameterized filters (status / target_type / reason_category / trigger / `from`–`to` date range), HTML-escaped HTMX rendering with a plain-`GET` fallback, and a per-row deep-link to the `/admin/users` suspend/unban surface (resolving the offending user by `target_type`). **Still DESIGN**: in-row resolution actions (Hide, Dismiss, Suspend, Ban, Shadow ban — today reachable only via the deep-link) and the "post has edit history" prioritization filter, both deferred to fast-follow changes (`admin-report-queue-resolution-actions` + `admin-report-queue-has-edit-history-filter`; see `FOLLOW_UPS.md`).
