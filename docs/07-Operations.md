...
> **Status (2026-05-28).** The Admin Panel is **PARTIALLY SCAFFOLDED**. As of `admin-panel-ktor-htmx-bootstrap` (Admin #2, PR [#115](https://github.com/aditrioka/nearyou-id/pull/115)), the `:backend:ktor` `admin` package now ships an `Application.admin()` extension function that mounts the `/admin/*` route subtree (Pebble templating + HTMX client + vendored static assets + shared base layout + index page). The mount is gated by a `KTOR_ENV != "production"` check — production exposure is structurally impossible until Admin #3 lifts the guard. Admin schema (V15 `admin-schema-bootstrap`, PR #107) is shipped and the lint invariants (`csrf_token_hash` required, admin-FK `ON DELETE SET NULL`) ARE Detekt-enforced. **What remains DESIGN**: authentication / session / CSRF (Admin #3 `admin-login-argon2-totp`), DB-touching admin features (Audit Log Viewer in Admin #4, Suspend/Unban Action in Admin #5), `admin.nearyou.id` separate-Cloud-Run-service + IAP/Cloud Armor deployment (Phase 3.5 deployment task #2), all admin business features in § Core Features below. Treat the per-feature blocks below as forward-looking design until each lands in its own change.
>
