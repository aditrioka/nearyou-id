...
- **Data Export Queue** — **SHIPPED** (`admin-data-export-queue`, frame 16): the user-facing **producer** is `account-data-export` (`POST`/`GET /api/v1/account/export` + the OIDC `/internal/data-export-worker` gathers the § Data Export Scope Matrix (`06-Security-Privacy.md`), packs a JSON+CSV ZIP, uploads to R2 (24h signed URL), and delivers via the `data_export_ready` notification + Resend email; `data_export_requests` table, V30). The **admin surface** is `GET /admin/data-exports` — a keyset-paginated, **newest-`requested_at`-first**, `status` + `q` (username/UUID)-filterable list joined to `users` for the requester username, with the five DB statuses surfaced as **QUEUED / RUNNING / DELIVERED / EXPIRED / FAILED** and a delivered-via cell (notification + email) on `ready`; served by the existing `data_export_requests` indexes (no migration); HTML-escaped HTMX render + plain-`GET` fallback; **identity-only PII** — status only, never the archive contents / `r2_object_key` / signed URL (username deep-links to `/admin/users?q=`). Plus a manual **trigger** write (`POST /admin/data-exports/{id}/trigger`; `owner`/`admin` + CSRF-gated, **required reason**, `hx-confirm` guard, rate-limited **10/admin/hour** on a distinct counter independent of the 20/hour destructive budget): for a `failed`/`pending` row the scheduler missed, it re-enqueues where needed and drives the **producer's single-request pipeline** (one export path, two callers — not a second export path) so a stalled job delivers immediately, and writes one immutable `data_export_triggered` audit row in one transaction; a `processing`/`ready`/`expired`/unknown target (or a one-active-index collision on re-enqueue) is a benign idempotent no-op (no mutation, no audit row). For UU PDP 7-day-SLA accountability + support "where's my export?" triage. [#361](https://github.com/aditrioka/nearyou-id/issues/361) closed.
...
- **Referral Manual Grant Path** — **SHIPPED** (`admin-referral-manual-grant`, frame 19): `GET/POST /admin/referral-grants` — for support tickets where the automated gate false-negatives a legitimate referral. Lookup (username/UUID) shows premium/subscription + referral context; the grant dispatches a 1-week promotional Premium entitlement via the existing `ReferralEntitlementGranter` RC port (stacking `GREATEST(current_entitlement_end, NOW()) + 7d`; fail-soft when RC is unconfigured; soft-deleted targets rejected) and writes **one immutable `admin_actions_log` row (`action_type = 'referral_manual_grant'`)** as the authoritative record. Premium activation + the `subscription_events` row stay owned by the existing GRANT **webhook echo** (`source='referral'`) — the admin path writes **neither** directly, and deliberately does **NOT** write `granted_entitlements` (counts against neither the inviter's 5-referral lifetime track nor the single lifetime inviter reward — support-desk remedy, not a referral-system action). `owner`/`admin` + CSRF-gated, required ticket reason, `hx-confirm`, rate-limited **10/admin/hour** on a distinct restorative counter; past grants render in a keyset-paginated viewer over the audit log.
...
- **CSAM Detection Log Viewer**:
  ...
  - **Admin-triggered handler invocation** — primary MVP path for the downstream auto-action: admin pastes the matched URL / image_id from the CF email notification into `POST /admin/csam/takedown` (`owner`/`admin` + CSRF-gated), which triggers the fixed-policy handler **in-process via the shared `CsamDetectionService`** — the same sink as `/internal/csam-webhook`'s admin-session path, which remains the *external* admin-authenticated contract (no same-process HTTP self-call). Handler: hard-delete post, permanent ban + token_version bump, cascade-delete of the user's other posts, encrypted metadata archival (AES-256-GCM), Kominfo report queue. Audit-logged with full `before_state`/`after_state`.
...
- **Subscription Grace Monitor** — **SHIPPED** (`admin-subscription-grace-monitor`, frame 18): `GET /admin/subscriptions/grace` — read list of users in `subscription_status = 'premium_billing_retry'` (the RevenueCat 7-day billing-retry grace), each row surfacing store/platform, the current-streak retry-since, the latest webhook event, and an already-expedited indicator; keyset-paginated newest-retry-first, composable `q` (username/UUID) + `store` filters, in-window count summary; index-served by the existing V2 partial index `users_subscription_idx` (no migration); any authenticated admin role reads; HTML-escaped HTMX render + plain-`GET` fallback; identity-only PII (username deep-links to `/admin/users?q=`). Plus a manual **expedite** bookkeeping write (`POST /admin/subscriptions/grace/{user_id}/expedite`; `owner`/`admin` + CSRF-gated; 20/hour-per-admin via a distinct `subscription_grace_expedite` counter on the audit-trail-as-ledger limiter, independent of the destructive budget; required support-ticket ref): it writes exactly one immutable `admin_actions_log` row and **does NOT mutate entitlement** — RevenueCat remains the sole source of truth (only an `EXPIRATION` event downgrades), so the before/after snapshots carry an identical `subscription_status`. For support tickets ("I was charged but premium stopped / the webhook is late"). The user-facing `subscription_billing_issue` notification already ships in the V10 catalog (webhook-emitted); this surface is admin-only.
...
- **Feature Flag Admin** — **PARTIALLY SHIPPED** (`admin-feature-flag-editor`, frame 20): `GET /admin/feature-flags` publishes single-flag writes to the Remote Config **Server template** (REST API against the `firebase-server` namespace + `If-Match` ETag concurrency — the Admin SDK can't publish the server template) for the flag/enum/integer surface (`image_upload_enabled`, `attestation_mode` enum, `search_enabled`, `perspective_api_enabled`, `premium_username_customization_enabled`, `premium_like_cap_override`, `moderation_match_threshold`). Each write is owner/admin-role- + CSRF-gated, rate-limited (5/hour per admin, distinct from the 20/hour destructive cap), reason-required, value-validated, and audited as one immutable `feature_flag_toggled` row. The two content-moderation **wordlist** parameters (`moderation_profanity_list`, `moderation_uu_ite_list`) render **read-only** (count + version) on this page, each with an **"edit" link** to the dedicated wordlist editor. **Array-content editing SHIPPED** (`admin-moderation-wordlist-editor`, resolves [#305](https://github.com/aditrioka/nearyou-id/issues/305)): a per-list sub-surface (`GET /admin/feature-flags/wordlists/{list}` + `POST …` publish + `POST …/preview`) — searchable list, add-single (literal), bulk-CSV import (skips `#` comment + blank lines + duplicates with a report), and per-entry remove, all **staged into a previewed diff** (added/removed/resulting) before a single publish to the Server template via `RemoteConfigPublisher.publishServerStringList`. Owner/admin-role- + CSRF-gated (CSRF-before-role), reason-required, entry-normalized (trim, `Locale("id")` lowercase diacritics-preserved, dedup, per-entry ≤100 chars / list ≤10000), **empty-list-guarded** (an empty list would disable the layer + trip fallback), `If-Match`-concurrency-protected, rate-limited on a **distinct 10/hour-per-admin** bucket (separate from the 5/hour flag-toggle + 20/hour destructive caps), and audited as one immutable `moderation_wordlist_edited` row carrying a diff summary. Edits the Server template only (the Tier-3 repo `*.default.txt` + Tier-4 Secret Manager fallbacks stay on the quarterly legal-review path) and take effect after the ≤5-minute moderation cache TTL. The write seam lives in `:infra:remote-config` (`RemoteConfigPublisher`); no migration. **Operator-setup**: the existing `firebase-admin-sa` service account needs the Firebase Remote Config **write** IAM role on the bound project — until granted, the panel degrades to read-only (graceful, never crashes).
- **Rejected Identifiers Viewer** — read-only anti-abuse audit/triage half **SHIPPED** (`admin-rejected-identifiers-viewer`, PR #156): `GET /admin/rejected-identifiers` — keyset-paginated newest-first over `(rejected_at, id)`; composable `reason` / `identifier_type` / UTC-date filters; per-reason/per-type count summary (rejection-spike detection); hash-only PII discipline (surfaces only the one-way `identifier_hash`); any authenticated admin role. The **manual clear path** (remove a row for legitimate adult re-verification on support request) is **SHIPPED** (`admin-rejected-identifiers-clear-action`, resolved [#190](https://github.com/aditrioka/nearyou-id/issues/190)): `POST /admin/rejected-identifiers/{id}/clear` hard-deletes one row + writes one immutable `admin_actions_log` row (`rejected_identifier_cleared`, cleared row preserved in `before_state`), in one transaction; **owner/admin-only** + CSRF-gated + required reason + idempotent on not-found, rate-limited at a **dedicated 10 clears/admin/hour** cap (sourced from the audit trail — NOT the shared 20/hr destructive limiter, whose set is user-punitive; clearing is restorative). The per-row clear control renders only for owner/admin sessions. No migration. Replaces the prior out-of-band raw-SQL path.
...
**Preferred path — in-panel editor** (`admin-moderation-wordlist-editor`): edit `moderation_profanity_list` / `moderation_uu_ite_list` from the admin panel at `/admin/feature-flags` → the per-list **"edit"** link → stage adds/removes (or bulk-CSV import) → **Preview diff** → supply a reason → **Publish**. This is owner/admin- + CSRF-gated, audit-logged (`moderation_wordlist_edited`), rate-limited (10/hour/admin), and empty-list-guarded — preferred over the raw Console because the change is governed + audited. The edit takes effect after the ≤5-min Redis cache TTL (same as below). The editor touches **only** the Remote Config Server template; the repo `*.default.txt` Tier-3 fallback + the Tier-4 Secret Manager slot still follow the Console / Secret-Manager procedure below (updated at the quarterly legal review).

**Fallback path — Firebase Console** (still required for the Tier-3/Tier-4 fallback lists, and usable for the Server list if the panel is unavailable):
