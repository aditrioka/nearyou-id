...
> **Status (2026-06-06).** The Admin Panel is **PARTIALLY BUILT**. The `:backend:ktor` `admin` package mounts the `/admin/*` route subtree (Pebble templating + HTMX client + vendored static assets + shared base layout) and now ships a working auth gate plus its first moderation action: **Admin #3 `admin-login-argon2-totp`** (Argon2id + TOTP login → `__Host-admin_session` cookie + per-session CSRF gate; this lifted the Admin #2 `KTOR_ENV != "production"` mount guard, so `/admin/*` is now gated by the session middleware in all environments), **Admin #4 `admin-actions-log-viewer`** (the read-only `admin_actions_log` audit-trail viewer), **Admin #5 `admin-suspend-unban-user-action`** (the panel's FIRST state-changing action — server-fixed 7-day suspend + manual unban under `/admin/users`, role-gated [`owner`/`admin`/`moderator`, with permanent-ban unban restricted to `owner`/`admin`] + CSRF-gated, each applied action writing one immutable `admin_actions_log` row atomically; suspend also inserts a sanitized `account_action_applied` notification), and **Admin #6 `admin-report-queue`** (the read-only Report Queue triage viewer — `GET /admin/reports`: keyset-paginated `reports` ⟕ `moderation_queue` with composable parameterized filters + deep-links to the `/admin/users` suspend/unban surface; the in-row resolution write-back now ships as `admin-report-queue-resolution-actions` — `POST /admin/reports/{id}/resolve` + `POST /admin/moderation-queue/{id}/resolve` performing Hide/Dismiss/Suspend/Ban/Shadow-ban enforcement atomically, CSRF + role-gated, one immutable `admin_actions_log` row per action), plus the read-only **`admin-rejected-identifiers-viewer`** (the `rejected_identifiers` anti-abuse audit/triage table at `GET /admin/rejected-identifiers` — keyset pagination + `reason`/`identifier_type`/UTC-date filters + a per-reason/per-type count summary for spike detection, hash-only, any-admin-role; its manual-clear write half is deferred to `admin-rejected-identifiers-clear-action`). Admin schema (V16 `admin-schema-bootstrap`, PR #107) is shipped and the lint invariants (`csrf_token_hash` required, admin-FK `ON DELETE SET NULL`) ARE Detekt-enforced. **What remains DESIGN**: `admin.nearyou.id` separate-Cloud-Run-service + IAP/Cloud Armor deployment (Phase 3.5 deployment task #2), a per-admin destructive-action rate limiter (deferred per `docs/08-Roadmap-Risk.md` § Pre-Launch and the Admin #5 design D11), and the remaining admin business features in § Core Features below (the full User Management search / profile / history page, Operational Dashboard, etc.). Treat those still-DESIGN per-feature blocks below as forward-looking until each lands in its own change.
>
...
- **Report Queue** — **PARTIALLY SHIPPED** (Admin #6 `admin-report-queue`, `GET /admin/reports` + the resolution write-back): reads from `reports` + joins `moderation_queue` (single representative row via `LEFT JOIN LATERAL`), keyset-paginated newest-first, composable parameterized filters (status / target_type / reason_category / trigger / `from`–`to` date range), HTML-escaped HTMX rendering with a plain-`GET` fallback, and a per-row deep-link to the `/admin/users` suspend/unban surface (resolving the offending user by `target_type`). **In-row resolution actions SHIPPED** (`admin-report-queue-resolution-actions`): `POST /admin/reports/{id}/resolve` (decision `actioned`/`dismissed` — bookkeeping) + `POST /admin/moderation-queue/{id}/resolve` performing the named enforcement atomically — **Hide/Dismiss** (content `is_auto_hidden` toggle on post/reply), **Suspend** (reuses the shipped 7-day suspend), **Ban** (permanent, owner/admin tier only), **Shadow ban** (`is_shadow_banned`, no user notification — stealth invariant) — CSRF + role-gated, one immutable `admin_actions_log` row per action, idempotent re-resolution. **Still DESIGN**: the "post has edit history" prioritization filter (GitHub issue [#191](https://github.com/aditrioka/nearyou-id/issues/191) `admin-report-queue-has-edit-history-filter`, label `follow-up`).
...
- **Rejected Identifiers Viewer** — read-only anti-abuse audit/triage half **SHIPPED** (`admin-rejected-identifiers-viewer`, PR #156): `GET /admin/rejected-identifiers` — keyset-paginated newest-first over `(rejected_at, id)`, composable `reason` / `identifier_type` / UTC-date filters, a per-reason/per-type count summary for rejection-spike detection, hash-only PII discipline (surfaces only the one-way `identifier_hash`), accessible to any authenticated admin role. The **manual clear path** (admin removal of a `rejected_identifiers` row for legitimate adult re-verification on support request) remains DESIGN — deferred to the fast-follow `admin-rejected-identifiers-clear-action` (role-gated + CSRF-gated + audit-logged + rate-limited; tracked in GitHub issue [#190](https://github.com/aditrioka/nearyou-id/issues/190), label `follow-up`). Until it ships, clearing a rejected identifier is the existing out-of-band raw-SQL path.
