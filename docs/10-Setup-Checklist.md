...
- [ ] `prod-perspective-api-key` dan `staging-perspective-api-key` — Google Cloud Console API key for the Perspective API toxicity classifier (Layer 3 of the multi-layer text moderation pipeline; consumed by `:infra:perspective` `KtorPerspectiveClient`). **Required BEFORE staging smoke** for the `text-moderation-perspective-api-layer` change. Wire as `PERSPECTIVE_API_KEY=staging-perspective-api-key:latest` in `deploy-staging.yml --set-secrets`; resolved in `Application.module()` via `secrets.resolve("perspective-api-key")` mirroring `firebase-admin-sa`. Free-tier quota is 1 QPS — file a quota-increase request before production tag-deploy if MVP traffic patterns warrant.
