...
- **Realtime-consumer seam (registered by `mobile-chat-screen`).** The canonical pattern for a mobile realtime/streaming consumer: a **vendor-SDK-free interface** (`ChatRealtimeSubscriber` returning a cold `Flow<…Inbound>`) + inbound model + a token-fetch fun-interface, living in a **KMP `:infra:*` module** (`:infra:supabase-realtime`) alongside the single vendor-SDK implementation (invariant #16 — the supabase-kt import is fenced there; `:mobile:app` consumes only the interface, with the vendor dep `implementation`-scoped so it never reaches the app's compile classpath). The consuming **route-scoped `ViewModel`** merges the cold inbound `Flow` into its REST-backed list with **id-based dedupe** (update-in-place; redaction terminal), reconciles optimistic-send temp ids to server ids, triggers a **REST resync on every (re)subscribe** (no-outbox recovery), and tears the subscription down on `onCleared`. A realtime/token failure degrades to REST-only, never error chrome. **Future realtime features (presence, typing, read receipts) build on THIS seam — do not derive a parallel one.** The publish side mirrors it (`ChatRealtimeClient` in JVM `:core:domain`); a post-swap Ktor-WebSocket transport replaces both impls behind the same two interfaces.
...
3. **UI-affecting changes: manual bring-up via `verify-loop` BEFORE archive** — screenshot evidence in the PR body, checklist pass per `mobile-ui-foundation`. "Tests green" alone is NOT done for UI; this gate exists because build-green-but-broken-on-device was this project's recurring failure mode. **The evidence source is context-routed (verify-loop handles it automatically):** a **local** session brings the change up on the Android emulator AND iOS simulator and screenshots them; a **cloud** session (no emulator possible) runs it on a real device via Firebase Test Lab — `scripts/run_on_device.sh` (Robo + screenshots/video) and/or the `device-run.yml` PR comment — and links that as the evidence. Either way the gate is the same: the change is observed running on a device surface, not just compiled. Routing is automatic via `scripts/_testing_context.sh`; never hand-pick emulator-vs-farm.
