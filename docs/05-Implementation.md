...
**Daily-cap windowing**: the per-day caps (Post / Reply / Chat / Like) are enforced as a **rolling ~24h sliding window per user** via the single `:infra:redis` Lua sliding-window script — entries older than the window are pruned on each admit. `computeTTLToNextReset(user_id)` sets the per-user key TTL (offset = `hash(user_id) % 3600` seconds, effective 00:00–01:00 WIB) so an **idle bucket's GC is staggered to avoid a thundering herd** near 00:00 WIB — it is NOT a hard synchronized midnight bulk-reset. Centralized in `computeTTLToNextReset(user_id)`; `RateLimitTtlRule` Detekt rule enforces usage at every daily-rate-limit endpoint (hourly limits skip the stagger). For the canonical low-cap, normal-cadence case (e.g. 10 likes/day) the user-visible behavior is "N successful actions within any rolling ~24h window."

**Windowing decision (revised 2026-06, supersedes the earlier fixed-window framing):** the original design framed Layer 2 as a fixed-window "resets at WIB midnight" cap and rejected a rolling-24h window as *"UX confusing, Redis sliding window complex."* That rejection is reversed: (a) the shipped implementation is already the sliding window above — the rolling complexity is built and paid for; (b) the direct comparables for a free per-day "likes" cap — **Tinder and Bumble both use rolling-24h cycles, not midnight resets** — so rolling-24h is the consumer-app norm for this exact UX, not confusing; (c) fixed-window boundary-burst (≈2× the cap at the reset edge) is an anti-abuse weakness for a public like/reply endpoint. The per-user `Retry-After` header communicates the next-available moment, so no fixed "midnight" promise is needed; anti-bot burst control remains the separate per-hour limiter (e.g. 500/h on likes).
