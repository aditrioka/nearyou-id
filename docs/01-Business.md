...
2. **Invitee activity gate (14-day window)**: logged in on ≥3 different days; ≥2 posts (Free tier limit forces real engagement); ≥5 app sessions (tracked via the `session_start` event)
3. **Anti-fingerprint-collision**: invitee `device_fingerprint` does NOT match the inviter's in the last 90 days; invitee IP subnet (/24) NOT among the inviter's last 10 login subnets; invitee Google/Apple ID hash NOT in the inviter's "recently seen" list
