...
- `interface AdProvider` in the vendor-fenced **`:infra:admob`** module (commonMain) — NOT `:core:data`. Reconciled by `mobile-admob-ads-foundation` (design D1): the Google Mobile Ads / UMP SDKs are fenced behind `:infra:admob` per the docs/11 §2.6 vendor-SDK `:infra` seam (the `:infra:revenuecat` / `:infra:supabase-realtime` precedent), which isolates the SDK far more cleanly than `:core:data` would and satisfies invariant #16. `:mobile:app` consumes only the interface + `NativeAdContent`.
- Android: Google Mobile Ads SDK + UMP SDK native (`AndroidAdProvider`, Gradle deps in `:infra:admob`)
- iOS: cinterop to Google-Mobile-Ads + Google-UserMessagingPlatform Pods (`IosAdProvider`, generated by the `native.cocoapods` plugin on `:infra:admob` — the GoogleSignIn-pod precedent)
...
2. **Invitee activity gate (14-day window)**: logged in on ≥3 different days; ≥2 posts (Free tier limit forces real engagement); ≥5 app sessions — derived server-side by sessionizing the durable `login_events` store (a 30-min idle-gap boundary; `login-history-tracking` / V35), replacing the consent-gated client `session_start` event. Login-days + app-sessions are security-purpose signals, always collected (not analytics-consent-gated).
