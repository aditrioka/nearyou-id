...
/**
 * JDBC-backed [Layer3ModerationWriter]. Each helper opens its own connection
 ...
 *
 * **Vendor history**: the SQL trigger string `'perspective_api_high_score'` is fixed
 * in the V9 schema `moderation_queue.trigger` CHECK constraint enum and persists as
 * a historical artifact (vendor pivoted from Perspective → OpenAI Moderation
 * mid-implementation; renaming the trigger would require a Flyway migration,
 ...
 */
class JdbcLayer3ModerationWriter(
    ...
    override fun applyAutoHideAndQueue(
        ...
        require(targetType == ReportTargetType.POST || targetType == ReportTargetType.REPLY) {
            "Layer 3 only applies to POST / REPLY targets; got $targetType"
        }
        val updateSql =
            when (targetType) {
        ...
        dataSource.connection.use { conn ->
            ...
            try {
                ...
                if (updated == 0) {
                    ...
                    conn.rollback()
                    return Layer3WriteResult.SoftDeletedTarget
                }
                val inserted =
                ...
                conn.commit()
                return if (inserted == 1) Layer3WriteResult.Applied else Layer3WriteResult.QueueConflictNoOp
            } catch (t: Throwable) {
                try {
                    conn.rollback()
