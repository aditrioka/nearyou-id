...
- **Override consume vs. concurrent change race** → the consume happens inside the existing `SELECT … FOR UPDATE` change transaction, so two concurrent changes for the same user serialize; a one-shot override is consumed at most once.

## Migration Plan

- **`V23__username_flag_overrides.sql`** — create the table (FKs to `users` ON DELETE CASCADE + `admin_users` ON DELETE SET NULL; `UNIQUE(user_id, candidate)`; `candidate VARCHAR(60)` matching the username ceiling). No partial-index `NOW()` predicates (critical invariant). Migration applies cleanly on the integration-test Postgres (no `admin_app` dependency — grants are operational).
