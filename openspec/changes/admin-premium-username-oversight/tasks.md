...
- [ ] 3.2 On a successful change that used an override, set `consumed_at = NOW()` on the matched override row INSIDE the existing `SELECT … FOR UPDATE` change transaction (so consume + rename + history + notification commit/rollback together).
- [ ] 3.3 Change the `username_flagged` insert from `ON CONFLICT … DO NOTHING` to `ON CONFLICT (target_type, target_id, trigger) DO UPDATE SET status='pending', resolution=NULL, resolved_by=NULL, resolved_at=NULL, notes=EXCLUDED.notes, created_at=NOW()`, populating `notes` with the flagged candidate (re-open + persist latest candidate).
