...
- [ ] 1.5 Add `CREATE TABLE chat_messages (...)` per canonical: columns + the empty-message CHECK + the 3-column redaction atomicity CHECK. Annotate `redacted_by UUID NULL` with `-- @allow-admin-fk-deferred: phase-3.5 -- FK to admin_users(id) ON DELETE SET NULL added when admin schema lands` matching the V9 pattern at [V9__reports_moderation.sql:20](../../../backend/ktor/src/main/resources/db/migration/V9__reports_moderation.sql). Annotate `embedded_post_edit_id UUID NULL` with `-- @allow-post-edits-fk-deferred: post-edit-history-change -- FK to post_edits(id) ON DELETE SET NULL added when post_edits ships` (the `post_edits` table does NOT exist at V14 — verified via `grep -rn "CREATE TABLE post_edits"` returning no matches; canonical Phase 4 schema per `docs/08-Roadmap-Risk.md:102`). The `embedded_post_id` and `embedded_post_snapshot` columns ship with their constraints normally.
...
- [ ] 2.3 Implement `listMyConversations` with **LEFT JOIN visible_users + COALESCE-to-placeholder** so shadow-banned partners surface masked, not filtered (per `design.md` § D9 invisible-actor model). Canonical SQL shape: `SELECT c.*, COALESCE(u.username, 'akun_dihapus') AS partner_username, COALESCE(u.display_name, 'Akun Dihapus') AS partner_display_name, COALESCE(u.is_premium, FALSE) AS partner_is_premium FROM conversations c JOIN conversation_participants me ON me.conversation_id = c.id AND me.user_id = :viewer AND me.left_at IS NULL JOIN conversation_participants other ON other.conversation_id = c.id AND other.user_id != :viewer LEFT JOIN visible_users u ON other.user_id = u.id LEFT JOIN user_blocks b1 ON b1.blocker_id = :viewer AND b1.blocked_id = other.user_id LEFT JOIN user_blocks b2 ON b2.blocker_id = other.user_id AND b2.blocked_id = :viewer WHERE b1.blocker_id IS NULL AND b2.blocker_id IS NULL ORDER BY c.last_message_at DESC NULLS LAST, c.created_at DESC LIMIT :limit`. Cursor adds `WHERE (c.last_message_at IS NOT NULL AND (c.last_message_at, c.id) < (:cursor_lmat, :cursor_id)) OR (c.last_message_at IS NULL AND :cursor_lmat IS NULL AND c.id < :cursor_id)` (NULL-safe — NULLS LAST ordering pages reach the NULL block last).
- [ ] 2.4 Implement `listMessages` reading raw `chat_messages` (Repository own-content path; suppress `BlockExclusionJoinRule` via the source annotation `// @allow-no-block-exclusion: chat-history-readable-after-block` per `design.md` § D10). Active-participant pre-check via `conversation_participants WHERE conversation_id = :conv AND user_id = :viewer AND left_at IS NULL`; throw `NotParticipantException` if no row. **Shadow-ban filter inline**: append `AND (cm.sender_id = :viewer OR NOT EXISTS (SELECT 1 FROM users u WHERE u.id = cm.sender_id AND u.is_shadow_banned = TRUE))` to the message query (per spec Requirement: List-messages endpoint, shadow-ban filter section). Apply `(created_at, id) < (:cursor_ts, :cursor_id)` for pagination. At serialization, mask `content = NULL` and never serialize `redaction_reason` when `redacted_at IS NOT NULL`.
...
- [ ] 2.6 Add a self-DM guard: `findOrCreate1to1` rejects if `callerId == recipientId` with `SelfDmException`. The check SHALL run BEFORE the user-pair `pg_advisory_xact_lock` is acquired (per spec scenario "Self-DM check happens before lock acquisition").
- [ ] 2.7 Add a recipient-existence pre-check: `findOrCreate1to1` looks up the recipient via RAW `users` (NOT `visible_users`) — only return 404 if the recipient row is genuinely absent. A shadow-banned recipient is a valid creation target (per `design.md` § D9 — non-banned third parties cannot detect shadow-ban state via 201/404 differential).
...
- [ ] 7.9.c `GET /api/v1/conversations`: bidirectional block-exclusion — A blocks B → conversation between A and B is NOT in A's list (and not in B's list either, by symmetry).
...
- [ ] 11.7 Open follow-up entry in `FOLLOW_UPS.md` titled `v2-rls-shadow-ban-clause-amendment` (HIGH PRIORITY — hard prerequisite for `chat-realtime-broadcast`). The V2 RLS policy at [V2__auth_foundation.sql:81-84](../../../backend/ktor/src/main/resources/db/migration/V2__auth_foundation.sql) currently has `AND NOT EXISTS (SELECT 1 FROM public.users WHERE id = cp.user_id AND is_shadow_banned = TRUE)` which DENIES the shadow-banned user from subscribing to their own conversation realtime channel — contradicting the invisible-actor model committed in this change's `design.md` § D9. The V2 policy must be amended (drop the `is_shadow_banned` clause from the subscriber check; the publish-side filter on broadcasts handles the OTHER-reader hiding) BEFORE chat-realtime-broadcast ships, or the REST and WSS paths will disagree.
- [ ] 11.8 Open follow-up entry in `FOLLOW_UPS.md` titled `chat-message-notification-emit-sites` for the `chat_message` and `chat_message_redacted` notification emit-sites, deferred until chat-realtime-broadcast lands the publish surface. The fcm-push-dispatch composite (PR [#60](https://github.com/aditrioka/nearyou-id/pull/60)) will pick them up automatically once emitted.
