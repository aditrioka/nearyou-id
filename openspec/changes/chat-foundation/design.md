...
| User-pair advisory lock collision under high load — `hashtext(...)` may collide for distinct pairs | `hashtext` is a bigint hash; collision rate at Indonesia user-pair scale (millions of pairs) is statistically negligible (<10^-9). On collision, the consequence is two conversations for ostensibly-distinct pairs are serialized — wasted CPU but correct behavior. Documented as accepted in design.md D1. |
