...
### D4 — Notification: both active participants, system row, content-free, in-app only
Recipients = **all active participants** of the conversation (`conversation_participants WHERE conversation_id = … AND left_at IS NULL`), per `docs/07` "affected conversation participants receive". `docs/05` line ~1034 phrases it "recipient receives" (singular) — the spec follows `docs/07`'s plural; the `docs/05` wording is flagged as a minor reconciliation (a `follow-up` doc-fix, not a behavior change). **Write mechanism = the shipped admin raw in-transaction `INSERT INTO notifications (…)` pattern** (mirroring `ReportResolutionRepository:407` / `UserModerationRepository`, which write `account_action_applied` the same way), **NOT the `NotificationEmitter` service path** — the admin module writes the `notifications` table directly (the established admin notification pattern; verified against shipped admin code). Each row is system-originated (`actor_user_id = NULL`); because it is a direct INSERT (not a `NotificationEmitter` call site), no block-suppression or shadow-ban masking applies, so a block between participants does NOT suppress the notice, and the emitter's cross-capability shadow-ban rule does not bind (redaction is a non-stealth moderation action — participants are openly informed). Per the canonical V10 catalog (`docs/05` § Notifications Schema): `target_type = 'message'` (the chat notification convention — NOT `'chat_message'`, which is the `reports` table's enum), `target_id = <redacted message id>`, and `body_data = {"conversation_id": <uuid>}` — **exactly one key**; it carries no content, no reason, and **no `message_id`** (the catalog's "do not duplicate `target_id` in body_data" rule — the message is the row's `target_id`). Delivery is **in-app feed only — no FCM push**, matching the shipped admin `account_action_applied` (admin-originated notifications are not currently FCM-dispatched; FCM-push-on-redaction is an explicit out-of-scope follow-up). Alternative (route through `NotificationEmitter` + FCM): rejected — it would introduce a second notification-write pattern into the admin module (a Pattern-Registry deviation needing a docs/11 amendment) plus net-new admin→FCM plumbing, for no functional gain over the shipped admin pattern.
...
- **Backend layering (§3.1):** `AdminChatRedactionRoute` (thin: parse/validate/authenticate/respond) → `ChatRedactionRepository` (transaction boundary + SQL). DTOs/view-maps live with the route; SQL rows do not leak into response shapes. The notification row is written via a raw in-transaction `INSERT INTO notifications` in the repository — the **established admin notification pattern** (the admin module writes operational tables directly per `docs/04` § Admin Panel Data Access; this is the admin exception to the cross-feature-via-interface rule, the same path `ReportResolutionRepository` / `UserModerationRepository` already use for `account_action_applied`). It deliberately does NOT introduce the `NotificationEmitter` service into the admin module.
...
- **Conversation hard-delete cascades the message** (`chat_messages.conversation_id ON DELETE CASCADE`, V15) → a redaction of a since-deleted message resolves to NotFound (404 on GET, not-found in-band on POST). This is intentional — there is nothing left to redact; no special handling.
- **Admin-originated redaction notification is in-app only (no FCM push)** → matches the shipped admin `account_action_applied`; a participant who is offline sees it on next app open via the in-app feed. If push-on-redaction is later wanted, it is a follow-up (a `PushCopy` case + dispatcher wired into the admin path) — deliberately out of scope to avoid net-new admin→FCM plumbing.
