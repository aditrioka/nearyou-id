...
- **Activate the `chat_message_redacted` notification emit site** — the type is already in the V10 catalog (currently one of the "reserved for future emit sites"); this change defines its `body_data` shape `{conversation_id, message_id}` (carries **no content and no reason** — data-plane PII discipline) and writes it.
...
- `in-app-notifications`: activate the `chat_message_redacted` emit site — move it from "reserved for future emit sites" to written, and define its `body_data` shape `{conversation_id, message_id}` (no content/reason) under § "body_data shape per emitted type".
