...
- [x] 1.1 Add `AdminRoleGate.requireOwnerOrAdmin(call)` — a sibling to `requireWriteRole` that returns 403 for a `moderator` / `read_only` session (configuration of the existing role-gate concern; no new pattern). Unit-test the allow/deny matrix.
- [x] 1.2 Create `admin/routes/AdminChatRedactionRoute.kt` exposing `GET /admin/chat-messages/{id}` + `POST /admin/chat-messages/{id}/redact`, wired INSIDE `authenticate(ADMIN_AUTH_NAME)` in `AdminModule.kt` alongside the report-queue/user-moderation routes; the bare `/admin/chat-messages` collection exposes no write route.
- [x] 1.3 GET handler: `requireOwnerOrAdmin` (403) → parse `{id}` UUID (malformed → 400) → load target + ±2 context window + active participants → 404 when no message → render `chat-redaction.peb` (HTML-escaped; an already-redacted target renders the redacted placeholder + an "already redacted" indicator).
- [x] 1.4 POST handler mirroring `AdminReportResolutionRoute` gate order: CSRF first (403 + `admin_csrf_violation` on miss) → `requireOwnerOrAdmin` (403) → parse `{id}` UUID (400, no write) → required `redaction_reason` (blank/missing → 400, no write) → `DestructiveActionRateLimiter` → repository; map outcomes to the dual-mode response (HTMX fragment / no-JS 303 / in-band message).

## 2. Redaction repository (one transaction)

- [x] 2.1 Create `admin/chatredaction/ChatRedactionRepository.kt`: read the target `chat_messages` row, the ±2 surrounding messages (by `created_at` within the conversation), and the active `conversation_participants` (raw reads — admin module allowlist; bounded dispatcher).
- [x] 2.2 Idempotent atomic `UPDATE chat_messages SET redacted_at = NOW(), redacted_by = :adminId, redaction_reason = :reason WHERE id = :id AND redacted_at IS NULL`; return an outcome enum (`Applied` / `AlreadyRedacted` / `NotFound`) from rows-affected + existence.
- [x] 2.3 In the SAME transaction (only on `Applied`): write a `chat_message_redacted` notification row per active participant via a **raw `INSERT INTO notifications (…)`** (the shipped admin pattern, mirroring `ReportResolutionRepository:407` — NOT `NotificationEmitter`): `actor_user_id=NULL`, `target_type='message'`, `target_id=<message id>`, `body_data = {"conversation_id": <uuid>}` (exactly one key; no content, no reason, no `message_id`).
- [x] 2.4 In the SAME transaction (only on `Applied`): write exactly one `admin_actions_log` row (`action_type='admin_chat_redaction'`, `target_type='chat_message'` (the admin-action target vocabulary — distinct from the notification's `'message'`), `target_id`, `reason`, `before_state`=original row state (content, or the embed snapshot when content is NULL), `after_state`=redacted marker, `ip=call.clientIp`, `user_agent`).
- [x] 2.5 In-app feed only — **no FCM push** for `chat_message_redacted` (matches the shipped admin `account_action_applied`; no dispatcher wired into the admin path). FCM-push-on-redaction is an explicit out-of-scope follow-up (file a `follow-up` issue).
- [x] 2.6 One transaction per redaction op (open/commit/rollback in the helper, not scattered autocommit).
- [x] 2.7 Add `admin_chat_redaction` to the destructive set in `DestructiveActionRateLimiter.COUNT_SQL` (direct `action_type IN (...)` arm) so a redaction both gates on AND counts toward the shared 20/hr cap (the `admin-destructive-action-rate-limit` MODIFIED delta). Gate the POST via `rateLimiter.isAtOrOverCap(conn, adminId)` inside the redaction transaction (mirror `ReportResolutionRepository`).
...
- [x] 4.1 Write the `chat_message_redacted` rows via a raw in-transaction `INSERT INTO notifications` helper in the redaction repository (mirroring `ReportResolutionRepository:407`); `body_data` JSON built in Kotlin + passed as `?::jsonb`. Do NOT introduce `NotificationEmitter` into the admin module. Document in code why the emitter's cross-capability shadow-ban rule does not bind here (not a `NotificationEmitter` call site; system row, no block-suppression).

## 5. Tests

- [x] 5.1 GET route tests: owner 200 + exactly ±2 context messages; `moderator` AND `read_only` → 403 (no content); malformed id → 400; non-existent → 404; already-redacted → redacted placeholder rendered; embedded-post-only target (content NULL) renders via snapshot (no crash); context window clamps at conversation boundaries (target as first/last message → fewer than 2 neighbors, no error).
- [x] 5.2 POST route tests: CSRF miss → 403 + `admin_csrf_violation` (no write); `moderator` → 403; `read_only` → 403 (no write, no audit); **CSRF-before-role precedence** — `moderator`/`read_only` + missing CSRF → 403 via the CSRF path WITH `admin_csrf_violation` written (not masked by a silent role-403); blank reason → 400 (no write).
- [x] 5.3 Repo `@Tags("database")`: first redaction sets `redacted_at`/`redacted_by`/`redaction_reason`; re-redaction with a DIFFERENT reason is a no-op that preserves the original trio (incl. `redaction_reason`) + writes no new audit/notification; not-found outcome; embedded-post-only (content NULL) message is redactable + `before_state` captures the snapshot; concurrent double-redaction → exactly one `Applied` (one audit row, one notification set) + one `AlreadyRedacted`.
- [x] 5.4 Repo `@Tags("database")`: both active participants get a `chat_message_redacted` row (`actor_user_id` NULL, `target_type='message'`, `target_id=<msg>`, `body_data` exactly `{"conversation_id": <uuid>}` — no content/reason/message_id); a `user_blocks` row between participants does NOT suppress; a `left_at IS NOT NULL` participant gets none; a no-op emits none.
- [x] 5.5 Repo `@Tags("database")`: exactly one immutable `admin_actions_log` (`admin_chat_redaction`) row on `Applied`; none on reject/no-op.
- [x] 5.6 Rate-limit test: the 21st destructive action in the hour is refused in-band with no write.
- [x] 5.7 Dual-mode response test: HTMX success → fragment with redacted render; no-JS success → 303 `Location: /admin/chat-messages/{id}`.
- [x] 5.8 New DB-tagged test HikariPool uses `autoClose(hikari())` + size 2 (CI connection budget).
- [x] 5.9 Data-plane regression guard: a redacted message still serializes `content: null` + omits `redaction_reason` on the chat REST/realtime path (the existing `chat-conversations` contract is preserved by this change).
