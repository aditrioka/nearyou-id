...
- Each arm's own keyset predicate + `ORDER BY` + `LIMIT 31` (the page-size-31 probe convention) bounds the outer input to ≤ 62 pre-sorted rows; the outer merge sort + `LIMIT 31` is O(62 log 62) — noise. The V7/V8 `post_likes` / reply-counter joins move OUTSIDE the union and run on ≤ 31 final rows (strictly fewer executions than today, where they join before the LIMIT is applied conceptually); their cardinality invariants (at-most-one / exactly-one row per outer row) are unchanged.
...
## Decision 6 — Lint mechanics (no rule changes)

- `@AllowRawPostsRead("<reason>")` goes on each SQL-holding declaration (the rule walks UP the `KtAnnotated` ancestor chain from the string literal — the annotation must enclose the literal; the shipped precedent is `JdbcPostLikeRepository.loadPostAuthorAndExcerpt` for functions and `JdbcUserProfileReader.SQL_SELF` for companion constants). Each reason names the own-content self-arm and this change.
- `BlockExclusionJoinRule`: every modified literal keeps the four tokens (the visible/first arm carries them); the rule checks the combined literal, so no `@AllowMissingBlockJoin` is needed anywhere. Raw `JOIN users` in the self-arm is outside `RawFromPostsRule`'s pattern (it matches `posts` only) and inside `BlockExclusionJoinRule`'s protected set — satisfied by the same four tokens.
- NEITHER Detekt rule is modified, and no allowlist path/regex is widened.

## Deliberately unchanged (reviewers: these are decisions, not gaps)

- **Viewer-independent aggregates stay shadow-ban-filtered for everyone**: the feeds' `reply_count` LATERAL counter and `GET /likes/count` JOIN `visible_users` and therefore exclude a shadow-banned user's own likes/replies from counters THEY see. Making them viewer-aware would make the public counts vary per viewer — exactly the block-state/ban-state leak the post-likes "Count endpoint does NOT apply viewer-block exclusion" requirement and post-replies Decision 5 forbid. Residual oracle: a shadow-banned user's own engagement never increments public counters; accepted (counter deltas are far weaker signals than vanished posts, and the alternative leaks ban state to OTHER viewers' clients via per-viewer counts).
- **Search** (`GET /api/v1/search` reads `visible_posts` and would also hide a Premium shadow-banned user's own posts from their own search): out of scope — separate surface, Premium-only, not on the mobile critical path. Filed as a `follow-up` issue at apply time rather than silently dropped.
...
- Per feed (Nearby, Global): shadow-banned author sees own post; a second user does NOT; un-shadow-ban restores normal behavior; author does NOT see own soft-deleted post; author DOES see own auto-hidden post while a second user does not; cursor pagination correct when own-shadow-banned posts interleave across the page-30 boundary (no dup, no gap); cardinality invariants hold on self rows (`liked_by_viewer`, `reply_count` populated).
- Following: shadow-banned viewer's own posts absent (parity scenario pinning Decision 3).
- Engagement: shadow-banned author can `POST /like` own post (204), read own `likes/count` (200), `POST /replies` on own post (201), `GET /replies` on own post (200, own replies visible); a second user gets 404 on the same shadow-banned-author post; author's own soft-deleted post still 404s for the author.
