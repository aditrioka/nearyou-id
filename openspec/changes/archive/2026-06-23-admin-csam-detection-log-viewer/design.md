...
Decryption is `owner`/`admin`-only and writes exactly one `admin_actions_log` `csam_metadata_decrypted` row per **authorized** decrypt invocation (docs/06 §223), surfacing the plaintext metadata as an HTMX fragment swapped into the row — it is not eagerly decrypted in the list query (which would be unauditable and bulk-exposing). **Audit-row semantics (from review):** the row is written *after* the `owner`/`admin` + CSRF gate passes — so a 403 authZ/CSRF rejection does **not** write a `csam_metadata_decrypted` row (otherwise a denied actor would pollute the audit-trail ledger that the dedicated decrypt counter reads, and could exhaust the counter). The two fail-soft cases (key unset / `encrypted_metadata` NULL) DO write the row (the authorized attempt is logged) and return "metadata unavailable" — never a 500.

**PII granularity (raw uploader id, from review).** The decrypt fragment surfaces the uploader's **raw internal UUID** — the value #358 archives in the encrypted blob (`uploader_user_id = uploaderId.toString()`). docs/06's Kominfo SOP wording says "user_id_hash"; this is a DESIGN-era doc/shipped-reality divergence. Operator-confirmed (2026-06-20) to surface the raw UUID: it is a pseudonymous internal identifier (not name/email PII), `owner`/`admin`-gated, per-decrypt-audited, and revealed one row at a time — and the raw id is what makes the Kominfo cross-reference + investigation tractable. The docs/06 SOP wording reconciliation is folded into follow-up #376.

### D5 — Rate-limit placement

- **Takedown** → the shared **20/hour destructive budget** (the existing `DestructiveActionRateLimiter`). It is a destructive/punitive action (ban + cascade-delete), so it belongs with suspend/ban/redact, not on a restorative counter. **Placement caveat (from review):** because D1 calls `CsamDetectionService.handleDetection` in-process and the *service* owns its transaction + writes its own audit row, the admin route cannot run the limiter's `isAtOrOverCap(conn, …)` inside the service's transaction the way suspend/ban do. The cap is therefore enforced as a **route-level pre-flight** check against the same shared limiter (reading the `admin_actions_log` trail) before invoking the service — a slightly-looser-than-in-tx placement that is acceptable for this rare, high-severity action. Same limiter, not a second one. A dedicated "over-budget → rejected pre-service" scenario covers it. **Contribution nuance (confirmed in implementation review):** the takedown is *gated by* the budget but does NOT *contribute* to it — `csam_takedown` is intentionally absent from `DestructiveActionRateLimiter`'s counted set (`user_suspended`/`banned`/`shadow_banned`/`warned`/`chat_redaction` + the moderation-queue resolutions). So the cap acts as a **compromised-admin circuit-breaker** (if an account is already mass-firing routine destructive actions, its CSAM takedowns are blocked too) without ever throttling a legitimate child-safety takedown by its own frequency — the desired behavior for a legally-mandated rare action.
...
- **Invariants:** raw `csam_detection_archive` reads are admin-module-sanctioned; the takedown's user/post mutations go through #358's already-lint-clean service; no `display_location`/block-join concerns (this surface touches neither timelines nor posts directly). The new `action_type`s (`csam_kominfo_reported`, `csam_metadata_decrypted`) need no migration — `admin_actions_log.action_type` is an unchecked `VARCHAR(64)` — and their rows inherit the existing out-of-band immutability posture (`admin_app` `REVOKE UPDATE, DELETE ON admin_actions_log`, provisioned via the Supabase console, not Flyway).
