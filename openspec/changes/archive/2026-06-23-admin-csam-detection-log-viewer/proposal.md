...
- **No Flyway migration.** Reads/writes existing V31 columns; extends `CsamRepository` (list/search query + Kominfo update + decrypt accessor) and reuses `CsamDetectionService` / `CsamMetadataEncryptor` / `AdminAuditLogger` — no second repository, no new pattern.
