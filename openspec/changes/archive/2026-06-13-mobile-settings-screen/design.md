...
There is **no** server consent-read endpoint (`analytics-consent-update` ships `PATCH` only; the onboarding screen issues no GET and hardcodes the V2 defaults). So `ConsentSettingsScreen` cannot issue a standalone read of the authoritative server value. It initializes from a **device-local last-submitted snapshot**, falling back to the V2 column defaults (analytics OFF, crash ON, ads OFF) when none exists. Crucially, the snapshot is **not a client guess**: `ConsentRoutes.kt` returns the full `{analytics, crash, ads_personalization}` triple in the `PATCH` **`200` response body** (`ConsentResponse`), so the snapshot is written from the server's authoritative acknowledgement of each write. On this single-device PATCH-only flow the mirror therefore cannot silently drift from the last server-acknowledged state — a stale snapshot could only under-report a change made on *another* device, which doesn't exist in the MVP. Persistence mechanism (as implemented): the `ConsentSnapshotStore` interface is bound to a **process-lifetime `InMemoryConsentSnapshotStore`** singleton (`single<ConsentSnapshotStore>`), so the snapshot survives ViewModel reconstruction within a process (a later Settings re-entry seeds from the last submit) with **no new library pin**. **Durable on-disk persistence (surviving process death)** — a platform expect/actual binding over the existing DataStore / NSUserDefaults family (the no-new-pin storage that backs `SecureTokenStore`), swapped in behind the same interface — is **deferred to issue [#198](https://github.com/aditrioka/nearyou-id/issues/198)**. Correctness is preserved meanwhile: the snapshot is a faithful mirror of the last server-acknowledged state, and the V2 fallback is privacy-safe, so a cold start simply re-shows the safe defaults until the user re-opens consent. The reused mobile consent seam (`consent/ConsentApiClient`, `ConsentFlow`, `ConsentOutcome`) is the networking path — **no second consent path** (anti-patchwork). A dedicated server consent-GET (multi-device robustness) is also a follow-up — the `PATCH` `200` already round-trips authoritative state, so the GET is a robustness nicety, not a single-device correctness gap. Alternative rejected: re-defaulting to V2 on every settings entry (would silently misrepresent a user who changed consent — a UU-PDP-relevant misstatement); alternative rejected: blocking the consent settings sub-screen until a GET ships (drops a backed write the operator wants).

### D6 — Logout is a client-side token wipe + `replaceAll` to sign-in

A confirmation dialog (`stringResource` copy) → `SecureTokenStore` clear → navigation `replaceAll` to the sign-in surface (authenticated back stack cleared; back gesture can't return). The `replaceAll` is dispatched as a navigation event consumed at the call-site back stack (the `PostCreationScreen` / auth-flow precedent), so it survives the `SettingsRoute` NavEntry — and its `viewModel`-scoped holder — being torn down as the route it replaces unwinds. No server call: the MVP relies on the client token wipe; server-side `token_version` rotation / token revoke-on-logout is a separate concern not gated by this UI. Mirrors the existing token-write discipline (the screen touches `SecureTokenStore` only for the clear). **Pre-launch hardening follow-up**: server-side bearer revoke on logout (so a wiped-but-exfiltrated token can't be reused until natural expiry) is filed as a `follow-up` issue — standard pre-launch expectation, explicitly out of this MVP UI change.
...
The project.md live-menu row #5 text lists "account-deletion entry, suspension-countdown UI"; the canonical mockup frame 16 contains **neither** (it has the Premium/PRIVASI/LAINNYA rows + logout instead). Per CLAUDE.md "the canonical visual reference is the mockup board … on behavior conflicts, specs/docs win over mockups" — here it is not a behavior conflict but an **entry-set** question (which rows exist), which the mockup governs. Additionally, neither has a backend endpoint (account deletion "ships later" per `AuthPlugin.kt`; suspension is surfaced only at the auth/write-403 boundary with no client read). Shipping either now would be a dead control. Resolution: defer both as `follow-up` issues; record the divergence here so a reviewer/operator can reconcile the live-menu text later if desired. This is a bucket-(c)-style divergence surfaced to the operator at proposal time (the operator confirmed the mockup-faithful scope). The `project.md` row-#5 wording ("account-deletion entry, suspension-countdown UI") will read stale once this lands without those entries; an **archive-time task** (tasks.md § 8) trims that row to the shipped settings scope so the live menu stays accurate.

### D9 — File placement: data seam in the § 2.1 target shape; screens co-located with the in-flight peers

The block-list **data seam** lands under the docs/11 § 2.1 target shape — `data/block/` (`BlockedUsersApiClient`/`Repository`/`Outcome`), matching the already-migrated `data/like/` peer — not a legacy top-level `block/` package. The **screen** files land under `screens/settings/**`, co-locating with the in-flight critical-path peers (`mobile-profile` #245, `mobile-following-timeline` #246, chat #247, search #248), which all add under `screens/` during the same migration window; and the new NavKeys edit `screens/routing/NavKeys.kt` where that file already lives. This keeps the new networking code on the target shape (the part § 2.1 cares most about) while avoiding a screens-package split from the sibling PRs landing the same week. No Pattern-Registry fork — the seam is the standard `ApiClient → Repository → sealed Outcome` shape regardless of directory.

### D10 — Implementation notes (settings-row icons, block-list pagination)

- **Row icons.** Material leading icons per mockup frame 16 are bundled as `:shared:resources` vector drawables (the app's `painterResource(Res.drawable.ic_*)` idiom — there is no `material-icons` font dependency, so adding one would breach "no new pin"). Nine standard Material baseline glyphs were authored (`ic_arrow_back`, `ic_chevron_right`, `ic_alternate_email`, `ic_workspace_premium`, `ic_credit_card`, `ic_lock`, `ic_block`, `ic_description`, `ic_logout`); the existing `ic_nav_profile` / `ic_post_location` / `ic_privacy_shield` cover the remaining rows. Exact glyph parity (esp. the Premium medal) is verified in the post-#245 DoD visual pass.
- **Block-list pagination.** `GET /api/v1/blocks` is cursor-paginated; this change renders the **first page only** (the demo blocklist is small) and does NOT wire `nextCursor` load-more — deferred to a `follow-up` issue per the spec's pagination-deferral allowance. The first page renders correctly; the cursor is preserved on the wire DTO for the follow-up.
