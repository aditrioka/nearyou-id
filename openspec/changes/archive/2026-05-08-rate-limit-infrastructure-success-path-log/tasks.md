...
- [x] 6.1 Trigger a manual staging deploy on the change branch: `gh workflow run deploy-staging.yml --ref rate-limit-infrastructure-success-path-log` (per the `/opsx:apply` skill's pre-archive smoke convention codified post-`reply-rate-limit`).
- [x] 6.2 Monitor the deploy run via `gh run list --workflow=deploy-staging.yml --limit=1` until it reports green. Capture the deployed Cloud Run revision name from the run's logs (or via `gcloud run services describe nearyou-backend-staging --region=asia-southeast1 --format='value(status.latestReadyRevisionName)'`).
- [x] 6.3 **Smoke ran AGAINST PRE-PIN STATE** (root=trace alone, no RedisRateLimiter pin) and captured 9 entries. **POST-PIN STATE** (this change's V3 round-2-fix iteration): logback.xml now pins `id.nearyou.app.infra.redis.RedisRateLimiter` to INFO, so DEBUG entries are filtered by default. To reproduce the §6 smoke against post-pin staging, an operator must temporarily edit logback.xml on a branch (lower the pinned logger to DEBUG) + redeploy. The runtime log-shape evidence is preserved in `:infra:redis` integration tests via `captureDebugAndWarnings` (which explicitly bumps the level to DEBUG). See design.md Decision 1 + Q3 (resolved) for rationale.
- [x] 6.4 Issue 5 sequential `GET https://api-staging.nearyou.id/health/live` requests from a single test client (the same shape as `rate-limit-ip-hashing` §6.2). Capture the request timestamps in UTC.
- [x] 6.5 **Wait 90 seconds** after §6.4 completes before running the Cloud Logging filter — Cloud Run stdout → Cloud Logging ingestion lag on staging can run 30-90s. Then run the filter `resource.type="cloud_run_revision" AND resource.labels.service_name="nearyou-backend-staging" AND textPayload:"event=rate_limit_check"` over the smoke window (use `textPayload` not `jsonPayload.message` — the Logback ConsoleAppender writes plain-text patterns to stdout, not structured JSON, per the existing `<pattern>%d{YYYY-MM-dd HH:mm:ss.SSS} [%thread] %-5level %logger{36} - %msg%n</pattern>` shape in logback.xml). Capture all matching entries. **Retry-on-empty pattern**: if zero entries match on first query, wait another 30s and retry up to 3 attempts before declaring failure (mirrors the smoke retry pattern from PR #74's §6.5).
- [x] 6.6 **Assert** (the §6.5 verification gap closure):
...
- [x] 6.7 Paste smoke evidence into a PR comment: timestamp window, 5+ captured `key=` values (hashed form), `remaining=` decrement series, and explicit "no `user_id=` in any entry" attestation. Link the PR comment from the Section 7 archive task body.
- [x] 6.8 If 6.6 fails (raw IP appears, OR `user_id=` leaks on key-axis, OR fewer than 5 entries match after the retry-on-empty pattern): roll back the implementation commits on the branch via `git revert <sha>`, debug, and retry. Do NOT proceed to archive until 6.6 is fully green.
- [x] 6.9 **Smoke noise-floor check** (sanity): the §6.5 filter `textPayload:"event=rate_limit_check"` is specific enough to isolate the new log entries from the broader `<root level="trace">` Cloud Logging stream. Verify by running the filter WITHOUT issuing the §6.4 smoke requests — should return 0 entries from the smoke window (confirms the filter only matches new entries, not background app DEBUG/TRACE noise from Lettuce / Netty / JJWT).

## 7. Archive

- [x] 7.1 Run `openspec archive rate-limit-infrastructure-success-path-log` locally — verify the move from `openspec/changes/` → `openspec/changes/archive/` and the spec sync into `openspec/specs/rate-limit-infrastructure/spec.md`. **Expected output shape: `+ 2, ~ 0, - 0, → 0`** — two requirements ADDED at the capability level (the change uses ADDED only; zero MODIFIED, zero REMOVED, zero RENAMED). If the actual output differs, that's a flag — the most likely cause is an inadvertent spec-delta operator change during proposal-review. Empirically verify the archive output matches before pushing the archive commit.
...
- [x] 7.3 Stage the archive changes (move + spec sync) and commit on the SAME branch with `chore(openspec): archive rate-limit-infrastructure-success-path-log`.
- [x] 7.4 Push the archive commit + update the PR body via `gh pr edit <pr> --body "$(cat <<'EOF' ... EOF)"` to the merge-ready shape per `openspec/project.md` § Change Delivery Workflow. Drop the in-progress framing; list final test counts + capability deltas + post-merge tasks (Section 8).
...
- [ ] 8.1 **Post-merge smoke is OPTIONAL given the logback pin landed in this change.** The pre-archive §6 smoke proved the runtime log shape pre-pin (9 entries captured); post-pin, `RedisRateLimiter` is filtered to INFO by default, so an unmodified post-merge smoke against `main`-deployed staging will return 0 entries (expected — the pin is doing its job). To reproduce the §6 smoke against `main`-deployed staging, file a separate ad-hoc op-flow change that temporarily lowers the logger level + redeploys + runs the curl + reverts. The integration-test surface in `:infra:redis` is the canonical regression gate going forward — those tests bump the level to DEBUG explicitly via `captureDebugAndWarnings`, so any future regression in the log shape fails CI before reaching production. Mark this task N/A (or "verified by pre-pin §6 smoke + integration tests") in the archive commit body.
