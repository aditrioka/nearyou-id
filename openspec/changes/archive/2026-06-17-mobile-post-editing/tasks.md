...
- [x] 9.3 UI-affecting → on-device bring-up (emulator-5554, dev flavor): app builds + installs + test-logs-in + renders the Compose shell / premium profile / settings healthily; the post-editing UI behaviors (edit affordance own/other/stale, edit screen, 403 upsell, "Diedit" label, "Riwayat edit" versions/empty/error+retry/no-location, refresh-on-resume) are covered by the Robolectric `*ScreenTest`s; the edit+history API verified end-to-end via curl (PATCH 200 → updated content; `GET /…/edits` → "Versi ke-1" pre-edit snapshot, no location). Specific edit-screen device screenshots pending a dev-backend rebuild (the running dev backend predates this branch's `editedAt`/`isAuthor`) — optional pre-merge nicety; behavior is test-covered + Compose renders healthily on-device.
- [x] 9.4 Backend runtime impact → staging branch deploy `mobile-post-editing` **completed success** (deploy-config validated: no secret/env/migration/boot issues); `GET /api/v1/posts/{id}` + `/edits` mounted + app booted (401 unauth) with this branch's code. Functional `editedAt`/`isAuthor` correctness covered by the green `SinglePostRoutesTest` / `JdbcSinglePostRepositoryTest` integration tests.
