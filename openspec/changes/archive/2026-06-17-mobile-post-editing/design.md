...
- **Implementation guardrail — visibility:** the `post_edits` derivation MUST hang off the already-resolved post row (after `JdbcSinglePostRepository.findById`'s `UNION ALL` visible/own-content two-arm gate + `LIMIT 1`), not a top-level/independent scalar subquery — otherwise it could surface an edit-existence signal for a post the viewer can't see (blocked/shadow-banned/auto-hidden). An invisible post yields no row → no `editedAt`.
- **Implementation guardrail — lint integrity:** the subquery MUST be added INSIDE the existing single triple-quoted SQL literal in `findById` (where the four block tokens `visible_posts` / `user_blocks` / `blocker_id =` / `blocked_id =` already co-reside), so `BlockExclusionJoinRule` keeps passing; `post_edits` is in neither `BlockExclusionJoinRule` nor `RawFromPostsRule`'s pattern, so it adds no annotation — but splitting the SQL into a second literal that re-mentions `posts`/`visible_posts` without the block tokens would trip the rule.
...
## Apply-time refinements (discovered during `/opsx:apply`)

- **Post-detail is route-payload-driven, not single-post-GET-driven** (the proposal assumed it round-tripped for its header; it does not). Operator-confirmed resolution ("fetch on open"): post-detail fetches `single-post-read` on each resume via `LifecycleEventEffect(ON_RESUME)` (the `ProactiveRefreshEffect` precedent — fires on first open AND when the edit screen pops, the revealed entry going RESUMED), freshening `content` + reading `editedAt` + `isAuthor`. A failure degrades silently to the route payload.
- **`isAuthor` added to `single-post-read`.** Ownership can't be determined client-side — neither the route nor the projection carries `author_id`. Rather than thread `isOwnPost` through the timeline layer (more files), the server computes `isAuthor` from `author_id` WITHOUT exposing the UUID (a per-viewer boolean) — server-authoritative, no PII. The Edit affordance gates on `isAuthor && within-30-min-window` (`isWithinEditWindow`, a pure testable helper).
- **Editor kept as a Nav3 destination (D3), not an overlay.** Per-entry lifecycle (`rememberViewModelStoreNavEntryDecorator`) makes the resume-refresh fire on pop, so the Nav3 destination reflects the edit without cross-entry result-passing. The "Riwayat edit" history is a screen-local full-screen overlay (in-composition, not a popup `Dialog`/`ModalBottomSheet`) — testable + idiomatic with the post-detail "Tutup" pattern.
- **Placement coheres with the post feature** (`screens/post/` + `post/`, like PostCreation/PostDetail), not a new `ui/postedit/` package — `docs/11` § 4 naming coherence (extend the existing feature). `EditPostViewModel` adopts the § 2.2 ViewModel pattern (the project's newer screens use it).
