...
- **Activity gate ships the durable legs only**: invitee has ≥ 2 posts inside the ticket window AND the ticket is not expired AND the inviter is still eligible (neither hard- nor shadow-banned). **DEFERRED** (no durable data source exists yet, per docs/05 §1186): the ≥ 3-login-days and ≥ 5-app-sessions engagement legs (docs/01 §212) and the **90-day-windowed device-fingerprint** + IP-subnet (/24) + recently-seen-identifier anti-collision legs (docs/01 §213 item 3). The residual abuse surface is bounded by the already-shipped signup-time *exact-equality* device-fingerprint check (point-in-time, not the deferred 90-day-windowed variant) + the per-inviter 3/week burst limit; grants are worth only 1 week of Premium.
