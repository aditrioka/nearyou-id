...
- [x] 10.5 Smoke results recorded above. **iOS smoke ALSO VERIFIED** (iPhone 17 Pro sim, iOS 26.0) after resolving two PRE-EXISTING iOS-build issues — NOT this change (the age-gate code is commonMain): sign-in → `404` → `AgeGateScreen` (renders correctly, light + dark) → DOB picker → under-18 DOB → `/signup` `403` + the `age_gate_under18_blocked` copy renders. The keystone **status-driven `403`→Blocked mapping was confirmed against the real FLAT `{"error":"user_blocked",…}` body** (Ktor logs the unparseable-as-nested envelope, the code catches it and maps on HTTP status → Blocked, exactly per D2). Anti-DOB-shopping confirmed live: a blocklisted identity stays `403` even with an 18+ DOB. The `18+ → 201 → Home` leg specifically needs a *fresh* (non-blocklisted) Google account — the `/signup` call itself is proven. The two build issues (both separate iOS-infra follow-ups): (a) CocoaPods `compose-resources` not bundled into the `.app` → fixed by re-running `pod install` *after* the resources are generated (build-order; enforce in the project build); (b) the earlier sign-in "NetworkError" was **stale-build cruft** — a clean DerivedData rebuild resolved it; the iOS sign-in code was always correct (`idToken present=true`). **Net: verified on Android + iOS.**
