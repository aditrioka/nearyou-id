...
- **Moderation-hit behavior**: the change is **REJECTED upfront** and an idempotent `moderation_queue` row (`target_type='user'`, `target_id=<user id>`, `trigger='username_flagged'`, `ON CONFLICT DO NOTHING` per V9's `UNIQUE(target_type,target_id,trigger)`) is inserted for admin awareness (authoritative per [`docs/06-Security-Privacy.md`](../../../docs/06-Security-Privacy.md) § Premium Username Moderation); the >3-flagged/24h case is throttled by the failed-attempt limit. The numeric anomaly-score increment docs/06 mentions is **deferred** — no `anomaly_score` substrate exists, and this change stays migration-free (see deferred list).
...
- **Explicitly deferred** (separate follow-on changes, captured as guard requirements here — NOT built in this change): the mobile Settings UI (Premium entry, paywall, live probe, cooldown countdown, downgrade banner); the Phase 3.5 admin username-change oversight (`username_history` viewer + borderline-candidate override + manual handle release); and the numeric anomaly-score increment on repeated flagged attempts (lands with the anomaly-detection capability, docs/08 Phase 4 #17).
...
- **Critical invariant touched**: the `users.username` write carries `// @allow-username-write: customization` (invariant #7, [`openspec/project.md`](../../../openspec/project.md):161 — a comment-convention allowlist; this is the **first-ever username `UPDATE` writer**, the slot the invariant reserves). Content-length guard (30-char app cap) and the rate-limit TTL / Redis hash-tag invariants also apply.
