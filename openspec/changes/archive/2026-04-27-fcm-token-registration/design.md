...
- [`docs/04-Architecture.md`](docs/04-Architecture.md) — registration trigger semantics (token refresh callback, login/logout) + the freshness signal `last_seen_at`.
...
- Make the upsert idempotent and race-safe via `INSERT ... ON CONFLICT (user_id, platform, token) DO UPDATE` — refreshing `last_seen_at` is the dominant success path (token-refresh callback fires periodically per [`docs/04-Architecture.md`](docs/04-Architecture.md)).
...
3. **All other claims align verbatim** with canonical docs:
   ...
   - Module ownership: [`docs/04-Architecture.md`](docs/04-Architecture.md) "User module owns FCM token registration" ↔ proposal § Impact + design § Constraints (with the explicit "creates new `user/` package" note).
