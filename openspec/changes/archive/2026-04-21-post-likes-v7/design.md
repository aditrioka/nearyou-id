...
- **Risk:** Adding the `LEFT JOIN post_likes` to the timeline query regresses latency under deep scroll. → **Mitigation:** The join is PK-scoped (`post_likes_pk = (post_id, user_id)`), returns at most one row per page row, and the primary query already touches `visible_posts` (the most expensive join); the planner merges the LEFT JOIN into the existing hash. Phase 2 benchmark (`docs/08-Roadmap-Risk.md`) is already in scope for timeline latency — `liked_by_viewer` is validated there. No new index needed.
