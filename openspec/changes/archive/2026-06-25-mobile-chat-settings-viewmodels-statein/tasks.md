...
- [x] 5.3 `./gradlew :mobile:app:iosSimulatorArm64Test`: K/N compile ✓ + the migrated/new commonTest VM tests pass on iOS. (Any pre-existing `*FlowIosTest` DI-drift failures, #348, are unrelated — iOS is never CI-gated and this change touches no DI.)
- [x] 5.4 Manual verification per docs/11 §5 DoD — **operator chose a connected-device screenshot pass**. Built `installDevDebug` on a physical device (Samsung A17), seeded the back stack to `ConversationListRoute` + `BlockedUsersRoute` via a temp offline harness (Koin-override `TokenStore` far-future + canned `Loaded` for both flows; reverted before commit), and captured both screens rendering the **Content** state over the new single-`uiState` collection: Pesan list (Raka Pratama / Dewi Lestari rows + dates) and Pengguna diblokir (Andi/Siti rows + @handle + "Buka blokir", no UUID). Screenshots in the PR body (evidence branch). The Robolectric `ConversationListScreenTest` + `BlockedUsersScreenTest` render the same states green.
