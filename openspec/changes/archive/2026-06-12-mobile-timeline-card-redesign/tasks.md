...
- [x] 7.3 Manual verification per docs/11 §5 DoD #3 (`verify-loop`): Android emulator AND iOS simulator — screenshot Nearby + Global feeds light/dark vs mockup frames 1/19, post-detail header identity, app bar logo; attach evidence to the PR body; `mobile-ui-foundation` checklist pass
- [x] 7.4 Staging branch deploy + smoke per docs/11 §5 DoD #4 (runtime-impacting backend change): authenticated `GET /api/v1/timeline/nearby|global|following` on staging each return `authorUsername`/`authorDisplayName` on every post (mint via `dev/scripts/mint-staging-jwt.sh`)
