...
- **NEW**: Flyway migration `V16__admin_users.sql` shipping five admin tables verbatim from canonical sources at [`docs/05-Implementation.md`](../../../docs/05-Implementation.md) § Admin Users Schema (lines 636-703) and § Admin Actions Log Schema (lines 1184-1208):
  ...
  - `admin_sessions` — admin_id FK CASCADE, session_token_hash TEXT UNIQUE, **csrf_token_hash TEXT NOT NULL** (mandatory per the `csrf_token_hash` invariant documented at [`openspec/project.md`](../../project.md) § Coding Conventions: *"Admin sessions: every `INSERT INTO admin_sessions` must populate `csrf_token_hash`"*; the schema layer is the first concrete enforcement — a future Detekt rule will lift this to a CI-blocking check), ip INET, user_agent, created_at, last_active_at, expires_at, revoked_at; partial index `WHERE revoked_at IS NULL`.
...
- **NEW**: FK backfills converting the three nullable UUID columns into validated FKs to `admin_users(id) ON DELETE SET NULL` via `ADD CONSTRAINT … NOT VALID + VALIDATE CONSTRAINT` (matches the V9 deferral comment's prescription):
  ...
  - `chat_messages.redacted_by` — **scope expansion vs the menu entry** at [`openspec/project.md:111`](../../project.md), which lists only the V9 columns. The V15 column carries the same deferral marker ([V15:99](../../../backend/ktor/src/main/resources/db/migration/V15__chat_foundation.sql:99)) and is gated by the same Phase-3.5 admin-users dependency; including it here closes all three in one atomic migration and avoids a near-immediate follow-up. Scope expansion is intentional and documented; the menu entry will be implicitly amended on archive (the shipped capability covers all three columns).
...
- `reports`: the `reports table created via Flyway V9` requirement carries two scenarios asserting the deferred FK state ([`openspec/specs/reports/spec.md:62-68`](../../../openspec/specs/reports/spec.md)) — those scenarios become FALSE the moment V16 lands. MODIFIED delta removes the deferred-state scenarios and adds the FK-present + comment-replacement scenarios.
- `moderation-queue`: analogous — [`openspec/specs/moderation-queue/spec.md:78-84`](../../../openspec/specs/moderation-queue/spec.md) carries the equivalent two scenarios for `resolved_by`. MODIFIED delta swaps them.
- `chat-conversations`: the `Conversation schema` requirement's prose at [`openspec/specs/chat-conversations/spec.md:9, 15`](../../../openspec/specs/chat-conversations/spec.md) describes `redacted_by` as "FK constraint to admin_users(id) ON DELETE SET NULL is DEFERRED to Phase 3.5". The deferral text on `redacted_by` becomes false post-V16 (note: the analogous deferral on `embedded_post_edit_id` to a future `post-edit-history` change is unchanged — that FK target table is unrelated and still deferred). MODIFIED delta amends only the `redacted_by` half of the prose.
...
- **Schema invariants**: `csrf_token_hash` column ships as `NOT NULL`, making the documented `csrf_token_hash` invariant (per [`openspec/project.md`](../../project.md) § Coding Conventions) DB-enforced for the first time (no Detekt rule exists yet — a future change can add one and use this schema-layer enforcement as backup). Admin-user FK `ON DELETE SET NULL` invariant (in `openspec/project.md` § Coding Conventions) is now enforced at the schema layer for all three operational tables.
- **Downstream unblocks**:
