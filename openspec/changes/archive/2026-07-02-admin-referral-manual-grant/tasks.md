...
- [x] 7.10 Read viewer: newest-first keyset pagination; `q` + date filters; a **tombstoned/soft-deleted grantee** row still renders (JOIN tolerance); plain-`GET` (no HTMX) still renders escaped. (Review round 1 closed the coverage gaps: 51-row keyset round-trip through the rendered next link — no overlap, filter-preserving; malformed cursor → 200; `q` narrows by username AND UUID; `date_to` inclusive-end; plus cursor-codec unit tests.)
...
- [x] 7.12 Dispatch `Failed` (MockEngine 4xx/5xx, RC configured): audit row written, Premium not activated, failure surfaced to the admin with retry guidance, rate-limit counts the attempt, no throw. (Covered at the port layer by a fake `Failed`-returning granter — the HTTP 4xx/5xx→`Failed` mapping itself is the pre-existing `:infra:revenuecat-api` MockEngine suite's; review round 1 added the route-level `Failed`-message render test via an injected failing granter.)
...
- [x] 8.1 File a `follow-up` issue (and/or amend in-PR) reconciling docs/07 § Referral Manual Grant Path: replace the stale `granted_entitlements` (`source/grant_role = 'manual_admin'`) prose with the as-built mechanism (RC promotional grant + GRANT webhook echo + `admin_actions_log` audit row; `granted_entitlements` deliberately untouched). (Filed: #441; review round 1 then amended docs/07 in-PR directly — close #441 as carried-by-PR at merge.)
