...
- **Verification evidence (§7.2).** The real `admin()` Ktor module + Pebble templates were rendered through the test harness (a throwaway renderer, since removed) and screenshotted with the production `admin.css`; the page shows the soonest-first list, urgency pills, source badges, the already-expedited handled indicator, the danger expedite controls, and the warn banner. Hosted on the orphan `evidence/admin-hard-delete-queue` branch. The 31 DB-tagged tests prove the behavior + rendered-HTML structure through the same real stack.
