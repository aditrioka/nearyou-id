...
- **Deferred — known same-shape forks (out of audit-05's feed/timeline scope):** `ConversationListViewModel` (`mobile-chat`) and `BlockedUsersViewModel` (settings) carry the *identical* `remember(outcome, isInitialLoad) { xxxUiState(…) }` in-composable-projection shape but live in different feature domains that audit 05 (shell + timeline + notifications) did not cover. They are intentionally NOT migrated here (this change is the cohesive feed/timeline family that shares the `LoadMoreController` / `InlineLikeController` substrate) and are tracked as follow-up [#410](https://github.com/aditrioka/nearyou-id/issues/410) so a future session does not re-fork the convention. Already on the single-`stateIn` shape (no work): `ProfileViewModel`, `FollowListViewModel`, the auth VMs, `PaywallViewModel`, `UsernameCustomizationViewModel`, `EditPostViewModel`.
