...
- [x] 4.4 Manual verification (docs/11 §5 DoD): verified live on the verify36 emulator via the offline Nav3 seed harness — the post-header report kebab ("Opsi lainnya") opens the single "Laporkan" menu item → the shared `ReportDialog` renders with title "Laporkan postingan ini", the six user-facing categories (Spam / Ujaran kebencian (SARA) / Pelecehan / Konten dewasa / Misinformasi / Lainnya — **no** self_harm/csam), the optional-note field ("Jelaskan lebih detail jika perlu"), and Batal / "Kirim laporan". Screenshots captured under `dev/device-runs/mobile-content-report/`. The shared dialog is identical for the reply entry point (title "Laporkan balasan ini"); affordance gating (post `!isAuthor`, reply ungated) + the submit/PII paths are covered by the Robolectric `PostDetailScreenTest` + the capturing-fake `PostDetailViewModelTest`.
