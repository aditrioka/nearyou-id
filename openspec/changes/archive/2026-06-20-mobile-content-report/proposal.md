...
- **Report a reply from `PostDetailScreen`.** Each reply row gains a "Laporkan" affordance; submission posts `target_type = "reply"`, `target_id = <reply id>`. PII discipline is preserved — reply rows never *render* `author_id` (the `mobile-post-detail` contract; the wire/DTO carries it but it is never shown), and the report sends only the reply id, exposing no new PII.
