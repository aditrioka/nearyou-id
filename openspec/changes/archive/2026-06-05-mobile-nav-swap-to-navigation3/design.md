...
### Decision 5: State retention — **both decorators; the Nearby feed's load state is scoped to a `HomeRoute` ViewModel**

**Choice:** `NavDisplay.entryDecorators = listOf(rememberSaveableStateHolderNavEntryDecorator(), rememberViewModelStoreNavEntryDecorator())` (the canonical Nav3 order), plus the `org.jetbrains.androidx.lifecycle:lifecycle-viewmodel-navigation3` artifact (JetBrains CMP port, `2.10.0` — the project's existing `androidx-lifecycle` pin). The Nearby feed's load state (`outcome` / `inFlight` / the reload trigger) moves out of `NearbyFeed`'s `remember` into a `HomeRoute`-scoped `NearbyTimelineViewModel`, resolved via `viewModel { NearbyTimelineViewModel(flow) }` (androidx `viewModel`, constructed with the `koinInject`-ed `NearbyTimelineFlow` — no Koin VM registration, so the test modules are unchanged).

**Why (the reload-on-return fix):** the saveable-state-holder decorator only preserves `rememberSaveable`; plain `remember` (and `LaunchedEffect`) is **disposed** when an entry goes off-screen (Nav3's documented "state-loss on back" behaviour). So with the feed's load state in `remember`, opening the composer (`PostCreationRoute` on top) disposed `HomeRoute`, and popping back re-composed it → the load `LaunchedEffect` re-fired → the Nearby feed **re-fetched** every return (visibly: a loading skeleton / location re-acquire; the apparent inconsistency was the 60s/90s `LocationTuning` warm-fix window making the refetch fast-and-imperceptible vs slow-and-visible). The canonical Nav3 fix — confirmed against the official Android save-state doc, `android/nav3-recipes`, and the Koin Nav3 integration doc — is to scope that state to the `NavEntry` via the ViewModel-store decorator: the VM survives the entry going off-screen and is cleared only when `HomeRoute` is popped, so returning reuses the loaded feed. Home is the "first ViewModel-backed screen" this decision originally named as the trigger for landing the decorator. (Note: this was discovered by on-device manual testing during `/opsx:apply` 9.5; folded into this change rather than deferred. Koin issue [#2235](https://github.com/InsertKoinIO/koin/issues/2235) — VM not cleared on pop with `koinViewModel` — does not apply: we use androidx `viewModel`, not `koinViewModel`.)

**Scope note:** only the Nearby feed's load state is hoisted to a VM. The `LocationGate` stays `remember`-ed (its `ON_RESUME` re-check is intentional, and its `status()` projection is fast — the brief gate `Loading` is not the visible reload the fix targets). Migrating the gate into the VM, and entry-scoping the composer draft, are out of scope.
