...
**D3 — Gate on the row projection `isReportable = !isOwn && !isRedacted`.** Only the *other* party's, non-redacted messages expose "Laporkan" (mirrors the post `!isAuthor` gate). The gate is computed on the rendered `ChatMessageRow`, which deliberately carries `isOwn` / `isRedacted` and **drops the raw `senderId`** (PII discipline — `isOwn` is derived at `chatMessageRows(...)` projection from `senderId == viewerId`). The implementation MUST NOT re-thread `senderId` onto the row, the long-press menu, or the report path just to satisfy a literal "`senderId != viewerId`" reading — `!isOwn` is the row-level realization. This is a **client-side UX gate, not a security boundary** — the backend `self_report_rejected` guard fires only for `target_type = "user"`, so a bypassed self-report of one's own chat message is accepted as a harmless, rate-limited, **recorded-but-inert** action: it writes a `reports` row attributable to the reporter but flips no column and cannot auto-hide (`chat_message` does not participate in the post/reply auto-hide coupling, and auto-hide needs 3 *distinct* aged reporters). We gate it out for UX clarity, not correctness. *Alternative rejected:* expose report on all messages — needless clutter, and a "report my own DM" action is meaningless.
...
- **PII leakage via logs.** → The seam already forbids logging tokens/bodies/targetId; this change adds no log of `senderId`, message body, or the JWT `sub`. Enforced by a source-scan / body-free-`LogLevel` test scenario.
- **Re-introducing `senderId` to satisfy the gate (PII regression).** → The long-press menu + report path operate on `ChatMessageRow` (no `senderId`); at apply time, do NOT pass the raw `ChatMessage` (which carries `senderId`) into the menu/affordance composable to recompute the gate — use the row's `isReportable`/`isOwn`. Guarded by the PII source-scan test.
- **iOS flow-test has no chat template.** → There is no pre-existing chat `iosTest`; the new chat report iOS flow test is modeled on `PostDetailFlowIosTest` (plain `fun name()` — already Kotlin/Native-legal), not a nonexistent chat one.
