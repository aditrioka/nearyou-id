...
- [x] 5.1 Repoint the Following pager page (page 1) in `screens/home/HomeScreen.kt` from `FollowingPlaceholderScreen` to `FollowingTimelineScreen`, hoisting `onOpenPost` / `onOpenPostReply` into it (identical to Nearby/Global) and wiring `onSeeGlobal = { scope.launch { pagerState.animateScrollToPage(<Global index>) } }`
...
- [x] 6.4 Robolectric `FollowingTimelineScreenTest` (androidUnitTest) — initial render + each of the six states via `FakeFollowingTimelineFlow` (incl. directive empty + "Lihat Global" CTA firing `onSeeGlobal`; no-UUID/no-coords assertion; SessionExpired redirect ≠ connectivity copy); ADD `**/FollowingTimelineScreenTest*` to the `mobile/app/build.gradle.kts` Release-variant exclude
...
- [x] 7.3 Manual verification per docs/11 §5 DoD #3 (`verify-loop` / `mobile-ui-foundation`): cloud sandbox routes to Firebase Test Lab via `scripts/run_on_device.sh` — screenshot the Following tab in its directive empty state (follows nobody) and, where a follow can be seeded, the loaded feed; light + dark; verify against `nearyou-screens-mockup.html` frame 1 (Beranda feed) — attach evidence to the PR body BEFORE archive. **Done via `device-run.yml` CI** (Firebase Test Lab Robo on a real Pixel, shiba/Android 34, staging flavor): ✅ app launched + crawled OK, 4 screenshots captured ([run](https://console.firebase.google.com/project/nearyou-staging/testlab/histories/bh.c31303f3a7015391/matrices/7012872079647335889)); evidence in the PR body.
