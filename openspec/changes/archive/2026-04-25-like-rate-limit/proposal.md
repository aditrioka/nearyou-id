...
- **MODIFIED: `POST /api/v1/posts/{post_id}/like`** — two limiters run in order before any DB work:
  ...
  - **Burst**: 500/hour, both tiers (anti-bot, per [`docs/02-Product.md`](../../../docs/02-Product.md) "Both tiers cap 500/hour burst"). Key `{scope:rate_like_burst}:{user:<uuid>}`. Sliding-hour window.
