...
**Why.** Lettuce is the mainstream Netty-based JVM Redis client (vs Jedis), supports cluster + Upstash flat mode, and is the standard pick in modern Ktor stacks. Sync API keeps the call shape identical to V9's wrapper — minimizing diff matters because V9's `ReportEndpointsTest` (HTTP-level, end-to-end) is the byte-for-byte client-contract regression gate. Async can be revisited if a benchmark shows it matters.
...
- **[Risk] CI Redis container missing on test runs.** → Mitigation: the existing CI test job already runs PostgreSQL via `postgis/postgis` service container; add a `redis:7-alpine` service container alongside it (`docker-compose.yml` already declares one for dev), and have `KotestProjectConfig` boot it once per test JVM. V9's `ReportRateLimiterTest` is re-pointed at an in-memory `RateLimiter` test double (no Redis needed for that unit-speed plumbing check); V9's `ReportEndpointsTest` and the new `RedisRateLimiterIntegrationTest` both depend on the Redis container — they fail closed if it's missing, surfacing the misconfiguration immediately. See Decision 7 for the test-gate split rationale.
