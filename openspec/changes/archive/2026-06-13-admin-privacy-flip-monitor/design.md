...
**Single bound evaluation instant, NOT SQL `now()`.** The classification and the `status` filter (D4) both compare against a **single request-scoped `eval_instant`** computed once at the top of the handler from an **injectable `Clock`** (`Clock.systemUTC()` in production; a fixed `Clock` in tests), and **bound as a positional `PreparedStatement` parameter** reused in BOTH the projection's `CASE WHEN ? < privacy_flip_scheduled_at THEN 'in_window' ELSE 'overdue' END` and the `status` predicate. **Why a bound parameter over SQL `now()`:** (a) it preserves the no-skew guarantee exactly as SQL `now()` would (one value drives class + filter), AND (b) it is **deterministically testable at the boundary** — a test can inject a fixed `Clock` and seed a row at exactly `eval_instant` to pin the `>` vs `>=` fencepost, which SQL `now()` makes impossible (you cannot seed a row at the server's evaluation of `now()`). The test-coverage review flagged this explicitly; the injectable clock is the answer. **This makes the docs predicate the IN_WINDOW subset, with zero divergence**: the monitor is a strict superset of the canonical population, and the OVERDUE superset is the documented stuck-row extension, not a contradiction of it (called out for B.3 reconciliation).

### D4: `status` (`in_window` / `overdue`) + optional `q` user-search — composable, lenient, parameterized

- **`status`**: `in_window` → `AND ? < privacy_flip_scheduled_at`; `overdue` → `AND privacy_flip_scheduled_at <= ?`, where `?` is the SAME bound `eval_instant` used by the D3 classification projection — so a `status=overdue` page contains EXACTLY the rows the table classifies OVERDUE (no read-vs-filter skew, even at the boundary). An unrecognized / over-long value is ignored (lenient), leaving the unfiltered both-buckets view.
...
- **Classification + `status` filter skew.** Computing the row class and the `status` predicate against two different clock reads could misclassify a row crossing its deadline mid-request. **Mitigation:** D3 — both the projection's `CASE` and the `status` predicate bind the SAME single request-scoped `eval_instant` parameter (computed once from the injectable `Clock`), so they cannot disagree within the page query — and, unlike SQL `now()`, the shared bound instant makes the boundary deterministically testable. The count summary is a separate statement but binds the SAME `eval_instant` value; a row whose `privacy_flip_scheduled_at` literally crosses the wall-clock between the page query and the summary query is moot because both use the request's fixed `eval_instant`, so they stay consistent for the whole request (the only residual drift is across DISTINCT requests, which is correct monitor behavior).
