...
- Response carries: `userId`, `username`, `displayName`, `bio` (nullable), `followerCount`, `followingCount`, `isSelf`, `followedByViewer`, `isPremium`, and the **self-only** field `isPrivate` (private-profile flag honoring the 72h privacy-flip grace window). camelCase wire keys, matching the repo's mixed-case timeline-DTO convention. There is intentionally **no `suspendedUntil`** — suspension is a session-terminating state, so the countdown is surfaced on the auth-rejection response, not this read (design D2 + `follow-up` issue).
...
### New Capabilities
- `user-profile-read`: the `GET /api/v1/users/{user_id}` profile-read endpoint — single-user profile projection (handle/display-name/bio/counts/follow-state/premium-flag + self-only privacy state), shadow-ban-safe via `visible_users`, bidirectional-block-aware (leak-safe 404), with the own-content raw-`users` exception for self reads. (Suspension state is deliberately NOT carried — see design D2.)
