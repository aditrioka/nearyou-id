...
- [x] 7.5 Filed `follow-up`+`backend` issues for the two deferred sweeps: WebAuthn-challenge cleanup → #365; moderation/reports 1-year archival → #366.
- [x] 7.6 Archive done: `openspec/specs/scheduled-retention-cleanup/spec.md` Purpose filled (no TBD); MODIFIED `fcm-token-registration` (+ a direct one-line canonical fix of the §33 stale "deferred" aside the delta under-scoped) and RENAMED+MODIFIED `in-app-notifications` (`enforcement deferred`→`shipped`) both merged cleanly; all 3 affected specs pass `openspec validate --type spec --strict`; no `TBD - created by archiving` remains.
