...
*Reconciliation (the name is double-booked in the canonical docs — all stale references must be fixed in this PR):* `/internal/cleanup` was the original "consolidated cleanup endpoint" name and is referenced for **two** different intents:
- The **hard-delete** worker — `docs/06` §386 ("Cloud Scheduler calls `/internal/cleanup` daily … reads `deletion_requests`") + §415, and `docs/08` Phase 3.5 item 6. That capability actually shipped as `/internal/account-hard-delete-worker` on its own subtree, so these references are already stale and would be made *actively misleading* by this change taking the name. **Fix (tasks §7): repoint those `docs/06`/`docs/08` hard-delete references to `/internal/account-hard-delete-worker`.**
- The **retention sweeps** — `docs/05` §§112/582/1120 (refresh/notifications/FCM) and `docs/04` §463/§482 (which still label the FCM stale sweep "DESIGN"). This change implements exactly these, so `/internal/cleanup` is the correct, canonical name to adopt. **Fix (tasks §7): annotate `docs/05` §§112/582/1120 + `docs/04` §463/§482 as shipped by this change.**

So `/internal/cleanup` is unused at runtime and free; this change takes it for the retention sweeps and reconciles every stale doc reference in the same PR. *Alternative considered:* a generic name like `/internal/retention-worker`; rejected because it would orphan the `docs/05`/`docs/04` retention references and still leave the `docs/06`/`docs/08` hard-delete mis-references unfixed.
...
Two further **explicit amendments of `docs/05` §112** (stated here so neither is a silent divergence): (1) the refresh-token sweep merges §112's two separate queries into a single idempotent `DELETE … WHERE expires_at < NOW() - INTERVAL '1 day' OR last_used_at < NOW() - INTERVAL '90 days'`; (2) it deletes **revoked** rows too (§112's wording omits `revoked_at`, but a revoked row past either threshold is exactly as eligible for reclamation). Note `last_used_at` is nullable — a never-used token has `last_used_at = NULL`, so the `last_used_at` predicate is UNKNOWN for it and the `expires_at` branch is what reaps it once expired; this is correct and is locked by a spec scenario. Both amendments are folded into the `docs/05` §112 "shipped" annotation in tasks §7.
...
- **Large first run on an aged table holds locks / spikes load** → The notifications (`notifications_user_all_idx`) and FCM (`user_fcm_tokens_last_seen_idx`) sweeps are index-served. The refresh-token sweep is **not fully** index-served: `refresh_tokens_expires_idx` is partial (`WHERE revoked_at IS NULL`), so it serves only the non-revoked `expires_at` branch — the revoked-row and `last_used_at` branches seq-scan (verify with `EXPLAIN` at apply-time; don't expect a pure index plan). All sweeps run on the bounded DB dispatcher, so none can exhaust the Hikari pool regardless of plan. Pre-launch tables are tiny; if a future run ever needs batching (`DELETE … LIMIT`/`ctid` chunking) or a covering index for the refresh sweep, that is a follow-up — noted, not built now (no silent cap).
