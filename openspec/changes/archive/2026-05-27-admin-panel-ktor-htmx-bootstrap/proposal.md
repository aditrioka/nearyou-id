...
- **`docs/07-Operations.md` Status callout** currently says "no `/admin/*` routes, no admin UI, no Pebble templates" — after this change squash-merges, three of those four claims become false. [`tasks.md`](tasks.md) Section 7 now includes a step to amend the callout to reflect the route + Pebble + HTMX scaffold landing (auth gate + business features remain DESIGN).
