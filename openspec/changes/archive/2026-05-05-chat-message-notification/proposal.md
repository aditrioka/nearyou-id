...
- **Rollback semantics**: emit failure rolls back the entire chat send (matches [`in-app-notifications/spec.md:86-88`](../../specs/in-app-notifications/spec.md) "Emit failure rolls back primary write"). The resulting 5xx surface for "recipient hard-deleted between auth and emit" is acceptable per the design's risk discussion (the race is structurally near-impossible under the canonical schema's `ON DELETE RESTRICT` semantics on `conversation_participants.user_id`; reaching the FK violation requires a non-canonical worker shape).
