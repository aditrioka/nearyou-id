...
| **FCM push spam.** Same shape as above — five messages = five FCM pushes. iOS may throttle; Android shows five notifications. | iOS handles this naturally via NSE-side grouping (per [`docs/04-Architecture.md`](../../../docs/04-Architecture.md) iOS alert + mutable-content path). Android shows individual notifications, which OS-level summarization handles. Backend doesn't need to coalesce at the FCM layer. |
