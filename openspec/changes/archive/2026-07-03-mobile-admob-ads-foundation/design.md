...
- **Both** the Android and iOS `AdProvider` actuals implemented (full platform parity, Decision D7) — Android Gradle SDK (Test-Lab-verifiable on Google test units), iOS cocoapods cinterop (local + simulator/device verify, since iOS is not CI-gated).
...
### D7 — Both platform actuals in-scope; iOS via the KMP cocoapods cinterop (GoogleSignIn precedent)
Operator decision (2026-06-26): ship full platform parity — the iOS `AdProvider` actual is implemented this slice, not deferred. Mechanism, grounded in the existing iOS-vendor precedents:
- **Android actual** — Google Mobile Ads + UMP SDK as a Gradle dependency in `:infra:admob` (Test-Lab-verifiable on Google test units).
- **iOS actual** — cinterop to the `Google-Mobile-Ads-SDK` + `GoogleUserMessagingPlatform` CocoaPods via the **KMP `native.cocoapods` plugin**. Google publishes **no** official KMP Ads SDK, so unlike `:infra:revenuecat` (`purchases-kmp`) / `:infra:sentry` (`sentry-kotlin-multiplatform`) — where a vendor KMP SDK owns the cinterop klib and `mobile/app` only links the Pod binary `linkOnly` — AdMob mirrors the **`GoogleSignIn` precedent**: we own the binding by declaring the Pods so the cocoapods plugin generates the cinterop. To keep invariant #16, the Pods + the generated cinterop + the iOS actual live in **`:infra:admob`** (apply `native.cocoapods` to that module), and `mobile/app`'s cocoapods block adds the same Pods `linkOnly = true` so the final `ComposeApp` framework links the binaries (the Sentry/RevenueCat `linkOnly` precedent). `:mobile:app` sees only the interface.
- **Verification reality** — iOS is **never CI-gated** (docs/13). The iOS actual is verified locally: `linkDebugFrameworkIosSimulatorArm64` after touching `iosMain` (docs/11 §2.5) + a simulator/device ad-render smoke. ObjC **category** members (e.g. on `GADRequest`/UMP types) need explicit `import platform.<Framework>.<symbol>` — the §2.5 compile-only gotcha Linux CI can't catch.
- *Alternative — defer iOS to a follow-up: rejected by the operator* (parity wanted now). *Alternative — adopt the third-party KMP wrapper `LexiLabs-App/basic-ads`: held as a documented fallback* (Open Questions) if the two-cocoapods-module `linkOnly` coordination proves intractable; primary path is hand-rolled cinterop, consistent with the GoogleSignIn precedent + guiding principle 4.
...
Layers spanned: **backend** (the `ads-config` read endpoint) + **mobile** (the ad-serving surface, **both Android and iOS** — full platform parity, D7). **admin** = none new (the `ads_enabled` flag is operated through the existing `admin-feature-flags` editor + Remote Config — declared, not silently omitted). Read-paths: `GET /api/v1/config/ads`, the existing consent read, the existing entitlement read. The only deferrals are *future placements* (interstitials, profile-banner, chat-list) + mediation — each an explicit `mobile-ads` requirement (positive + negative-guard) with a tracked `follow-up`, not bare prose. No layer is deferred.

## Risks / Trade-offs

- **iOS cinterop is the heaviest part + not CI-gated (D7)** → mitigated by the GoogleSignIn/Sentry cocoapods precedents; verified locally via `linkDebugFrameworkIosSimulatorArm64` + simulator/device smoke; ObjC category imports handled per §2.5. If the two-cocoapods-module `linkOnly` coordination proves intractable, the `basic-ads` KMP wrapper is the documented fallback (Open Questions). Gated OFF, so a late iOS-specific issue can't ship a broken live experience.
...
- **iOS cocoapods two-module coordination** — confirm at apply that declaring the Ads/UMP Pods in `:infra:admob` (for the cinterop klib) + `linkOnly` in `mobile/app` links cleanly into the `ComposeApp` framework (the Sentry/RevenueCat `linkOnly` pattern uses a vendor KMP klib; ours generates the klib from the Pod, the GoogleSignIn pattern — these two precedents differ, so validate the combination early). If intractable, fall back to the `basic-ads` KMP wrapper.
