...
`post-replies` (V8) shipped without the canonical Free-tier 20/day reply cap that anchors the freemium upsell in [`docs/01-Business.md`](../../../docs/01-Business.md), [`docs/02-Product.md`](../../../docs/02-Product.md), and the Layer 2 table in [`docs/05-Implementation.md:1740`](../../../docs/05-Implementation.md). The cap was deferred because the foundational rate-limit infrastructure — Phase 1 item 24 in [`docs/08-Roadmap-Risk.md`](../../../docs/08-Roadmap-Risk.md) — hadn't shipped yet. V8 self-documented the gap at [`docs/05-Implementation.md`](../../../docs/05-Implementation.md) § Post Replies Schema (V8 callout): "rate limiting remain[s] deferred." Phase 2.3 in [`docs/08-Roadmap-Risk.md`](../../../docs/08-Roadmap-Risk.md) explicitly calls this out: "Reply rate limit (20/day Free) remains deferred — separate future change."
...
1. **No burst limit.** Likes have a 500/hour burst clause (per [`docs/02-Product.md`](../../../docs/02-Product.md): "Both tiers cap 500/hour burst"). Replies do NOT — the canonical line at [`docs/02-Product.md`](../../../docs/02-Product.md) is just "Free 20/day, Premium unlimited" with no burst clause. So this change ships a daily-only limiter, no rolling-hour limiter.
...
- **NEW: `premium_reply_cap_override` Firebase Remote Config flag** — when set to a positive integer N, overrides the 20/day Free cap server-side without a mobile release. Mirrors the `premium_like_cap_override` flag definition canonically described at [`docs/05-Implementation.md:1416`](../../../docs/05-Implementation.md) (with secondary cross-reference in `08-Roadmap-Risk.md` Decision 28). Unset / malformed / ≤ 0 / oversized (> 10,000) / SDK error → fall back to default 20. Premium users are unaffected (skip the daily limiter entirely).
- **DELETE / GET on `/replies` are NOT rate-limited.** DELETE preserves the V8 author-only idempotent-204 soft-delete contract (rate-limiting it would let attackers prevent users from deleting their own replies). GET read-side throttling lives at the timeline session/hourly layer per [`docs/05-Implementation.md`](../../../docs/05-Implementation.md); per-endpoint read limiters are deferred per the like-rate-limit precedent (which also did not rate-limit GET `/likes`). Both decisions are codified once in `specs/post-replies/spec.md` § DELETE / GET requirements.
