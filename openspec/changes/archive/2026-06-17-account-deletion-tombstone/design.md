...
|---|---|---|
| `users` (self) | tombstone | `UPDATE`: set `deleted_at`; NULL nullable PII `bio, google_id_hash, apple_id_hash, device_fingerprint_hash, email`; set `NOT NULL` `display_name = 'Akun Dihapus'` + `date_of_birth = DATE '1900-01-01'` (sentinel, satisfies the `>= 18y` CHECK); reset `apple_relay_email = FALSE`; `username = 'deleted_user_' || left(id::text, 8)` (D9) |
...
### D9 — NOT-NULL PII columns tombstone to a placeholder/sentinel, not NULL (apply-time)
Surfaced during `/opsx:apply`: the shipped V2 `users` schema makes `display_name VARCHAR(50) NOT NULL` and `date_of_birth DATE NOT NULL` (+ a `>= 18y` CHECK), so docs/06's "NULL display_name / date_of_birth" is unimplementable as literal NULL. Canonical tombstone handling for a `NOT NULL` PII column is a placeholder/sentinel (you cannot violate `NOT NULL`): `display_name = 'Akun Dihapus'` (which doubles as the docs/06 user-facing placeholder → every server-rendered surface shows it with **no client logic**, the simplest cross-surface render mechanism — chosen over `display_name = ''` + per-client "deleted_user_" detection) and `date_of_birth = DATE '1900-01-01'` (erases the real DOB, satisfies the CHECK). Nullable PII columns still NULL. This is a docs/06 reconciliation (the PII-list wording assumed all columns nullable) — folded into task 7.1's doc-amend follow-up. The `display_name` write rides the same `// @allow-username-write: deletion` annotation site discipline.

### D10 — Tombstone-surfacing is scoped to POST authorship; reply/contributor surfaces keep the existing visible_users filter (apply-time)
Surfaced during `/opsx:apply`: operator Q1 ("tombstoned authors' feed content remains anonymized") is implemented for **post authorship** — `visible_posts` (V28) + the author-identity join switched from `visible_users` to raw `users` in the three timelines + single-post-read, so a tombstoned author's POSTS surface anonymized. The **reply list + reply_count** already filter contributors through `visible_users` (`LEFT JOIN visible_users ... (vu.id IS NOT NULL OR author=:viewer)` for the list; `JOIN visible_users` for the counter) — which excludes a tombstoned replier exactly as it excludes a shadow-banned one. Making replies-by-tombstoned-authors *surface* anonymized would require a new "users-minus-shadow-banned" join across ~5 queries + BlockExclusionJoinRule navigation — a large, risky cross-cutting expansion. **Decision:** bound the surfacing to POST authorship (the primary docs/06 feed case + the explicit Q1 concern); a tombstoned **replier's** reply is RETAINED in the DB (the row persists — RESTRICT FK guard + docs/06 "replies retained") but is filtered from the public reply list + count by the unchanged `visible_users` contributor-filter, identical to a shadow-banned replier. Retained **likes** stay in `post_likes` (docs/06 "count remains accurate"). The fuller "surface tombstoned repliers anonymized" behavior is a follow-up if the operator wants it. This bounds blast radius while delivering the core compliance + Q1 behavior.
...
1. `V27__deletion_requests.sql` — `CREATE TABLE deletion_requests` + the two partial indexes (verbatim `docs/05:603-624`) + `CREATE TABLE deletion_log`.
2. `V28__visible_posts_surface_tombstoned_authors.sql` — `CREATE OR REPLACE VIEW visible_posts` dropping **only** the author-side `u.deleted_at IS NULL` (keeping `p.deleted_at IS NULL`, `is_auto_hidden = FALSE`, `is_shadow_banned = FALSE`). `visible_users` is **NOT** touched. `CREATE OR REPLACE`, no data backfill.
...
- **Renumber note:** at rebase the two migrations landed as V27 (`deletion_requests`) + V28 (`visible_posts`), shifted above the merged V24–V26 cluster (#323/#328/#325) (R5).
