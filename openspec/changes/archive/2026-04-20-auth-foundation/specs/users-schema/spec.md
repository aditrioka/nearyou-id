...
The `users` table MUST include the constraint `CHECK (date_of_birth <= (CURRENT_DATE - INTERVAL '18 years'))` as a defense-in-depth backstop behind the (later) application-layer age gate.
