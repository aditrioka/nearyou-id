...
`ImageUploadApiClient.upload(bytes, mime): ImageUploadResult` issues multipart `POST /api/v1/images` via Ktor `MultiPartFormDataContent` on the shared `HttpClient` (never an ad-hoc client; Bearer + 401 refresh owned by the Auth plugin). `ImageUploadRepository` maps the shipped backend's exact status + `error.code` (`ImageRoutes.kt`) to a sealed `ImageUploadOutcome`: `Success(imageId, deliveryUrl)`, `PremiumRequired` (403 `premium_required`), `FeatureDisabled` (403 `image_upload_disabled`), `QuotaExceeded` (429 `image_upload_quota_exceeded`), `Throttled` (429 `image_upload_throttled`), `ModerationRejected` (422 `image_rejected`), `TooLarge` (413 `image_too_large`), `Unavailable` (503 `image_upload_unavailable` — Vision/Cloudflare unconfigured/down, the dark-launch default), `Network` (transport). The two 403s and two 429s are disambiguated by `error.code`, not status. `CancellationException` is rethrown, never mapped to `Network`. ViewModels talk to the repository only.
...
Add `imageUrl: String?` (bare camelCase wire — per the `TimelineRoutes.kt` mixed-case precedent; the spec snake_case examples are stale) to `NearbyPostDto`, `FollowingPostDto`, `GlobalPostDto`, the single-post-read DTO, and the post-detail payload. The read queries select `image_id` from `visible_posts` — **already projected** (V28 `visible_posts = SELECT p.* FROM posts p JOIN users u …`; `posts.image_id` exists since V4), so **no Flyway migration and no view recreate** (the two-arm-UNION reads must project `image_id` in both the `visible_posts` arm and the viewer-scoped self-arm — see tasks 2.2/2.3). A **shared** server-side builder (reused by both the upload and read paths — one source of truth) constructs the `<deliveryBaseUrl>/<accountHash>/<image_id>/public` URL — the exact 4-segment shape the shipped `CloudflareImageStore` emits — from the env-aware `deliveryBaseUrl` (`Application.kt`: `img.nearyou.id` prod / `img-staging.nearyou.id` staging), the secret-sourced (non-sensitive) `accountHash`, and the `public` variant, keeping Cloudflare specifics off the client; `imageUrl` is null when `posts.image_id` is null. Because images ride `visible_posts`, they inherit the existing shadow-ban + block-exclusion joins — **no new invariant surface**. A Phase-1 task-0 runtime check confirms `visible_posts.image_id` is live before code relies on it (mirrors the external-data sanity-check convention).
...
Render with **Coil 3 (KMP)** `AsyncImage` in the shared `PostCard` + post detail. Propose-time re-check (verified 2026-06-20): Coil 3.5.x is the production-stable CMP async image loader with full iOS support and a **Ktor network backend** (reuses the project's existing Ktor stack — no new networking lib) — per coil-kt.github.io + the Coil 3.0 release notes. Pin in `gradle/libs.versions.toml`. **Apply-time re-check outcome (2026-06-20):** Coil **3.5.0**'s iOS klib is built with Kotlin **2.4.0** (`compiler_version=2.4.0`), which the project's Kotlin **2.3.21** K/N cannot consume (`linkDebugFrameworkIosSimulatorArm64` fails — a gap the Android/JVM gate cannot catch). Pinned **Coil 3.4.0** instead (klib `compiler_version=2.3.10`, ABI-compatible) — a version correction within the same chosen substrate, NOT a substrate switch; the alternative (bump Kotlin 2.3.21 → 2.4.0) was rejected as a large cascading change out of this change's scope. Verified: full Android tests + `linkDebugFrameworkIosSimulatorArm64` + lint green on 3.4.0. Delivery rules (docs/02 §6): no thumbnail preload during timeline scroll (load on on-screen render only), aspect-ratio placeholder, immutable cache headers (already server-set), graceful failure → placeholder (no error chrome).
...
- **Delivery-URL shape (launch-confirmed)**: the shipped upload builder is `<deliveryBaseUrl>/<accountHash>/<image_id>/public` (`CloudflareImageStore.kt`), but the *final* CF Images path is flagged in `docs/02 §6` + `CloudflareImageStore` as pending Pre-Phase-1 verification (possible `/cdn-cgi/imagedelivery/` prefix or Custom-Image-URLs; `/public` vs a named variant). The read-path **MUST reuse the same shared builder** rather than re-derive the string, so whatever the launch-confirmed shape, write and read stay identical. Does not block the spec — it pins the dependency, not a divergent format.
