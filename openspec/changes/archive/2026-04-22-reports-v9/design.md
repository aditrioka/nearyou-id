...
V9 is the moderation-activation change. V4 `posts.is_auto_hidden`, V7 `post_likes` (no auto-hide column; post-level flag is sufficient), and V8 `post_replies.is_auto_hidden` have each reserved the fields without a code path to flip them. Phase 2 item 4 in `docs/08-Roadmap-Risk.md` calls for the Report feature with a 3-unique-reporters auto-hide trigger. The canonical schema, reason enum, trigger enum, and auto-hide semantics are already decided in `docs/05-Implementation.md` §742–816 and §737.
