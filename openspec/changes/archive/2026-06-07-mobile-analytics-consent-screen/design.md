...
The mobile onboarding chain is shipped end-to-end except one screen. As of Mobile #4 (`mobile-age-gate-screen`, archived 2026-05-31), a new user flows `SignInScreen` → `GoogleSignInClient` ceremony → `404 user_not_found` → `AgeGateScreen` (DOB + 18+ gate) → `POST /api/v1/auth/signup` → `SecureTokenStore` → **the `AgeGateScreen` `onSignedUp` callback (wired in `AppEntryProvider.kt`) `replaceAll(HomeRoute)`s straight to `HomeScreen`** (`RootRouterScreen` handles only cold-start token-presence routing, not this post-signup terminus). Location permission is an in-screen gate at `HomeScreen` (Mobile #6, `mobile-location-permission-flow`), not a routed onboarding step. [`docs/03-UX-Design.md`](../../../docs/03-UX-Design.md) § User Onboarding Flow sequences the **Analytics & Tracking Consent screen** between the age gate and location permission — i.e., between signup-success and `HomeScreen`. Mobile #4's design Non-Goals explicitly deferred it to "a later change."
...
The signup-`201` terminus is modified to route to `ConsentScreen` (which routes to `HomeScreen` on submit) instead of straight to `HomeScreen`. Concretely, the edit is in `AppEntryProvider.kt` — the `AgeGateScreen(onSignedUp = …)` callback that today does `backStack.replaceAll(HomeRoute)` now `replaceAll(ConsentRoute)`, and a new `entry<ConsentRoute> { ConsentScreen(onDone = { backStack.replaceAll(HomeRoute) }) }` is added — NOT in `RootRouterScreen` (which only does cold-start token routing). The **returning-user sign-in terminus is unchanged** (`/signin 200` → `HomeScreen` directly).
...
- **OQ1 — `consent_skip` affordance visibility — RESOLVED (only-after-failure).** Whether the skip is always visible or only after a failed PATCH. Resolved to **only-after-failure** (a single "Simpan & lanjutkan" CTA on the happy path; skip appears only on a `Retryable` outcome, so consent does not read as optional). This is now a hard, scenario-backed requirement in the `mobile-analytics-consent` spec ("A failed persist offers a non-trapping proceed-to-Home; the happy path shows no skip"), not an open question — recorded here only as the resolution trail.
