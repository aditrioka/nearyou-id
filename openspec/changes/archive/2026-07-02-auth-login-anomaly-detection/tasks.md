...
- [x] 6.4 Operator (human-required, surfaced in Preflight): create the Cloud Scheduler job invoking `POST /internal/login-anomaly-check` with an OIDC identity for the internal audience (mirroring `/internal/cleanup`). Not a code task — tracked separately; does NOT block the squash-merge. (DONE 2026-07-02: `nearyou-login-anomaly-check-staging` created in `asia-southeast1`, hourly `0 * * * *` Asia/Jakarta, OIDC SA `unban-scheduler-staging@…` + audience `https://api-staging.nearyou.id`, mirroring the privacy-flip job. OIDC binding verified via manual run — authenticated and reached the service; 404 until this PR's squash-merge deploys the route (the branch deploy was clobbered by a main auto-deploy; self-heals on merge). Evidence: PR #429 comment.)
