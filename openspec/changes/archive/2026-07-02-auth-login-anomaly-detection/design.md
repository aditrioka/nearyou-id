...
Record each detection as a `moderation_queue` row (`target_type='user'`, `target_id=user_id`, `trigger='anomaly_detection'`, `status='pending'`, non-PII `notes`). Both enum values are valid in the current authoritative CHECK (`V36__moderation_queue_area_spam_trigger.sql`, which re-stated the V9 `moderation_queue_trigger_check` and kept `anomaly_detection`) → **no Flyway migration**, so the footprint is pure `:backend:ktor` Kotlin and parallel-merge-safe against the migration-bumping in-flight changes (referral, data-export). Idempotency via the existing `UNIQUE(target_type, target_id, trigger)` → `ON CONFLICT DO NOTHING`.

**Forward-compat note on the shared `anomaly_detection` trigger.** `anomaly_detection` is a *generic* per-user "this account is anomalous" trigger, not leg-specific — the V36 header comment earmarks it for the future Phase 4 #17 rolling-30-day-baseline mechanism, but the value's name and the `moderation_queue` anomaly surface are general. This change writes it for the login-source-spread leg; a future Phase 4 #17 leg would write the same trigger. The `UNIQUE(target_type, target_id, trigger)` therefore means **one open `anomaly_detection` row per user across all anomaly legs** (the `notes` disambiguate which leg fired first). Cross-leg note accumulation / multi-reason representation is explicitly a *future Phase-4-#17 concern*, not this change's — this change owns only the login-spread leg's write and the single-row idempotency it implies.
...
- **Sweep cost on a large `login_events`.** The windowed `GROUP BY user_id HAVING COUNT(DISTINCT ip_subnet_24) > N` over the trailing hour is served by the `(user_id, occurred_at DESC)` index and bounded by the 90-day retention; the trailing-1h filter keeps the scanned set small. → *Mitigation:* parameterized window bound; no new index needed. The endpoint is **OIDC-gated + Cloud-Scheduler-only** (no user-reachable trigger), so an attacker cannot force repeated expensive sweeps to amplify DB load.
