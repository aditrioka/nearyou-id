...
- **NEW** `TextModerator` orchestrator: combines loader + two matcher instances (one per list), exposes `moderate(content: String): ModerationVerdict` returning either `Allow`, `Reject(reason: ProfanityHit)`, or `Flag(uuIteHit: UuIteKeywordHit)`. Decision rules per [`docs/06-Security-Privacy.md–159`](../../../docs/06-Security-Privacy.md):
...
- New `:infra:remote-config` Gradle module scaffolded under `infra/remote-config/` (resolves the "DECISION NEEDED" status flagged for this module in [`openspec/project.md`](../../project.md) § Module Structure — this change is the first concrete platform-wide-config consumer; the `no vendor SDK import outside :infra:*` rule mandates the Firebase Remote Config Admin SDK live in an `:infra:*` module). Owns `RemoteConfigClient`, initializes a named FirebaseApp `"nearyou-rc"` from `secretKey(env, "firebase-admin-sa")`. Bumps the Firebase Admin SDK pin in [`gradle/libs.versions.toml`](../../../gradle/libs.versions.toml) from `9.5.0` (the pin shared with `:infra:fcm` since `fcm-push-dispatch`) to `9.8.0` — minor version bump (semver-additive) needed because the **Server template API** (`getServerTemplate()` + `ServerConfig`) was added in SDK 9.7.0. Server templates are Firebase's official server-side guidance: backend downloads the complete template + evaluates conditions locally. Client templates (the only option pre-9.7.0, which `getTemplate()` returns) are designed for mobile/web SDKs that rely on Firebase to pre-evaluate. `:infra:fcm` is unaffected by the SDK bump (no new APIs touched).
...
- `FOLLOW_UPS.md` entry: amend `docs/06-Security-Privacy.md` to align "1 match" phrasing with the Remote Config `moderation_match_threshold` (default 3) — see design.md D5.
