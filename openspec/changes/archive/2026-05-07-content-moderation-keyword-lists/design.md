...
**Firebase Admin SDK version bump (9.5.0 → 9.8.0).** The pin in [`gradle/libs.versions.toml`](../../../gradle/libs.versions.toml) (added by `fcm-push-dispatch`) is bumped from `9.5.0` to `9.8.0` (latest at change-authoring time, 25 February 2026). The bump is needed because the **Server template API** — `FirebaseRemoteConfig.getServerTemplate(KeysAndValues defaultConfig)` returning a `ServerTemplate` whose `evaluate()` produces a `ServerConfig` — was added in SDK 9.7.0 (24 September 2025). Server templates are Firebase's official guidance for backend services: the SDK downloads the complete template (parameters + conditions) and evaluates conditions locally based on a caller-supplied context. By contrast, the historical `getTemplate()` method (the only option pre-9.7.0) returns a Client template, which is designed for mobile/web SDKs that rely on Firebase to pre-evaluate conditions before delivering values to the client. For our backend use case, the server-side evaluation model matches our needs exactly (we hold the wordlist locally, run the matcher locally). The bump is semver-additive (9.x.y minor + patch); `:infra:fcm` is unaffected because it doesn't touch any of the SDK's Remote Config APIs. Pre-Phase 1 §36 reservation in the **Server** tab of Firebase Console becomes load-bearing under this design — the same parameters under the **Client** tab would not be visible to the SDK.
