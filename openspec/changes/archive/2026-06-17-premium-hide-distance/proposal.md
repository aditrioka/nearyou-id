...
- New endpoints `PATCH /api/v1/user/hide-distance` (JWT-required, boolean body — mirrors the `PATCH /api/v1/user/consent` precedent) and `GET /api/v1/user/hide-distance` (returns `{hideDistance, premium}` so the mobile Settings toggle reflects state + gates on tier). `hide_distance_opt_in` is a fresh column — **not** `username` and **not** `private_profile_opt_in` — so it is outside both the username-write and privacy-flag-write Detekt allowlists (no `@allow-*` annotation; no new lint invariant).
