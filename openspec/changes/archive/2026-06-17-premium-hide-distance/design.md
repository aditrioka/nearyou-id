...
**D4 — Symmetric rule evaluation.** For each Nearby row, hide iff `authorEffectiveHide OR viewerEffectiveHide`. The **author's** `hide_distance_opt_in` + `subscription_status` are projected as a single boolean from the already-joined author row (`visible_users`/self-arm `users`) — no new join. The **viewer's** effective-hide is read from the **auth principal** (`UserPrincipal.hideDistanceOptIn`, loaded at the auth boundary by the existing `users.findById` alongside `subscriptionStatus`) and applied to every row — when the viewer hides, all rows drop the number. **This is deliberately NOT a per-request `users` SELECT in the timeline path**: the `timeline-read-rate-limit` capability asserts "zero `users` SELECTs in the timeline handler" (tier is read from the principal, not a fresh query), and the viewer hide flag rides the same auth-time load to preserve that invariant. (Apply-time refinement: an earlier design sketched a per-request indexed-PK read; that broke the zero-users-SELECT scenario, so the flag was promoted onto `UserPrincipal`, mirroring the shipped `subscriptionStatus` precedent — `infra/.../UserRow` documents exactly this load-at-auth rationale.) Both Nearby arms (visible + self, per `shadow-ban-feed-self-visibility`) honor the rule identically; on the self arm author==viewer so the two terms coincide. The radius FILTER and `(created_at DESC, id DESC)` ordering are untouched — no ordering leak. **The Settings GET (`GET /api/v1/user/hide-distance`) DOES read the flag from the DB** — that is a separate endpoint, not the timeline path, so it does not touch the zero-users-SELECT invariant.
