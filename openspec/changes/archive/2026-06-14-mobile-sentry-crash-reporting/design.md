...
A vendor-free `interface CrashReporter` in `:infra:sentry` commonMain (`init(config)`, `captureException(t, context)`, `setUser(id)`, `clearUser()`, `addBreadcrumb(crumb)`, `close()`), with a `SentryCrashReporter` implementation bound by Koin. **Apply-phase refinement (verified 2026-06-14):** the Sentry KMP facade (`io.sentry.kotlin.multiplatform.*`) is itself multiplatform, so `SentryCrashReporter` is a single **commonMain** impl — no androidMain/iosMain expect-actual split is needed (mirrors `:infra:supabase-realtime`'s single commonMain `SupabaseChatRealtimeSubscriber`); confirmed by the iOS klib (`compileKotlinIosSimulatorArm64`) + Android compile both passing. **Why over docs/04's `expect object SentryProvider`:** docs/11 §2.5 mandates interface + Koin platform binding (expect class/object is still Beta; reserve expect/actual for top-level functions). The interface also makes the Open Decision #17 fallback (dual native pipeline) a drop-in impl swap. *Alternative considered:* `expect object` per the docs/04 sketch — rejected as a Pattern-Registry violation and harder to fake in tests.
