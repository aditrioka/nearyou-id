...
- **Composer (frame 6)**: add a coral location chip (`NearYouColors` `locationPinContainer`/`onLocationPinContainer`/`locationPin` tokens + `ic_post_location` glyph) with a static label string (new: `post_create_location_chip`) — NOT a live city name (see design.md Decision 2); add the privacy note (new string `post_create_privacy_note` "Lokasi kamu disamarkan hingga ±5 km sebelum tampil ke pengguna lain" — the board's own softer "hingga" phrasing from its onboarding privacy frame, chosen over frame 6's "(±5 km)" so the copy cannot be read as a precise obfuscation-magnitude claim (operator decision at proposal review) — + new bundled `verified_user` glyph `ic_privacy_shield`); move the existing `N/280` counter to the mockup's bottom composer-bar position (right-aligned). NO attachment toolbar (media is Month 6 roadmap — the mockup's image/camera buttons are explicitly out of scope). The frame-6 avatar beside the input is likewise deferred (no profile/avatar capability is shipped; it travels with the chrome-restyle follow-up).
...
- `shared-resources`: the brand-logo requirement changes from "blue glyph on white background / white glyph on blue background, same viewBox as source SVGs" to transparent-background scheme-primary glyphs (light `#1E4FD6`, dark `#B7C4FF`) on a cropped 46×46 canvas; the hex-grep scenarios change accordingly, and the presence scenario drops the stale Moko-migration-era clause (the `moko-resources/images/` directory check — an obsolete one-time migration guard). The bundled Material-glyph set gains the privacy-shield (`verified_user`) drawable.
...
- `mobile-post-creation`: the composer screen requirement gains the location chip + privacy note and pins the counter to the bottom composer bar; the composer-strings requirement gains 2 new keys (`SharedStringsCatalogTest` tracked-accessor assertion 86 → 88 — the test tracks its referenced-accessor list, not the raw catalog size; drawables assertion 12 → 13); the test-coverage requirement gains the chip / privacy-note / no-toolbar / counter-placement coverage.
...
- **Tests**: `SharedStringsCatalogTest` (tracked-accessor assertion 86→88; the catalog itself goes 100→102 entries — the pre-existing 14-key tracking drift is a separate follow-up issue), `SharedDrawablesCatalogTest` (12→13), `SignInScreenTest` (title-node assertion replaced — note its hardcoded `TITLE` constant, not a key reference), `PostCreationScreenTest` (chip + privacy-note presence + counter bottom-bar bounds; CTA scenarios unchanged).
