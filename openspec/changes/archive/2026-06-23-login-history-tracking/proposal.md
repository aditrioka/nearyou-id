...
- **New durable login-history store** — a `login_events` table (Flyway **V35**) recording each authenticated sign-in and refresh: `user_id`, `occurred_at`, `event_type`, `ip` (+ generated `ip_subnet_24`), `device_fingerprint_hash`, `identifier_hash`. Written best-effort from the sign-in and refresh **routes** (where `call.clientIp` and the request body live), via a thin `LoginEventRecorder`; `RefreshTokenService` is untouched.
...
- **Activate the 5 deferred activity-gate legs** — flip the `referral-grant-worker` negative-guard requirement to positive: invitee ≥ 3 distinct login-days; ≥ 5 app sessions (server-side sessionization); and the anti-collision checks against the inviter's history. **Voiding is device-fingerprint-based** (the invitee's fingerprint in the inviter's 90-day history, and the invitee's identifier seen on the inviter's device); the **IP /24 is recorded but non-voiding** — an operator decision, because Indonesian carrier-grade NAT makes a shared /24 common among unrelated users, so subnet-alone voiding would terminally false-void legitimate same-carrier referrals. An anti-collision hit **voids** the ticket; an engagement shortfall leaves it `pending_activity`.
...
- `referral-ticket-creation`: the "richer anti-collision checks are out of scope" requirement is reconciled — its now-stale claim that the downstream legs "remain unimplemented by the worker / deferred" is corrected (the recently-seen-identifier leg is now evaluated by the activity-check worker, and the IP /24 is now recorded by it as a non-voiding signal); the signup-time scope is unchanged (only the point-in-time fingerprint check runs at signup).

## Impact

- **Schema**: new `login_events` table (V35); no changes to existing tables.
