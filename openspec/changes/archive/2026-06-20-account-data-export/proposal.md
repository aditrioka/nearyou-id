...
- **NEW** `data_export_requests` table (migration **V30**) — status state machine `pending → processing → ready → (expired | failed)`, R2 object key, 24h download expiry, a **one-active-per-user partial unique index**, and a worker-scan partial index (both `WHERE` clauses `NOW()`-free per the partial-index invariant).
- **NEW** OIDC-authed internal worker `POST /internal/data-export-worker` (Cloud Scheduler) — gathers the scope matrix, serializes to a ZIP of **JSON + CSV** files (per the canonical Format column) + a manifest, uploads to R2, issues a **24h signed URL**, sets `ready`, emits the `data_export_ready` notification, sends the Resend email. **Fail-soft** when R2/Resend creds are unset (established `:infra:*` posture). Mirrors the shipped suspension-unban / privacy-flip / account-hard-delete worker pattern.
- **NEW** `:infra:r2` module — vendor-neutral `ObjectStore` + signed-URL contract over Cloudflare R2 (S3-compatible). First **general-purpose** object-storage / signed-download substrate on the backend classpath (`:infra:cloudflare-images` writes images via its own `ImageStore` but offers no general object store / signed GET URL); activates the `:infra:r2` slot docs/04 pre-named (DESIGN).
...
- **Migration V30** `data_export_requests` (+ partial indexes). **No change** to the `notifications` CHECK.
- **New endpoints:** `POST` / `GET /api/v1/account/export`; `POST /internal/data-export-worker` (OIDC, `internal-endpoint-auth`).
- **Secrets** via `secretKey(env, name)`: R2 (account/access-key/secret/bucket) + Resend (API key) + `export-peer-hash-secret` (peer-id HMAC) slots, staging + prod.
