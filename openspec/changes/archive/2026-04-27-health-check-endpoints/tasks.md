...
- [x] 11.5 Negative test — Cloud Run startup-probe revision-promotion gate verified end-to-end. **NOT** by pausing Upstash (operator-driven via console); instead via `gcloud`-automated approach: created a temporary `staging-redis-url-broken-test` secret slot pointing at `redis://127.0.0.1:1` (unreachable), then `gcloud run services update --update-secrets="REDIS_URL=staging-redis-url-broken-test:latest"`. Cloud Run created the new revision (`nearyou-backend-staging-00051-bxt`) but the startup probe (`GET /health/ready`) failed because `redis.ok=false` → `failureThreshold=3` × `periodSeconds=10` = ~30s of failed probes → Cloud Run **did NOT promote** the broken revision to Ready/serving status. Verified at +30s, +60s, ..., +150s: `latestCreated=00051-bxt` while `latestReady=00049-bsx` and `serving=00049-bsx` (the previous healthy revision). Production `/health/ready` returned `200, status:"ready"` for 3 sequential `curl` samples during the broken-revision window — production traffic 100% unaffected. **Revision-promotion gate works as the spec demands.**
- [x] 11.6 Recovery — Restored `REDIS_URL` to `staging-redis-url:latest` via `gcloud run services update --update-secrets`. Triggered a fresh `deploy-staging.yml` workflow run ([24977579327](https://github.com/aditrioka/nearyou-id/actions/runs/24977579327)) which deployed `nearyou-backend-staging-00053-n6v` with the correct config + the latest image. **Surprise**: after the previous broken-revision sequence, Cloud Run had pinned traffic explicitly to `00049-bsx` rather than tracking `LATEST`. The recovery deploy created `00053-n6v` but traffic stayed on `00049-bsx`. Resolved with `gcloud run services update-traffic --to-latest` which reset the routing-config to track the latest healthy revision. Post-recovery: `latestCreated=00053-n6v`, `latestReady=00053-n6v`, `serving=00053-n6v` at 100%, `/health/ready` green (postgres 5–22ms, redis 7–17ms, supabase_realtime 50–235ms). Temporary `staging-redis-url-broken-test` secret slot deleted. **Lesson logged**: a sequence of failed-startup-probe revisions can leave Cloud Run's traffic config pinned to the last-known-good revision rather than tracking `LATEST` automatically. The fix (`update-traffic --to-latest`) is one command, but the gotcha is worth a runbook note — added as a `FOLLOW_UPS.md` entry `cloud-run-traffic-pinning-after-failed-revisions` for an ops-runbook amendment.
