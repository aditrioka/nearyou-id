...
**Scope split with V2 RLS — RECONCILED**: re-reading V2 carefully, the policy at [V2__auth_foundation.sql:64-89](../../../backend/ktor/src/main/resources/db/migration/V2__auth_foundation.sql) is wrapped in a `DO $$ ... IF EXISTS conversation_participants ... END $$` block. When V2 originally migrated, `conversation_participants` did NOT exist, so the inner `EXECUTE` was skipped and the policy was NEVER installed. Flyway is forward-only — V2 will not re-run, so the policy gate cannot fire retroactively. **Conclusion: the policy is currently uninstalled in every environment.** V15 SHALL therefore install the policy directly with the CORRECTED definition: drop V2's subscriber-side `AND NOT EXISTS (SELECT 1 FROM public.users WHERE id = cp.user_id AND is_shadow_banned = TRUE)` clause from lines 81-84 — this clause would deny the shadow-banned user's own realtime subscription, contradicting the invisible-actor model. The shadow-ban hiding from OTHER readers is a publish-side concern (handled when the chat-realtime-broadcast change ships the publish layer; the publish must skip emit when the sender is shadow-banned, mirroring the read-path inline filter on `GET /messages`). With V15 installing the corrected policy, the REST and WSS authorization paths are aligned from day one — no V2-vs-policy asymmetry, no FOLLOW_UPS deferral needed for the policy's subscriber-side definition.
...
| `redacted_by` admin-FK shipping without the constraint creates a future-Phase-3.5 trap if the FK isn't added when `admin_users` lands | The `COMMENT ON COLUMN chat_messages.redacted_by` statement matches the existing V9 deferred-FK pattern at [V9__reports_moderation.sql:72-73, 110-111](../../../backend/ktor/src/main/resources/db/migration/V9__reports_moderation.sql); grep / schema-diff tooling stays aware of the deferred target. The Phase 3.5 admin-panel change owns the FK addition. |
...
1. **V15 migration** lands the three tables, all indexes, all CHECKs, and the `redacted_by` column without FK constraint (documented via `COMMENT ON COLUMN` matching the V9 pattern at [V9__reports_moderation.sql:72-73, 110-111](../../../backend/ktor/src/main/resources/db/migration/V9__reports_moderation.sql)). V15 ALSO directly CREATEs the `participants_can_subscribe ON realtime.messages` policy with the corrected D9 definition — V2's gated DO block was a one-time no-op (`conversation_participants` did not exist at V2-time, and Flyway is forward-only).
...
None at proposal time. The canonical schema in `docs/05-Implementation.md` is detailed and unambiguous; the design calls — D1 (advisory-lock layering), D2 (RESTRICT FK + Phase 3.5 cleanup-worker prerequisite), D3 (trigger vs application update for `last_message_at`), D4 (cursor shape), D5 (block at create AND send), D6 (redaction render policy), D7 (`last_read_at` shipped unused), D8 (status code for idempotent create-or-return), D9 (shadow-ban invisible-actor on REST + V2 RLS amendment deferred), D10 (lint-rule annotation for chat-history-readable-after-block) — are all resolved above. The V2 RLS amendment is the only deferred item and it's tracked as a hard prerequisite for `chat-realtime-broadcast` in `tasks.md` 11.7.
