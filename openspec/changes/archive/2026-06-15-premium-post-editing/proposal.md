...
- **New `GET /api/v1/posts/{post_id}/edits` history read** — chronological versions labelled "Versi ke-N" via `ROW_NUMBER() OVER (PARTITION BY post_id ORDER BY edited_at)`; honours shadow-ban (`visible_posts`) + a **real** bidirectional-block exclusion predicate on the read path; **returns content versions only — never the raw `location_snapshot`/`actual_location`** (spatial-fuzzing invariant; the modal shows content, not location).
...
- **Per-user edit rate limit** — the edit endpoint is rate-limited per user (reusing the rate-limit infrastructure; cap ops-tunable), so rapid re-edits can't amplify the per-edit moderation I/O + external Perspective calls. Exceed → 429 + `Retry-After`.
- **Non-leaky failure codes + no-op guard** — non-author / non-existent post → uniform **404** (no existence reveal); the author's own out-of-window post → distinct **409 `edit_window_expired`**; an edit identical to current content → **400 `no_changes`** (no junk versions).
