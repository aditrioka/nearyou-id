...
- **Code (new, `commonMain`):** a caching `LocationProvider` decorator (single-flighted via `Mutex`) + an injected clock/`TimeSource` seam in `id.nearyou.app.location`.
