...
1. **No burst limit.** Likes have a 500/hour burst clause for both tiers ([`docs/02-Product.md`](../../../docs/02-Product.md): "Both tiers cap 500/hour burst") to absorb anti-bot velocity-fingerprint patterns. Replies and chat sends do not — chat lacks the velocity-fingerprint surface that anti-bot burst caps target on likes (likes are mass-issuable click-events; chat is a 2000-char compose surface). The canonical line at [`docs/02-Product.md`](../../../docs/02-Product.md) is "Free 50/day, Premium unlimited" with no burst clause. So this change ships a daily-only limiter, no rolling-hour limiter — same daily-only shape as `reply-rate-limit`.
