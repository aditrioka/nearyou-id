...
- [x] 10.2 iOS simulator (per the iOS-sim verification recipe): launch → Nearby → rationale → when-in-use prompt → grant → feed renders; deny → denial state + Settings deep link. Confirm no coordinate appears in device logs. — verified on the **iOS 18.2 iPhone 16 Pro** sim: `pod install` (canonical helper) + `xcodebuild` BUILD SUCCEEDED (the KMP `ComposeApp` framework with the `iosMain` actuals links into `NearYouID.app`); app launches with **no `MissingResourceException`** (font/resources bundled); consent rationale modal renders (`status()`→`notDetermined`→Rationale); `simctl privacy grant location` + simulated Jakarta fix → `IosLocationProvider` acquires the coordinate → `GET /timeline/nearby?lat=***&lng=***` (**masked**, zero raw-coordinate leaks in the console) → 401 auth-reroute; `revoke location` → `status()`→DENIED → denial fallback ("Aktifkan lokasi…" + "Buka Pengaturan"). (The OS when-in-use *dialog* tap itself isn't scriptable headlessly, but `status()` for all three states + the provider + the denial render are verified.)
