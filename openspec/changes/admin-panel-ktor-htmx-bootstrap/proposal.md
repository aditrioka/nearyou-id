...
- **NEW** `/admin/` index route serving a "hello admin" page that extends the base layout.
...
- **NEW** Ktor `testApplication` test coverage (per [`design.md`](design.md) Decision 5) asserting (a) `/admin/` returns 200 + "hello admin" content, (b) layout template renders without errors, (c) HTMX asset returns 200 with correct `Content-Type` (only if Decision 2 picks vendored).
- **NEW** capability `admin-panel-scaffold` (4 requirements — see [`specs/admin-panel-scaffold/spec.md`](specs/admin-panel-scaffold/spec.md)).
- **Explicitly OUT of scope** (deferred to later Admin changes): authentication / login (Admin #3), session management + CSRF token issuance (Admin #3), any DB queries from admin routes (Admin #4+), `admin.nearyou.id` subdomain DNS + IAP/Cloud Armor + separate Cloud Run service deployment (Phase 3.5 deployment task — operates as Pre-Phase 1 ops work), WebAuthn / TOTP / Argon2id (Admin #3).
