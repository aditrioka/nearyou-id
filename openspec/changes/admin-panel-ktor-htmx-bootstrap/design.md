...
- Serve a "Admin Panel" index page at `/admin/` that extends the base layout, verifying end-to-end that templating + routing work.
...
- CSS framework / design system / branded styling — the scaffold uses minimal inline-or-vanilla CSS sufficient for the "Admin Panel" verification. Visual design comes alongside the first real admin feature (likely Admin #4).
...
**Alternatives considered:**
- **End-to-end browser test (Playwright / Selenium).** Rejected as massive overkill for a "Admin Panel" page. Adds a browser-driver toolchain dep + CI runtime cost. Defer to a real admin feature later if browser testing is needed.
...
- **Trade-off: scaffold ships no styling.** → Accepted. Visual design comes alongside the first real admin feature (likely Admin #4 `admin-actions-log-viewer`). The "Admin Panel" page is verification-grade, not user-facing.
...
4. Squash-merge → auto-deploys to `main`-staging. `https://api-staging.nearyou.id/admin/` becomes reachable; the "Admin Panel" stub IS publicly accessible (intentional per the unauthenticated-subtree design AND bounded to non-prod env per Decision 6's `KTOR_ENV` mount guard); Admin #3 closes this before any sensitive surface lands.
