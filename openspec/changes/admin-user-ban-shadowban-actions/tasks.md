...
- [ ] 1.3 File the `docs/06` reconciliation follow-up: `gh issue create --label follow-up,backend` — "docs/06 § Suspension vs Ban prescribes token_version+1 + refresh-token deletion for suspend/ban; shipped suspend/unban/report-queue-ban + this change deliberately do neither (enforcement = per-request AuthPlugin is_banned check). Reconcile the doc." (Design D2.)
...
- [ ] 5.1 Repository tests: eligibility guards (soft-deleted reject, already-permanent no-op, suspension→ban escalation `before_state`, already-shadow-banned no-op, not-shadow-banned shadow-unban no-op), column state, audit `action_type` + before/after, ban notification present, shadow-ban/shadow-unban notification absent, atomic rollback on forced audit/notification failure.
- [ ] 5.2 Route tests: role-gate matrix (moderator forbidden from ban; admin permitted; moderator permitted for shadow-ban/unban; read_only forbidden from all), CSRF missing/wrong rejection, malformed-`{id}` 4xx-not-500-no-write, redirect/HX-Redirect on success.
...
- [ ] 5.4 Profile-page render test: all controls + `hx-confirm` + quota chip render; un-shadow-ban shown for a shadow-banned target; ban control absent for a moderator session.
- [ ] 5.5 `database`-tagged tests: confirm any new DB-touching test pools `autoClose(hikary)` / size-2 per the CI connection-budget convention; run the touched-area DB tests explicitly (CI includes `database`, excludes `network`).
...
- [ ] 6.3 Pre-archive staging smoke (branch deploy): assert column state + one audit row each + the ban notification + the over-cap rejection; tick Section 6 before archive.
