...
- [ ] 2.4 Capture spec-class runtime for evidence: `time ./gradlew :core:domain:test --tests "id.nearyou.app.core.domain.ratelimit.ComputeTtlToNextResetTest" --rerun-tasks`. Note: Kotest StringSpec individual test names with spaces are not addressable via Gradle's `--tests` flag — the class-level filter runs all 7 tests in the spec class but the amended method dominates the runtime, so the wall-clock measurement remains a useful proxy for the new threshold's cost. Record the wall-clock time in the apply commit body. Goal: <10s wall clock for the spec class (with 2× safety over the 5s expected for the dominant method).
- [ ] 2.5 Contingency: if the wall-clock time on CI exceeds 10s for the affected method, fall back to threshold approach 2 — sample 1000 with `(differingPairs >= 995) shouldBe true` (Poisson `P(failure) ≈ 5×10⁻⁷`, ~5σ-equivalent — still statistically safe per `design.md` Decision 1 alternatives). The contingency MUST land BOTH (a) the test edit AND (b) a re-amendment of [`openspec/changes/harden-compute-ttl-test/specs/rate-limit-infrastructure/spec.md`](specs/rate-limit-infrastructure/spec.md) flipping the MODIFIED scenario from "100,000 / 99,500" back to "1000 / 995" so the spec and test stay in sync at archive-time. Re-run `openspec validate --strict` after both edits. Document the contingency invocation explicitly in the apply commit body (which approach was actually shipped + why).
