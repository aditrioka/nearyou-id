...
- **Empty** (`Loaded`, empty posts, no `upsell`) → the **loading-skeleton presentation** with `stringResource(Res.string.timeline_empty_global)` set to "*Sedang memuat postingan…*" (the exact copy from `docs/03-UX-Design.md` § Empty State, which frames the Global-empty edge case as a loading skeleton because Global is effectively never empty).
