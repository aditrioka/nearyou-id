...
- **MODIFIED** backend capability `single-post-read`: add a nullable `editedAt` field (non-null **iff the post has `post_edits` rows** — derived from edit-history existence, not from `posts.updated_at`) for the "Diedit" label, AND a per-viewer `isAuthor` boolean (computed from `author_id` **without** projecting the UUID) so the mobile client can gate the Edit affordance to the viewer's own post (ownership is otherwise undeterminable client-side). Post-detail consumes this projection via a `single-post-read` GET on each resume (it is otherwise route-payload-driven). Preserves the response's no-PII discipline (no author UUID, no raw lat/long).
