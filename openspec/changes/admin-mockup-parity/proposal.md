...
- **`/admin` (no trailing slash) routes by session instead of 404** — an explicit handler 301/302-redirects `/admin` → `/admin/`, which then applies the existing session gate (no session → `/admin/login`; session → index). No global `IgnoreTrailingSlash` (that would change matching for every API route).
