...
The admin panel ships as Pebble templates + HTMX + vendored vanilla CSS (`admin-panel-scaffold` spec; docs/11 § 3.6). The visual target for every frame is the admin mockup board `dev/mockups/nearyou-admin-mockup.html` — frame 1 (login) and frame 2 (shell + scaffold landing) are the targets here; frame 3 (Operational Dashboard) is a later, separate change that *replaces* the scaffold landing. Current state: `layout.peb` already renders a grouped sidebar (Moderasi / Anti-abuse & keamanan / Sistem `ngroup` headings), the brand logo, a session box containing only a bare Logout form, a pure-CSS hamburger drawer for narrow viewports, a header `<h1>`, and a page footer — the actual gaps vs frame 2 are: no per-item icons, no Dashboard nav item, no active-state marking, no identity-box contents (role chip / display name / session line), no top bar with page title + env chip, and frame 2 has no page footer (the current one gets removed). `index.peb` renders four static description cards; `login.peb` renders a heading + 3 unadorned inputs; `GET /admin` (no trailing slash) 404s because only `get("/")` is mounted inside `route("/admin")` and `IgnoreTrailingSlash` is not installed.
...
The mockup uses the Material Symbols *font* with ligature names (`mail`, `key`, `timer`, `visibility`, `visibility_off`, `error`, `info`, `dashboard`, `flag`, `group`, `block`, `receipt_long`, `logout` — 13 total, `visibility_off` being the toggled state of the password eye). The product needs ~13 icons; vendoring the font (~200 KB+ subset pipeline) is not worth it. Instead: one `icons.peb` Pebble macro emitting inline `<svg>` (24dp Material Symbols outlines, `fill="currentColor"`), called as `{{ icon('mail') }}`. Each emitted `<svg>` carries a `data-icon="<name>"` attribute so tests (and the measurement-annex comparison) can assert *which* icon renders where without matching path data. Apache-2.0 attribution comment at the top of the partial. Alternative rejected: separate `.svg` static files + `<img>` (can't inherit `currentColor`).
...
Session validation already performs two reads per request (`SessionRepository.findBySessionHash` for the session row, then `AdminUserRepository.findById` for the admin row — that is where `role` comes from in `AdminAuthPlugin.onAuthenticate`); adding `display_name` to the existing `findById` SELECT and carrying it on the principal costs nothing — no *third* lookup is introduced in the layout path. The identity box renders: role chip (uppercased role, `owner` gets the accent style per mockup), display name, and `Session idle {idleTimeout} · expires {HH:mm} UTC` where the expiry shown is `min(lastActiveAt_afterRefresh + idleTimeout, expiresAt)` — the actual moment this session dies if the operator goes idle now. The 8 h absolute cap thus surfaces naturally as it becomes the binding constraint.
...
**D8 — Sidebar markup lives entirely in `layout.peb`** (groups: Moderasi → Dashboard `/admin/`, Reports, Users; Anti-abuse & keamanan → Rejected identifiers; Sistem → Audit log), with an `active` state matched on the current path (model value, same mechanism as `pageTitle`). The mockup's per-item status dots are board annotations (frame 2 caption: "dot hijau = shipped, dot hollow = usulan") and are not product UI. **Dashboard is rendered despite its hollow board dot**: that annotation marks the frame-3 *Operational Dashboard content* as unbuilt, not the `/admin/` landing page itself, which is shipped — and the operator needs a nav entry for the page they land on. The current `layout.peb` page footer is dropped (frame 2 has none).
...
- **[Responsive contract is CSS-only]** The narrow-viewport hamburger behavior (frame `#f04b`) is pure CSS (`nav-toggle` checkbox pattern) — route tests cannot observe it, so it has no spec scenario before or after this change. → Deliberate: the docs/11 § 5 manual-verification step (screenshots at desktop + narrow widths) is the gate, as it is today.
