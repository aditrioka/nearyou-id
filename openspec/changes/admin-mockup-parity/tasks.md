...
- [ ] 3.1 Add `displayName` to `AdminPrincipal`, populated from the same session-validation read (no extra query), per design D5
- [ ] 3.2 Pass layout model values from authenticated routes: admin display name, role, computed session-expiry display value (`min(lastActive_afterRefresh + idleTimeout, expiresAt)`, formatted `HH:mm` UTC), idle-timeout label, environment name (design D6), page title + active-path marker (design D7/D8)

## 4. Shell (`layout.peb` + `admin.css`)

- [ ] 4.1 Rebuild the layout shell per frame `#f02` + measurement annex: grouped sidebar (Moderasi → Dashboard / Reports / Users; Anti-abuse & keamanan → Rejected identifiers; Sistem → Audit log) with per-item icons + active state; NO unshipped menu items, NO status dots
- [ ] 4.2 Sidebar footer identity box: role chip (uppercase; owner accent), display name, `Session idle {n} m · expires {HH:mm} UTC` line, Logout button with icon (existing POST + `_csrf` semantics untouched)
...
- [ ] 6.1 Rebuild per frame `#f01` + measurement annex: logo only (drop "Admin login" heading), start icons on email/password/totp inputs, password visibility toggle (`<button type="button">` + minimal inline JS per design D3), error banner with leading icon
- [ ] 6.2 Verify the no-enumeration byte-equality contract still holds (existing scenario/test + assert the error banner markup is failure-path-invariant)
- [ ] 6.3 Login route tests updated: presentation scenario (no heading, icons present, toggle script present), CSRF-meta-absence + authenticated-redirect scenarios still green

## 7. Verification & delivery

- [ ] 7.1 Full local gate: `./gradlew ktlintCheck detekt :backend:ktor:test :lint:detekt-rules:test` (fresh DB containers for the DB-tagged specs)
- [ ] 7.2 Manual verification per docs/11 § 5 DoD: run the panel locally (`verify-loop` recipe), screenshot login + landing + one feature page at desktop and narrow widths, compare against rendered mockup frames, attach evidence to the PR body
...
- [ ] 7.4 `openspec validate admin-mockup-parity --strict` + PR title/body current at the phase boundary
