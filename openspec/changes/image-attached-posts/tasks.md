...
- [x] 7.5 iOS: `:mobile:app:iosSimulatorArm64Test` (or at minimum `linkDebugFrameworkIosSimulatorArm64`) for the PHPicker/ImageIO actual.
- [x] 7.6 Backend `:backend:ktor:test`: **all 5 read surfaces** — Nearby, Following, Global, single-post, and post-detail responses — carry `imageUrl` (built 4-segment URL) for an image post and `null` for a text-only post; an image-bearing post is excluded under shadow-ban/block exactly as a text-only post; a viewer reading their OWN shadow-banned image post via the self-arm still sees their `imageUrl` (two-arm UNION self-arm projection, per task 2.2).
- [x] 7.7 Local gates green: `./gradlew ktlintCheck detekt :backend:ktor:test :lint:detekt-rules:test :mobile:app:testDevDebugUnitTest :mobile:app:testDevReleaseUnitTest`.
