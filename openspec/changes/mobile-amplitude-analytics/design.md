...
**D2 — Durable `ConsentSnapshotStore` via `expect/actual` (resolves #198); write-on-`200`-echo preserved.** Replace `InMemoryConsentSnapshotStore` with a durable binding — Android DataStore (already pinned for `SecureTokenStore`) / iOS `NSUserDefaults` (a system framework, no pin) — so **no new library pin** is added. This follows the `expect/actual` *precedent* set by `SecureTokenStore` (`expect class`), a deliberate choice over `docs/11` §2.5's interface-default: the per-platform storage differs (consent flags are **non-secret**, so iOS `NSUserDefaults` is intentional — not the Keychain `SecureTokenStore` uses for tokens), which a single shared `commonMain` impl can't express. Write semantics are **unchanged**: the persisted triple is the server-echoed `PATCH 200` body, never a client guess (`mobile-settings` hard requirement — this rules out optimistic local-first writes). The **onboarding** `ConsentScreen` now also writes the snapshot on its `200` (today only Settings does), so a user who consents at onboarding has a durable snapshot. *Consequence:* the Sentry crash gate, reading the same seam, gains durable cross-process decline for free — the resolution `mobile-crash-reporting` explicitly anticipated. *Alternative — server-authoritative consent field on a profile/`me` response:* rejected by the operator in favor of the mobile-only durable path (no backend coupling; converts deferred debt #198 into done).
...
**D5 — US ingestion endpoint by default, config-driven.** Default `https://api2.amplitude.com/2/httpapi` (Indonesia is not EU residency). `amplitudeConfig.endpoint` overrides to `https://api.eu.amplitude.com/2/httpapi` without code change. The privacy policy / consent disclosure should state that product analytics are processed by Amplitude (US) — a Pre-Launch documentation task, not a code dependency.
...
- **Per-fire consent check is non-atomic with the async send.** → If a user toggles analytics OFF after an event passes the gate but before its fire-and-forget POST flushes, that single in-flight event still reaches Amplitude. Sub-second window; spec-compliant (`docs/06`: toggles apply to *future* events). Accepted, not mitigated.
