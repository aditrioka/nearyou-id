...
- **RESOLVED (review): who is the UI caller of the grace-window opt-out?** The "confirm switch public" opt-out (D3) is a privacy-REDUCING action reached via the **privacy-flip banner** (`privacy_flip_warning`, `docs/03` § Downgrade flow — a separate, not-yet-built in-app surface), NOT the Settings "Profil privat" toggle. The Settings toggle stays Premium-gated (Free → upsell, no write) exactly like "Sembunyikan jarak", so this slice's `mobile-settings` delta correctly specs only Free→upsell and adds no Free-opt-out scenario. The backend write is permitted for any tier (D1), so the future banner can issue `PATCH {"privateProfile": false}` and the D3 grace-clear takes effect; the behavior is fully exercised at the API layer now (tasks 5.2). Wiring the banner's "confirm switch public" button is OUT of scope here (the banner UI does not exist yet) and is a follow-up — D3 is endpoint correctness ready for that caller, not a dead path.
