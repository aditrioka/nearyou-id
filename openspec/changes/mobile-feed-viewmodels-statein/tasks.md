...
- [x] 7.4 Manual verification — **operator accepted the test-coverage evidence in lieu of a fresh device screenshot (explicit buy-in, 2026-06-24)** for this behavior-preserving refactor. Evidence: the Robolectric `*ScreenTest` suite renders the REAL feed screens over the new single-`uiState` collection and passes (all visual states); the change is provably behavior-preserving (same pure projection, same states/transitions); and the single-`stateIn` + `collectAsStateWithLifecycle` pattern is already device-shipped on `SignInViewModel`/`FollowListViewModel`.
