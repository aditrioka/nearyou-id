...
- **Peer ids hashed via a server-keyed HMAC** — `HMAC-SHA256(export-peer-hash-secret, peer_id)`, the secret via `secretKey` (the invite-code-HMAC precedent), NOT a bare `SHA256`: a bare digest of enumerable UUIDs is brute-forceable AND identical across every export, enabling a colluding-users correlation attack to confirm a shared counterparty. New dedicated secret slot **`export-peer-hash-secret`**.
- **Posts carry the user's own `actual_location`** (+ `city_name`) — own-data is the sanctioned non-`display_location` read; the repo site needs the **`@AllowActualLocationRead("data export own-content")`** annotation (the exact token `CoordinateJitterRule` requires — `@AllowRawPostsRead`/`@AllowMissingBlockJoin` alone would still fail the coordinate-jitter detekt lane).
...
Builds on, without deviating from: **backend layering** (Route → Service → Repository; the export-gather logic lives in a Service, raw SQL in a Repository); the **internal-worker pattern** (OIDC + `GoogleOidcTokenVerifier` + Cloud Scheduler, the suspension-unban precedent); **JDBC discipline** (Hikari pool, bounded per-user queries, no N+1 — the scope-matrix gather is a fixed set of per-user queries); **new `:infra:*` module boundaries** — each vendor-neutral interface (`ObjectStore`, `EmailSender`) lives **inside its own `:infra` module** (packages `id.nearyou.app.infra.r2` / `id.nearyou.app.infra.resend`), the shipped `ImageStore` (`:infra:cloudflare-images`, package `…infra.cloudflareimages`) / `ImageModerator` (`:infra:cloud-vision`) precedent — NOT `:core`; `:backend:ktor` imports the interface from the `:infra` module and no `aws.sdk.*` / Resend transport type leaks past it (the no-vendor-SDK-outside-`:infra` invariant). **This conforms to the existing `:infra`-owned-interface pattern → no docs/11 §Pattern Registry deviation, no amendment needed.**
...
- **Scope creep across many tables** → the spec mirrors the canonical docs/06 §350 matrix as discrete Included/Excluded scenarios, so the set is fixed by canonical, not invented.
- **Bundling two new `:infra` modules + feature in one change is large** → kept bundled deliberately: R2 + Resend are *both* hard dependencies of the single deliverable (the worker cannot deliver without upload + email), neither has another in-flight consumer, and splitting would ship two inert modules with no exercising path (worse testability); the fail-soft NoOp posture keeps each independently boot-safe.
- **`signed_url` persists in `notifications.body_data` for the 90-day notification-retention window** (long after the 24h URL dies) → inert: the URL resolves to nothing once the R2 object-lifecycle rule deletes the object at 24h; no action beyond that lifecycle rule + the no-PII-in-logs discipline.
