...
- **Share-vs-tombstone race closed.** The send transaction re-checks the author after the INSERT (the new FK's `FOR KEY SHARE` lock serializes against the tombstone's row lock), scrubbing a snapshot built from a pre-tombstone read and returning the scrubbed snapshot to the 201 response + Realtime broadcast.
