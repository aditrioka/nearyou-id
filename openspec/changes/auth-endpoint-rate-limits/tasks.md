...
- [ ] 1.2 Confirm key construction hoists `hashedIp` out of the literal and uses literal scopes (RedisHashTagRule), non-`_day` keys with hardcoded `Duration` ttl (RateLimitTtlRule passes burst keys), hashed-IP value only (OtelForbiddenAttributeRule).
...
- [ ] 3.1 `signin` — over-cap (11th call same IP within window) → `429` + positive `Retry-After`; under-cap → admitted (Redis-backed test double / InMemoryRateLimiter).
- [ ] 3.2 `signup` — over-cap → `429` + `Retry-After`; under-cap → admitted.
- [ ] 3.3 `refresh` — over-cap → `429` + `Retry-After`; under-cap → admitted.
...
- [ ] 4.4 `TokenRefresher.performRefresh`: branch on `429` BEFORE the terminal-invalidate branch — keep the token pair, do NOT call `sessionInvalidator.invalidate()`, surface a transient/retryable condition (transport-failure contract).
...
- [ ] 6.3 `TokenRefresher`: `429` → token pair kept + `invalidate()` NOT called (regression for the would-be logout); a non-429 non-2xx still invalidates (unchanged).
