...
**Token + reconnect**: the infra impl fetches a fresh HS256 token immediately before joining the channel. `GET /api/v1/realtime/token` returns the shipped **`{ "token": "<jws>", "expires_in": 3600 }`** envelope (`RealtimeTokenResponse`, snake_case per the auth-route convention) — the client DTO mirrors that envelope (per D9), NOT a bare string, and the refresh cadence keys off `expires_in` rather than a hardcoded 1h literal. On a transport drop it rejoins with a freshly-fetched token (the old one may be near expiry). On every (re)subscribe the ViewModel triggers a **REST resync** of the first page and merges by id — this is the no-outbox recovery path (`chat-realtime-broadcast`: missed broadcasts are recovered via REST, not replayed). Subscription is cold: collection starts on thread entry and is cancelled (channel unsubscribed) when the route is popped / the ViewModel is cleared.
...
### D6 — Shadow-ban is server-authoritative (no client ban-knowledge); redaction handled at render

Shadow-ban filtering is **server-side and authoritative**, and the client does NOT re-implement it — a deliberate change from an earlier draft that had the consumer "drop a self-echo while the viewer is shadow-banned." That draft was unimplementable: shadow-ban status is intentionally **invisible to the banned user**, so the client has no signal to gate on, and it isn't needed anyway:
- The server's **publish-side skip** means a shadow-banned sender's messages are never broadcast — so a shadow-banned user receives no realtime echo of their own message to filter; they still see their own messages via the optimistic local append + the REST read path (which returns own messages to the sender).
- The server's **read-path filter** excludes shadow-banned *other* senders, so a non-banned viewer's realtime stream only carries non-banned senders' messages.
- Any own-message that DOES arrive via realtime (the non-banned normal case, or an admin-flip mid-flight race) is **collapsed by the id-keyed dedupe** (D4) onto the already-present optimistic/REST row — no duplicate, no special-casing, no ban-knowledge required.

So the client's only realtime-merge invariant is the id-dedupe; shadow-ban correctness is the server's. Redaction IS honored at render: a message with `content == null` (REST or realtime) renders the neutral `chat_message_redacted` placeholder ("*Pesan ini telah dihapus*"), never an empty bubble; the id-keyed dedupe means a redaction arriving after the original collapses onto (updates) the same row.
...
- **`mobile-home-tab-host` is modified by three concurrent PRs (#245, #246, this)** → the three deltas are disjoint requirements (Profil-section content / Following-tab content / app-bar "Pesan" action), but they touch the same spec file and likely the same `HomeScreen`/`AppShellScreen` source. Mitigation: the MODIFIED delta here is scoped to a single ADDED requirement (the app-bar action) that does not edit the section/tab requirements #245/#246 touch; the squash order is flagged so whichever lands last rebases the app-bar action onto the merged host. **Two distinct reconciliations**: (a) at *implementation* time, the source (`HomeScreen`/`AppShellScreen`) rebases; (b) at *archive* time, each of the three PRs adds its own `mobile-home-tab-host` delta `spec.md`, so whichever archives last must reconcile the MERGED requirement set into `openspec/specs/mobile-home-tab-host/spec.md` (an additive app-bar requirement alongside #245's Profil-section + #246's Following-tab requirements) — a spec-sync step, not just a source rebase. Both surfaced in the PR body for reviewer awareness.
