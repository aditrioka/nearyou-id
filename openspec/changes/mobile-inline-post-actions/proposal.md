...
- **Send-message action NOT rendered**: mockup frame 1 shows a third (kirim pesan) action; chat is not built, and the no-dead-controls rule forbids rendering it. An explicit deferred requirement (positive + negative-guard + tracking scenario) in `mobile-post-card` is the future chat change's MODIFY hook.
