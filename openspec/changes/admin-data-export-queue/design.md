...
- **[Double-run race + audit attribution]** An operator trigger landing while the scheduled worker also picks the row up. → Mitigation: `claimPending`'s `pending → processing` affected-rows guard already serializes; the second claimant gets `SKIPPED` (no double-process) — the existing producer invariant, inherited unchanged. **Audit attribution:** the trigger's audit row is keyed on the trigger *causing a state transition* (the re-enqueue `failed → pending`, or the claim of an already-`pending` row), NOT on the final delivery outcome. So if the trigger re-enqueues a `failed` row and the scheduler then claims+delivers it before the synchronous drive (the drive no-ops `SKIPPED`), the trigger is still **one** successful audited action — the export is delivered by whichever claimant wins the `claimPending` guard. The rate-limit COUNT + audit INSERT + state transition run on **one connection in one transaction** (matching `DeletionQueueExpediteRateLimiter`'s in-tx cap check) so two concurrent triggers cannot both pass the 10/hr cap.
- **[Re-enqueue UPDATE vs the producer's INSERT 23505 handler]** The producer catches `23505` on an **INSERT** (`insertPendingOrReturnActive`). The trigger's re-enqueue is a different statement — an **UPDATE** flipping a `failed` row to `pending` — which raises `23505` against `data_export_requests_one_active_idx` only if the user already holds another active row. → The implementer MUST wrap the UPDATE in its own `23505` catch (do not copy the producer's INSERT handler verbatim) and map the violation to the "already active, no-op" outcome.
- **[Target user hard-deleted mid-flight]** `data_export_requests.user_id` is `ON DELETE CASCADE`, so a hard-deleted user's request row vanishes. → A trigger on a now-absent `id` collapses into the unknown-`id` benign no-op (no audit row); no separate handling needed.
