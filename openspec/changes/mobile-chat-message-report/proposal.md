...
- **Gate** the affordance on `senderId != viewerId` (only the *other* party's messages are reportable — mirrors the post `!isAuthor` gate) and exclude already-redacted messages (`redactedAt != null`) — an already-moderated message is not reportable.
...
### New Capabilities
- `mobile-chat-message-report`: the `:mobile:app` chat-thread surface for reporting an individual chat message — the long-press → "Laporkan" → shared `ReportDialog` entry point, the `senderId != viewerId` + non-redacted affordance gate, submission via the shared seam with `target_type = "chat_message"`, the post-detail-style outcome→UI mapping, PII discipline, and the commonTest / Robolectric / iOS test trio.
