...
- [ ] 1.2 Mode A — Tier 1 forbidden-attribute keys: regex match (preferably `Set<String>` exact-match) the 22 entries listed in `specs/observability-otel-foundation/spec.md` § "Tier 1 — forbidden-attribute-key literals" (11 Group A + 8 Group B + 3 Group C).
...
- [ ] 2.2 Tier 1 Group A positive-fail cases (11 tests): one per FORBIDDEN_KEYS mirror entry — the 3 HTTP client-identity keys (`client.address`, `client.port`, `http.client_ip`), 2 new-semconv peer keys (`network.peer.address`, `network.peer.port`), 3 old-semconv peer keys (`net.peer.ip`, `net.peer.port`, `net.sock.peer.addr`), and 3 user-id typo variants (`user_id`, `user_uuid`, `user.uuid`).
...
- [ ] 2.25 Synchronization guard test: assert the rule's Tier 1 Group A list `containsAll ForbiddenAttributeStripper.FORBIDDEN_KEYS` (regression guard against silent drift in the runtime list). Test failure message MUST name the missing key(s) so the next contributor adding to `FORBIDDEN_KEYS` knows exactly what to update. NOTE: requires `:lint:detekt-rules:test` to reference `ForbiddenAttributeStripper.FORBIDDEN_KEYS` at compile time — pick at implementation: (a) add `testImplementation(project(":infra:otel"))` to `lint/detekt-rules/build.gradle.kts`, OR (b) hardcode the expected keys in the test fixture with a comment pointing at `ForbiddenAttributeStripper.kt:89-108`.
...
- [ ] 5.2 At archive time, ADD three new `FOLLOW_UPS.md` entries for the deliberately-deferred scope (per `design.md` § "Explicitly deferred follow-ups"):
