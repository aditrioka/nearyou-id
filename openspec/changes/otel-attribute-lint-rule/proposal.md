...
- Add `OtelForbiddenAttributeRule.kt` in `lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/` — a Detekt Rule that fires on:
  - **Tier 1 — forbidden attribute keys (22 entries total)**:
    - Group A (11 keys, exact mirror of `ForbiddenAttributeStripper.FORBIDDEN_KEYS`): 3 HTTP client-identity semconv keys (`client.address`, `client.port`, `http.client_ip`), 2 new-semconv peer keys (`network.peer.address`, `network.peer.port`), 3 old-semconv peer keys (`net.peer.ip`, `net.peer.port`, `net.sock.peer.addr`), and 3 user-id typo-defensive variants (`user_id`, `user_uuid`, `user.uuid`).
