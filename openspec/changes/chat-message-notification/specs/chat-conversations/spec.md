...
**Rollback semantics.** If `NotificationEmitter.emit(...)` throws (e.g., the recipient was hard-deleted between auth and emit, causing FK violation on `notifications.user_id`), the entire transaction SHALL roll back: no `chat_messages` row persists, `conversations.last_message_at` is unchanged, `ChatRealtimeClient.publish` SHALL NOT be invoked (publish runs only post-commit). The HTTP response in this case SHALL be `5xx` (the existing chat-foundation error-handling path is reused — no new error contract). This matches the `Emit failure rolls back primary write` scenario in [`in-app-notifications/spec.md`](../../../../openspec/specs/in-app-notifications/spec.md).
...
- **GIVEN** A is an active participant; the schema CHECK permits an INSERT with `content = NULL` AND a non-null `embedded_post_snapshot`; a synthetic test path exercises this insert (e.g., via a future-compat repository hook that bypasses the public endpoint's content-required guard)
