...
| **Sender hard-deleted mid-flight between emit and FCM dispatch.** Alice's emit fires (in-tx, Alice not shadow-banned, notification row written with `actor_user_id = Alice`). Before FCM dispatch fires, admin hard-deletes Alice. The `notifications.actor_user_id` FK uses `ON DELETE SET NULL`, so the dispatcher reads the row with `actor_user_id = NULL`, calls `ActorUsernameLookup.lookup(null)` (or short-circuits per [`fcm-push-dispatch/spec.md:98`](../../specs/fcm-push-dispatch/spec.md)), and renders the `"Seseorang mengirim pesan"` fallback. | **Correct behavior — already covered by existing `fcm-push-dispatch` null-fallback path.** Sender hard-delete is also structurally near-impossible during an active chat send under the canonical schema: `chat_messages.sender_id REFERENCES users(id) ON DELETE RESTRICT` per [`docs/05-Implementation.md:1276`](../../../docs/05-Implementation.md), AND the same `conversation_participants.user_id` RESTRICT semantics that block recipient-hard-delete also block sender-hard-delete (sender is also a participant). The race is reachable only if the canonical hard-delete worker manually removes the participant row pre-user-hard-delete AND the user already has historical chat_messages rows that cascaded SET NULL on their FKs (none — `chat_messages.sender_id` is RESTRICT, not SET NULL — so the user can't be hard-deleted while ANY chat_messages row references them). The composition therefore works correctly via the FK SET NULL on `notifications.actor_user_id` only for a sender hard-deleted AFTER all their chat_messages rows are also hard-deleted (cascade via `conversations` deletion or admin tombstone purge). No additional code or spec needed; the fallback path catches the rare race even when present. |
