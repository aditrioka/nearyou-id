...
2. **Strict ordering with `target_id`.** The notification's `target_id = chat_messages.id` references a row in `chat_messages`. If we emit before INSERT, the target_id doesn't exist yet. If we emit after INSERT but post-commit, we add a window where a crash leaves a chat row with no notification — and the FK on `notifications.target_id` (which is nullable but logically references `chat_messages`) is dangling-tolerant, so this would fail silently.
...
| **Doc divergence — `in-app-notifications/spec.md:11` "V10 only writes 4" becomes stale.** | The MODIFIED in-app-notifications delta updates the count. Reviewers will catch lingering "4" references in docs. |
