...
- **D3: Navigation is a nullable one-shot state field, emitted to the host.** `ProfileUiState.openChat: ProfileChatTarget?` is set on `Ready` and cleared via `onChatOpened()`. It mirrors `navigateBack` / `onNavigatedBack()` (docs/11 §2.2: one-shot events are state, not streams). `ProfileChatTarget` holds `conversationId` plus the partner's display identity, snapshotted from the loaded profile when the call resolves. The screen's `LaunchedEffect` forwards it to the hoisted `onOpenChatThread(conversationId, username, displayName)`. That is the same name and shape as `AppShellScreen`'s existing thread-push lambda; the shell's `onOpenChat` means "open the conversation list" (docs/11 §4, one concept one name). `AppEntryProvider`'s `ProfileRoute` entry wires it to `backStack.add(ChatThreadRoute(...))`. The route carries display identity only, and the partner UUID never enters the serialized back stack.
  - Review round: the first cut read the identity off the current `ProfilePhase.Content` inside the effect. That had a latent trap: if the phase were not `Content`, the effect returned without clearing, and because the endpoint returns the *same* id again, a later `Ready` would not re-key the effect. Snapshotting at resolve time removes the phase dependency.
- **D7: A block that has already succeeded beats a late `Ready`.** If the user confirms Blokir while a create-or-return is in flight and the block wins the race, the `Ready` branch sees `navigateBack` pending and drops the navigation. The backend would 403 every send in that thread anyway, and it would break the `mobile-profile` "block pops back" contract.
...
- **D6: Button placement and style.** An `OutlinedButton` labelled "Kirim pesan" sits in the other-user action row, between the follow toggle and the kebab. The row is now a `FlowRow` (review round): a plain `Row` measures children in order, so at a large font scale it would squeeze the trailing kebab, which is the Blokir/Laporkan safety entry; the `FlowRow` wraps it instead. This is verified on the emulator at font scale 2. A Robolectric check can't discriminate this, because Robolectric has no real text metrics and buttons measure at their minimum width. The follow toggle stays the primary CTA (filled "Ikuti" / outlined "Mengikuti"), and messaging is secondary. No other-user profile frame exists on the mockup board, so the frame-3 profile layout (one full-width tonal action under the identity block) governs the look. The shipped profile's compact centered action row is kept rather than re-laid-out, because that re-layout is the separate `mobile-mockup-visual-conformance` concern. M3 buttons meet the 48dp minimum interactive size.
