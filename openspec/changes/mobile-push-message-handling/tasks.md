...
- [ ] 2.3 Dispatcher tests: iOS payload carries the routing fields + stays ≤4 KB (incl. the oversized-`body_full` clamp case still holding with the extra fields); Android payload carries the masked `actor_username` (resolved + generic-fallback + empty-when-null).
