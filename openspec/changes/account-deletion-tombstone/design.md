...
No `account` package exists (`auth, user, …`). Create `account/` with `AccountRoutes` (thin) → `AccountService` (tx boundary) → `AccountDeletionRepository` (JDBC), per docs/11 §3.1. The worker lives as `AccountHardDeleteWorker` mounted under the existing `/internal/*` OIDC subtree (precedent: suspension-unban-worker). Rejected: folding into `user` (deletion is its own feature surface with a worker; SRP favors a package).
