...
No data migration. Rollout: ship the screen + `:infra:revenuecat` with the fail-soft state; the CTAs route to a working (if `Unconfigured`-until-provisioned) paywall. When the operator provisions RevenueCat dashboard + store products + `staging-revenuecat-*` secret slots, the live Offerings/purchase path activates with no code change. Rollback: revert the PR (no schema/state to unwind); the CTAs return to dismiss-only.
...
- **Confirm RevenueCat entitlement identifier** (the dashboard entitlement key the client checks, e.g. `premium`) at provisioning time; the `PurchaseController` abstracts it so the app code is unaffected.
