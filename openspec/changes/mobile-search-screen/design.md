...
**D5 — The `Retry-After` header is the only rate-limit reset signal; the elapsed surface offers a manual retry.** The 429 body is bare `{"error":"rate_limited"}`; the reset is the `Retry-After` header (seconds). The repository parses it into `RateLimited(retryAfterSeconds)`; an absent/stripped/unparseable header (proxy-rewritten to an HTTP-date) maps to `RateLimited(0)`, and the screen floors a non-positive value to one minute (the shipped post-detail / cap-dialog floor precedent — never a flash-dismiss). The countdown reuses the `mobile-cap-upsell-dialog` `capCountdownMinutes` formatter directly (minutes rounded up), rendered via `search_rate_limited` ("Reset dalam %1$d menit" — an integer-minutes form with the unit in the resource, a cosmetically-equivalent simplification of the spec's `%1$s` countdown-string), decremented via monotonic `delay` (no wall-clock API). When the countdown reaches zero the cap has reset: the countdown is **replaced by a "Coba lagi" retry control** (`search_rate_limit_reset` + `cta_retry`) the user taps to re-issue the query — a deliberate user action, NOT an auto-fetch. *Alternatives rejected*: a static at-open countdown (diverges from the docs "Reset dalam X menit" live treatment); auto-retrying on elapse (silently re-consumes the 60/hr quota, and on a same-`retryAfterSeconds` re-limit the conflated `RateLimited` StateFlow value can stall the re-armed countdown — the manual control sidesteps both).
...
Mobile-only, no migration, no flag of its own (the `search_enabled` kill switch already gates the backend). The screen ships behind the new Home app-bar search icon; on a `503` it self-presents the disabled state. One squash-merge; no sequencing dependency on the in-flight profile (#245) / following (#246) / chat (#247) picks (disjoint files — a new `screens/search/` + `data/search/` tree plus an additive `mobile-home-tab-host` app-bar action). The `mobile-home-tab-host` delta deliberately **ADDS** a new `onOpenSearch` requirement rather than modifying the shared `onOpenPost` hoist requirement that #246 also touches — so the only requirement this change MODIFIES is "Bottom navigation is a top-level section shell" (the app-bar; not touched by #245's Profil-placeholder MODIFY or #246's onOpenPost/placeholder MODIFY). That keeps the spec deltas non-overlapping with the concurrent picks and avoids a renamed-header MODIFIED that would not header-match at archive.
