...
2. **Relocates** `ASSETCATALOG_COMPILER_APPICON_NAME` out of the hardcoded `project.pbxproj` target-level slot (`project.pbxproj:307` / `:336`) so the per-configuration xcconfig value wins. Mapping (refined during apply — `Config.xcconfig` is staging-flavored, `PRODUCT_BUNDLE_IDENTIFIER=id.nearyou.app.staging` + staging API, NOT production-default as F5 assumed): `Config.xcconfig` → `AppIcon-Staging` (base = a staging build; fail-safes any unconfigured future build to the non-production icon), `Staging.xcconfig` → `AppIcon-Staging`, `Production.xcconfig` → `AppIcon` (the sole cobalt-resolving config). This fully resolves the F2 "cobalt icon on a staging-flavored base" mismatch, not just the spec-scenario wording.
3. Adds a `AppIcon-Staging.appiconset` (staging-tinted 1024 PNG).

**CocoaPods base-config caveat (verified at proposal time).** The app **target's** `Debug`/`Release` configurations base off the CocoaPods-generated `Pods-iosApp.{debug,release}.xcconfig` (`project.pbxproj:304` / `:333`) — *not* `Config.xcconfig`, which is only the **project-level** base (`project.pbxproj:362–363`). (Verified **no swap**, and noting it because a naive grep mis-pairs the blocks: the target's `/* Release */` config opens at `project.pbxproj:302`, bases off `Pods-iosApp.release.xcconfig` (`:304`), closes `name = Release` (`:329`); the `/* Debug */` config opens at `:331`, bases off `Pods-iosApp.debug.xcconfig` (`:333`), closes `name = Debug` (`:358`). The standalone `name = Debug` at `:300` belongs to the *project-level* Debug block opened at `:237` (→ `Config.xcconfig`), NOT to the target Release block whose base is at `:304` — mis-pairing those two is the source of the recurring "swap" false-positive.) So the new `Staging` target configuration's xcconfig (`Staging.xcconfig`) must itself `#include` the relevant `Pods-iosApp.*.xcconfig` (or the implementer sets `ASSETCATALOG_COMPILER_APPICON_NAME` per-configuration directly in the target `buildSettings`) for the value to actually resolve at the target level. Either mechanism is acceptable; the `xcodebuild -showBuildSettings | grep APPICON` check (tasks.md §3.5) is the mechanism-agnostic gate that proves the resolved value per configuration.
