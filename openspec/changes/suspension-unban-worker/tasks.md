...
- [ ] 1.3 Add `com.auth0:jwks-rsa` + `com.auth0:java-jwt` to `gradle/libs.versions.toml` with current stable versions (per `design.md` D10). EXCEPTION: if `gradle/libs.versions.toml` or any existing `build.gradle.kts` already depends on `com.nimbusds:nimbus-jose-jwt` for the Supabase HS256 verifier path, switch to that lib instead — using one JWT lib across the codebase beats two. Verify by `grep -r "nimbusds\|auth0" gradle/ build-logic/ **/build.gradle.kts`. Document the chosen versions in `docs/09-Versions.md` per the version-pinning policy.
...
- [ ] 9.27 Test: route-level JWKS rotation — mount the route with a `OidcTokenVerifier` whose JWKS cache initially does not contain a specific `kid`. Issue a JWT signed with that `kid`. Verify the route returns `200` (path D10 D7 case: refresh resolves) AND verify a SECOND call with a JWT whose `kid` is still missing post-refresh returns `401 invalid_token`. This complements unit-test 3.5 with the route-level integration shape.
