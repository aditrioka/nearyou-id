...
**D5 — Read open to any admin role; write restricted to `owner`/`admin`.** Matches the monitor-read precedent (reads are broadly visible) and the chat-redaction write precedent (`role IN ('owner','admin')`). CSRF is enforced on the write only (reads are idempotent GETs).
...
Critical-invariant conformance: admin-module raw read of `users`/`subscription_events` is exempt from the `visible_*` view + block-exclusion-join lint rules (annotated per the privacy-flip-monitor precedent); `admin_actions_log` writes are append-only (never UPDATE/DELETE); all user-controlled output HTML-escaped; no secret reads; no `:infra:*` vendor import.
