...
- [ ] 1.4 Match the admin read-only-monitor lint posture for the raw `users` / `subscription_events` read (mirror `AdminPrivacyFlipsRepository`): the admin module is exempt from the `visible_*` views, and neither `BlockExclusionJoinRule` nor `RawFromPostsRule` fires on `FROM users` / `FROM subscription_events` — so **no functional block-exclusion annotation is required**. Add only a descriptive KDoc note of the admin exemption; add a functional lint annotation ONLY IF a rule actually fires at build time.
...
- [ ] 5.2 Filter + pagination + count tests: `q` by username; `store` filter; filters compose; count reflects full population independent of page limit; keyset paging drops/duplicates nothing; `q`/`store` with SQL metacharacters bound as a literal (no injection; table unaffected); blank/whitespace `q` ignored (full population listed, not an empty-username match).
...
- [ ] 5.5 Guard tests: read-only role cannot expedite; CSRF mismatch → 403 + `admin_csrf_violation` + no write; a CSRF- or role-rejected attempt leaves the expedite counter untouched (writes no `subscription_grace_expedite` row).
...
- [ ] 5.8 Expedite target-state tests: expedite against a `premium_active`/`free` user → rejected, no `admin_actions_log` row, no mutation; expedite against a soft-deleted (`deleted_at IS NOT NULL`) or unknown `{user_id}` → rejected, no row, no mutation.
