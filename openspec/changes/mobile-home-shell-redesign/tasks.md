...
- [ ] 10.1 Update/add Robolectric shell/host tests: Material icons present (not dots), selected nav/tab label visible, swipe changes tab + no re-fetch, tap animates pager, icon-only FAB present on Home only + absent on Notifikasi/Profil, the empty-state `onSeeGlobal` CTA switches to the Global tab via the host. Add the EXACT new/renamed `*ScreenTest` globs to the `mobile/app/build.gradle.kts` Release-variant exclude list (e.g. `**/AppShellScreenTest*`, `**/HomeScreenTest*`, and any renamed host test) alongside the existing entries; verify with `:mobile:app:testDevReleaseUnitTest`.
- [ ] 10.2 Update `NearbyTimelineScreenTest` / `GlobalTimelineScreenTest`: no header node, list fills, initial-load skeleton (one indicator, PTR `isRefreshing=false`), refresh keeps content + PTR `isRefreshing=true`, Global `Empty` renders the `timeline_loading` skeleton, and pull-to-refresh works from the empty/error state (non-Content states are scrollable). Assert "which feed is on screen" via the feed list **test tags** (`NEARBY_TIMELINE_LIST_TAG`; add/confirm a Global equivalent) — NOT the removed header. Use `waitUntil` polling for real-flow async (waitForIdle won't await MockEngine submit).
- [ ] 10.3 commonTest projection tests (`NearbyTimelineUiStateTest` / `GlobalTimelineUiStateTest`): `isInitialLoad` → Loading; retained Loaded + `isInitialLoad=false` → Content; refresh-while-empty (retained `Loaded(empty)` + `isInitialLoad=false`) → Empty (not skeleton); VM reload toggles `isRefreshing` not `isInitialLoad` and retains the outcome. Add serializable round-trip tests for BOTH the `Tab` enum AND the `Section` enum (iOS-safe).
- [ ] 10.3b Static source-scan guard tests (strip comments first per 10.5): exactly one `Scaffold` in the authenticated tree (only `AppShellScreen`); `HomeScreen` + timeline content declare no `Scaffold`/`TopAppBar`; nav/tab items use `painterResource(Res.drawable.*)` not `CircleShape` dots; item colors come from `NavigationBarItemDefaults`/default `Tab` color (no custom color resolving to the background).
- [ ] 10.4 iOS flow test under `mobile/app/src/iosTest/...` (K/N-legal test-function names — no `,()#`; iosTest placement, NOT commonTest): shell + Home tabs + swipe on the simulator; run `:mobile:app:iosSimulatorArm64Test`. Verify `rememberPagerState` + the serializable `Tab`/`Section` survive process death on K/Native.
