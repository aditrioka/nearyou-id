...
- Raw `posts`/`users` writes occur only inside the internal-worker service path and carry the `RawFromPostsRule` allowlist annotations (admin/internal context).

## Risks / Trade-offs

- **Migration V29 contention** (4 parallel sessions also add migrations — #353/#354/#355/#356) → the migration is additive + isolated (one new table), so if a sibling lands first, `git mv` to the next free `V<N>` + bump the in-file/docs refs pre-merge (the documented parallel-Flyway-collision fix). No rebase of logic needed.
...
1. **`V29__csam_detection_archive.sql`** — additive new table (schema per the `csam-detection` spec); renumber to the next free `V<N>` if a sibling migration squash-merges first. No down migration (project convention).
