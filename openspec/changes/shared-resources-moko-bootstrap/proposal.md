...
- Verify the "no hardcoded UI strings" mobile convention via **explicit grep step in tasks.md** (NOT via a Detekt rule — Mobile #1 deferred the rule as `mobile-negative-requirement-ci-grep` in `FOLLOW_UPS.md`; the rule still doesn't exist in `:lint:detekt-rules`). This change ships a grep-based assertion that `:mobile:app` UI strings flow through Moko Resources; upgrade to a real Detekt rule is tracked as a separate follow-up.
