...
- `backend/ktor/src/main/kotlin/id/nearyou/app/admin/auth/` (new directory) — as-shipped filenames: `AdminAuthPlugin.kt` (`AdminAuthProvider` session middleware), `AdminCsrfGate.kt` (`object AdminCsrfGate` CSRF validation, called per-handler — not a Ktor plugin), `AdminLoginRoutes.kt` (GET + POST login handlers), `AdminLogoutRoute.kt` (POST logout handler), `PasswordHasher.kt` (Password4j wrapper), `TotpVerifier.kt` (samstevens java-totp wrapper), `AesGcmCipher.kt` (JCA AES-256-GCM wrapper), `HashUtil.kt` (SHA-256/hex + constant-time compare + HMAC CSRF derivation), `InetSanitizer.kt` (INET-literal guard), `AdminPrincipal.kt`, `AdminAuditLogger.kt` (audit row insert helper), `SessionRepository.kt` (admin_sessions JDBC queries), `AdminUserRepository.kt` (admin_users JDBC queries). Plus `dev/AdminBootstrapMain.kt` (operator CLI).
