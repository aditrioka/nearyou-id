...
**CSRF token storage uses the same shape:** SecureRandom 256-bit → base64url → returned to client + stored as SHA-256 in `admin_sessions.csrf_token_hash` (the schema-layer NOT NULL invariant from V16 enforces population).
