...
- [ ] 3.1 Add a pure resolver mapping `(type, target_type, target_id, actor_user_id, body_data)` → a typed nav intent: `Post(target_id)`, `Profile(actor_user_id)`, `ChatMessage(conversationId, actorUserId)`, or `None` (`chat_message_redacted` [actor NULL] + reply-target + informational + unknown/missing-field). Unit-testable, PII-free.
...
- [ ] 3.3 For `ChatMessage`, fetch the partner via `ProfileApiClient.getProfile(actorUserId)` (the sender = the 1:1 partner) → set `pendingNavTarget` to `OpenChatThread(conversationId, username, displayName)`; on profile-fetch failure set `OpenChatThread(conversationId, "", "")` (the conversation is valid; the top bar degrades to its blank-name placeholder). For `Profile`, set `OpenProfile(actor_user_id)` with no fetch.
- [ ] 3.3a Expose `pendingNavTarget` as a **nullable, consumed-once field on the `StateFlow` UiState** cleared by a VM `onNavConsumed()` callback — the established `EditPostUiState` one-shot-signal pattern; **NO `Channel`/`SharedFlow`** (forbidden by docs/11 § 2.2) — so navigation does not re-fire on recomposition.
