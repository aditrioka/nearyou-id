...
- Kotlin time seam: `kotlin.time.TestTimeSource` is **stable as of Kotlin 2.3** (we pin 2.3.20) — so the injected `TimeSource` seam (D3) and its test use it with **no `@ExperimentalTime` opt-in**. The decorator itself uses only the long-stable `TimeSource` / `TimeMark` / `markNow()` / `elapsedNow()` APIs.
