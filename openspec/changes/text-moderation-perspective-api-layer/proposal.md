...
- **NEW** `PerspectiveModerator` orchestrator in `:backend:ktor` (`id.nearyou.app.moderation`) — checks `perspective_api_enabled` via the existing `RemoteConfigClient`, invokes `PerspectiveClient` with a 500ms `withTimeoutOrNull` budget, applies the score-threshold mapping (>0.8 → auto-hide + queue; 0.6–0.8 → queue only; ≤0.6 → no action), writes to `moderation_queue` with `trigger = 'perspective_api_high_score'`, and (high-score only) flips `posts.is_auto_hidden = TRUE` (or `post_replies.is_auto_hidden`) in the same SQL transaction as the queue insert.
