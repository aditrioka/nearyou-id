...
- [ ] 12.2 Extend the existing `vendor-sdk-leakage-scan` capability (per [`openspec/specs/vendor-sdk-leakage-scan/spec.md`](../../../specs/vendor-sdk-leakage-scan/spec.md), the canonical home for "no vendor SDK in `:backend:ktor`" lint) to add Perspective endpoint substrings to its denylist: `"commentanalyzer.googleapis.com"` and `"v1alpha1/comments:analyze"`. If this requires a MODIFIED delta on the `vendor-sdk-leakage-scan` capability spec, add it to the proposal — otherwise extend the existing test class. **Decision at implementation time**: if extending requires a spec MODIFY, fold it into this change's proposal as a third capability MODIFIED; if it's a pure test-class extension (no spec change), do it inline
- [ ] 12.3 (If 12.2 requires spec MODIFY) — add a `vendor-sdk-leakage-scan` MODIFIED block to `openspec/changes/text-moderation-perspective-api-layer/specs/vendor-sdk-leakage-scan/spec.md` per OpenSpec format conventions, OR scope-down to "extend the existing test class only" if the spec doesn't enumerate per-vendor strings
