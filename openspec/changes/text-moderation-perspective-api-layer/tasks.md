...
- [ ] 2.1 Create `infra/perspective/build.gradle.kts` matching the `:infra:fcm` / `:infra:remote-config` shape (Kotlin/JVM, ktor-client-core + ktor-client-cio + ktor-client-content-negotiation + kotlinx-serialization-json deps; depend on `:core:domain` for the `SecretResolver` interface)
...
- [ ] 2.7 Implement `KtorPerspectiveClient` with: API-key resolution via `secretResolver.resolve("perspective-api-key")`; `secretKey(env, "perspective-api-key")` for the diagnostic init log line; Ktor client (CIO engine, Content-Negotiation with kotlinx.serialization JSON); POST to `https://commentanalyzer.googleapis.com/v1alpha1/comments:analyze?key=<api-key>` with the canonical `{"comment": {"text": "..."}, "requestedAttributes": {"TOXICITY": {}, "SEVERE_TOXICITY": {}, "IDENTITY_ATTACK": {}, "THREAT": {}}}` body
...
- [ ] 2.9 Unit tests for `KtorPerspectiveClient`: happy-path JSON parse against a `MockEngine`, malformed-JSON parse error, HTTP 4xx, HTTP 5xx, network-exception path, API-key URL-encoding correctness
...
## 6. `PerspectiveModerator` orchestrator + threshold loaders
...
- [ ] 6.3 Implement threshold loaders: `loadHighScoreThreshold()` / `loadFlagThreshold()` reading from `RemoteConfigClient.fetchDouble(...)` (the new method from Section 5) for `perspective_api_high_score_threshold` (default 0.8) and `perspective_api_flag_threshold` (default 0.6); apply `[0.0, 1.0]` clamp on EVERY read (cache + Tier 2); emit Sentry WARN with `event = "perspective_threshold_fallback"` on out-of-range
- [ ] 6.4 Implement kill-switch read: `RemoteConfigClient.fetchBoolean("perspective_api_enabled")` (default TRUE); fail-OPEN to enabled=true on Remote Config exception with Sentry WARN `event = "perspective_kill_switch_unavailable"`
- [ ] 6.5 Implement `moderate(targetType, targetId, content): Outcome` — kill-switch check → `withTimeoutOrNull(500.ms) { client.analyze(content) }` → score aggregation `max(toxicity, severeToxicity, identityAttack, threat)` → threshold mapping → DB write (transactional) for AutoHide / FlagOnly → return Outcome
- [ ] 6.6 Wrap every PerspectiveClient invocation in try/catch capturing all failure kinds → emit Sentry WARN with `event = "perspective_dispatch_failed"`, populate `failure_kind` ∈ `{timeout, http_4xx, http_5xx, network, parse}`, populate `status_code` for HTTP failure kinds → return `Outcome.NoAction`
- [ ] 6.7 Verify the orchestrator NEVER throws to its caller (every code path returns an `Outcome`); add a contract-level test that asserts no-throw across all failure-mode fixtures

## 7. DB write helpers — transactional auto-hide + queue insert

- [ ] 7.1 Add JDBC helper `applyAutoHideAndQueue(targetType, targetId, score)` that opens a transaction, runs `UPDATE posts SET is_auto_hidden = TRUE WHERE id = ?` (or `UPDATE post_replies ...`), runs `INSERT INTO moderation_queue (target_type, target_id, trigger, status, priority, ...) VALUES (..., 'perspective_api_high_score', 'pending', ?, ...) ON CONFLICT (target_type, target_id, trigger) DO NOTHING`, COMMITs
- [ ] 7.2 Add JDBC helper `applyFlagQueue(targetType, targetId, score)` that runs the queue INSERT only (no flip), same ON CONFLICT clause
...
- [ ] 7.4 Integration tests against a real Postgres (Testcontainers / `KotestProjectConfig` Docker setup) — write helper twice for the same target → exactly one queue row (idempotency); concurrent helper invocations from two transactions → exactly one queue row (race-safe via UNIQUE constraint)
- [ ] 7.5 Atomicity test: simulate the queue INSERT failing mid-transaction (e.g., via a triggered exception) → assert the `is_auto_hidden` flip is rolled back (the row remains visible, no half-applied state)

## 8. `PerspectiveDispatcherScope` lifecycle (mirror `:infra:fcm` `FcmDispatcherScope`)

- [ ] 8.1 Define `PerspectiveDispatcherScope` class wrapping a `CoroutineScope(SupervisorJob() + Dispatchers.IO + CoroutineName("perspective-dispatch"))`
- [ ] 8.2 Implement `shutdown(drainMillis: Long = 5_000)` using `withTimeoutOrNull(drainMillis) { scope.coroutineContext[Job]?.cancelAndJoin() }`; emit a structured WARN log line listing cancelled-mid-dispatch coroutine count if the budget elapsed
- [ ] 8.3 Implement `launch { ... }`-style API that detects post-shutdown invocation and emits `event=perspective_dispatch_after_shutdown` WARN log + silent no-op (mirror `FcmDispatcherScope` `dispatchAfterShutdown` pattern)
...
- [ ] 8.5 Tests using `kotlinx-coroutines-test` `runTest` + `TestScheduler.advanceTimeBy` for deterministic timing — sibling-isolation test (D1 throws → D2 still completes), drain-completes-within-budget test, drain-exceeds-budget test (cancelled + WARN), dispatch-after-shutdown test

## 9. Wire async dispatch at `posts` and `post_replies` INSERT call sites

- [ ] 9.1 Add `perspectiveDispatcherScope.launch { perspectiveModerator.moderate(POST, newPostId, content) }` to [`backend/ktor/.../post/CreatePostService.kt`](../../../backend/ktor/src/main/kotlin/id/nearyou/app/post/CreatePostService.kt) immediately AFTER the INSERT commit, BEFORE the service returns
- [ ] 9.2 Add the analogous launch invocation with `REPLY` target type to [`backend/ktor/.../engagement/ReplyService.kt`](../../../backend/ktor/src/main/kotlin/id/nearyou/app/engagement/ReplyService.kt) immediately AFTER the INSERT commit
- [ ] 9.3 Add a structural source-scan test (`PerspectiveDispatchCallSiteTest` mirroring `PostCreationCallOrderTest` / `ChatModerationCallOrderTest`) asserting: (a) the launch invocation appears AFTER the canonical INSERT call in each service; (b) chat path does NOT contain the launch invocation
- [ ] 9.4 Verify chat handler explicitly does NOT carry the Layer 3 dispatch (deferred per design Open Question 1)
...
- [ ] 11.1 Sentry-capture fixture that intercepts emitted events; for each `event = "perspective_*"` event captured during test runs, assert the event payload's serialized JSON does NOT contain the substring `"sentinel-content-DO-NOT-LEAK"` when test content is exactly that sentinel
- [ ] 11.2 Assert raw per-attribute scores (e.g., `"0.85"`, `"0.40"`) do NOT appear in Sentry payloads — only `score_bucket` (`"low"` / `"mid"` / `"high"`) appears
- [ ] 11.3 Assert at most ONE Sentry event is emitted per `moderate(...)` invocation (no event spam during cascade)

## 12. Static-source-scan defense (parallel to existing `PostCreationCallOrderTest` etc.)

- [ ] 12.1 New test `PerspectiveDispatchCallOrderTest` asserting the call order at the post + reply handler sites: synchronous moderate → INSERT → `perspectiveDispatcherScope.launch`
- [ ] 12.2 New test `BackendNoDirectPerspectiveImportTest` asserting `:backend:ktor` source files contain zero references to the literal string `"commentanalyzer.googleapis.com"` or `"v1alpha1/comments:analyze"` (encapsulation invariant)

## 13. Canonical-doc amendments shipped in this change

These docs are amended IN THIS CHANGE because the change introduces the surface they describe (new Remote Config flags, new attribute-aggregation interpretation). Per the canonical-docs reconciliation pass, NEW surfaces are documented in canonical docs as part of the change that introduces them — not deferred to a follow-up.
