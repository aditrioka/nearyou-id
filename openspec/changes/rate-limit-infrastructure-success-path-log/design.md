...
### Decision 1: Log level → DEBUG, with explicit `RedisRateLimiter` logger pin to INFO so the new log shape is opt-in

**Logback posture.** [`backend/ktor/src/main/resources/logback.xml`](../../../backend/ktor/src/main/resources/logback.xml) declares `<root level="trace">` with a few project-specific loggers pinned to INFO (Jetty, Netty, Firebase, Google client). This change ALSO pins `id.nearyou.app.infra.redis.RedisRateLimiter` to INFO — without this pin, the new per-call `event=rate_limit_check` DEBUG log would fire on every admit attempt (one Cloud Logging entry per request that touches a rate-limited path), which is operationally expensive at production scale.

**Choice rationale: DEBUG-emit + INFO-pin = opt-in.** The success-path log is emitted at DEBUG (the right semantic level for "per-call mechanical trace") AND filtered out by the appender's effective INFO threshold by default (because the `RedisRateLimiter` logger is pinned to INFO, the Logback inheritance chain resolves to INFO regardless of root=trace). When operators triage a hot-key incident, they raise the logger to DEBUG via a logback config edit + branch-redeploy — same workflow as any per-logger level adjustment in this codebase today. The failure-path WARN logs (`event=redis_acquire_failed`, `event=redis_connect_failed`) continue to fire regardless of the pin; those carry the operator signal for fail-soft + EVALSHA failures.

**Cloud Logging volume.** With the pin, baseline cost is zero (DEBUG entries are filtered before reaching the appender). The §6 pre-archive smoke ran SUCCESSFULLY against the pre-pin state (root=trace alone, no RedisRateLimiter pin) and captured 9 `event=rate_limit_check` entries in steady state — proving the runtime log shape is correct and the spec scenario "tryAcquireByKey omits userId from telemetry" is non-vacuous. After the pin lands in this change, future smoke runs require either a temporary level flip in logback.xml + redeploy, OR rely on the `:infra:redis` integration tests (which explicitly bump level to DEBUG via `captureDebugAndWarnings`).
...
**Rejected alternative**: invent a `KTOR_RATE_LIMIT_DEBUG` env var that flips the logger level at runtime. The current `application.conf` / logback.xml config does NOT support env-var-driven per-logger overrides, and adding that mechanism is scope creep beyond the single-capability MODIFY of this change. Edit-and-redeploy is the realistic operator workflow.

**Rejected alternative (initial draft)**: ship the log without the pin and rely on operators to remember to pin before production cutover. The Phase B reconciliation pass + round-1 multi-lens review surfaced this as an unresolved Open Question Q3; this change's V2 round-2-fix iteration shipped the pin inline because the scope addition is one XML line and avoids a deferred follow-up that could rot in `FOLLOW_UPS.md` indefinitely.
...
**Test runtime cost.** Six new database-tagged scenarios (user-axis success, key-axis success without user_id, user-axis rate-limited, key-axis rate-limited, DEBUG-filtered non-emission, cross-test pollution check) run against the CI `redis:7-alpine` service container, plus one fail-soft scenario that uses the unreachable-URL precedent (no real Redis), plus one source-level structural test (no real Redis). Adds a few hundred ms to the `:infra:redis:test` lane. → Trade-off accepted: integration-grade telemetry coverage requires real Redis; the alternative (mocked Redis) doesn't catch format-string bugs.
...
- **Q3 (defensive logger pin before production)** — **RESOLVED in this change**: pinned `<logger name="id.nearyou.app.infra.redis.RedisRateLimiter" level="INFO"/>` in `logback.xml` per the user's "fix it now" decision after the §6 smoke succeeded. The earlier "deferred to logback-refactor" framing is dropped: pinning a single logger by name is one line of XML, doesn't depend on a project-wide refactor, and avoids a follow-up that could rot in `FOLLOW_UPS.md` indefinitely. A broader logback refactor (root=info + per-package pins) is still a separate, larger concern — but no longer a prerequisite for this change's posture being correct.
