...
- [x] 6.2 Monitor the deploy run via `gh run list --workflow=deploy-staging.yml --limit=1` until it reports green. Capture the deployed Cloud Run revision name from the run's logs (or via `gcloud run services describe nearyou-api-staging --region=asia-southeast2 --format='value(status.latestReadyRevisionName)'`).
