...
A new `AreaPostDensityLimiter` (under `id.nearyou.app.post`) **delegates to the shared `RateLimiter`** exactly as `PostRateLimiter` does — same Redis-backed-custom pattern, **no second counter mechanism** (Pattern Registry §3.3 conformant). It calls the limiter with the **geocell as the rate-limit key**, capacity **50**, window **1 h**. The limiter's `RateLimited` outcome is mapped to an `OverThreshold` result, which the service turns into a **`moderation_queue` write — never a 429**. Posts 1–50 in the cell/window are clean; the 51st onward (`count > 50`) are flagged.

- **No per-user stagger**: this is an *hourly*, *geocell-keyed* limit — `computeTTLToNextReset` (the per-user WIB-midnight daily stagger) deliberately does NOT apply, per docs/05's own "hourly limits skip the stagger". The `RateLimitTtlRule` enforces the stagger only at *daily* endpoints, so this is conformant, not an exception.
...
- **§3.1 layering**: `CreatePostService` (business rule + tx boundary) gains the gate; `AreaPostDensityLimiter` is a service-layer collaborator delegating to the shared `RateLimiter` (mirrors `PostRateLimiter`). Routes/DTOs unchanged; no SQL in the route.
...
- **Cell-boundary spam** (a spammer straddling two cells gets 2× headroom) → Mitigation: 500 m jitter already diffuses clusters toward the source cell; 100 posts to game two cells still trips the per-user daily cap and is itself anomalous. Accepted at MVP; the PostGIS-radius upgrade remains a future option if telemetry warrants.
