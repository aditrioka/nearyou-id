...
- [ ] 6.3 `ImageRoutes` (thin): `POST /api/v1/images` multipart parse with a **streamed** size cap (abort once >5 MB — no full-part buffering), authenticate, `call.clientIp` for limiter keys, map service results → 201 / 403 / 413 / 415 / 422 / 429 / 503 with the existing error envelope. Wire into Koin + route registration.
