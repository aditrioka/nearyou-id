...
**D2 — The Premium gate is owned by the route-scoped screen, NOT the Settings row (revised after sub-agent review).** Review surfaced that `SettingsScreen` holds **no** Premium signal today (it injects only `SettingsViewModel(tokenStore)`), so a Settings-side proactive `isPremium` branch would be unstated net-new plumbing. Resolution: the "Ganti username" row pushes `UsernameCustomizationRoute` **unconditionally**, and the route-scoped `UsernameCustomizationViewModel` resolves Premium on entry via the existing stateless self-profile read (`ProfileFlow.loadProfile(selfUserId)`, `selfUserId` from `SelfUserIdProvider`) → renders the `PremiumGate` as the initial state for a not-Premium self-read, the editor otherwise. This honours docs/03 §114 ("Free taps → paywall") with the gate appearing immediately (no flash), keeps Settings dumb, and puts the one profile read where a route-scoped fetch is natural. The **authoritative** gate remains the reactive `403 premium_required` from a probe/submit — the backstop for constraint 1's `premium_billing_retry` mismatch (`isPremium` is `premium_active`-only) and for staleness; a self-read failure degrades to letting the user attempt the action (the `403`/`200` then governs), never an error wall. Alternative (Settings reads `isPremium` itself) rejected — it adds a profile fetch + an `onActivatePremium` param to Settings for one row; the screen-owned gate is cleaner and removes the phantom seam.
...
**D7 — Submit-confirmation modal gates the destructive `PATCH`** (docs/03 §133); success → toast + pop to Settings, modeled as a one-shot nullable UiState field per docs/11 §2.2 (no event-bus). New-handle propagation (docs/03 §134 "the profile reloads") relies on the **stateless** `ProfileFlow` re-fetching on the next read of the profile/settings surface — review confirmed there is **no** observable cross-screen self-profile cache to invalidate, so this change neither asserts nor introduces a cache-invalidation seam (the earlier "refresh the self ProfileFlow" framing was an overclaim; corrected).
