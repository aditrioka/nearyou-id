...
- **Gated write action** per flag: HTMX `hx-confirm` + a mandatory free-text **reason** → publishes the new value to the **Server** template → writes exactly one immutable `admin_actions_log` row (`action_type = 'feature_flag_toggled'`, `before_state`/`after_state`, `reason`). CSRF-gated; **role-gated** — all writes require `owner`/`admin` (the security-touching `attestation_mode` is explicitly within that; the mockup pins it, and config writes are operator-tier — see design D4); writes are capped at 5/hour/admin.
...
- **`:infra:remote-config` gains a write/publish method** behind the existing interface seam (Firebase vendor types stay inside the module; plain Kotlin types cross to `:backend:ktor`). Uses the **Remote Config REST API** against the `firebase-server` namespace (`GET` template+ETag → patch the one parameter → `PUT` with `If-Match`) with **ETag optimistic concurrency** so a concurrent publish is rejected (`412`), not silently clobbered — the Admin SDK cannot publish the server template the backend reads. The OAuth token comes from the existing `firebase-admin-sa` credentials + the JDK HTTP client → **no new dependency / no new library pin**.
