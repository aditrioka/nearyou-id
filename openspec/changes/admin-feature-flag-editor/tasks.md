...
- [ ] 1.3 Confirm the `:infra:remote-config` read seam shape (`RemoteConfigClient`, `FirebaseServerConfigSource`) and the `DestructiveActionRateLimiter` / `AdminAuditLogger` conventions to extend.
...
- [ ] 2.2 Add `publishServerParameter(key, value, expectedVersion): PublishResult` to the public interface, implemented via the Firebase Admin SDK Remote Config management API (`getServerTemplate` → set the one parameter → `publishTemplate`), keeping all vendor types module-internal.
...
- [ ] 4.2 Role-gate: writes require `owner`/`admin` (design D4) — `moderator` → 403, no publish, no audit (spec R5). GET available to any authenticated admin; write controls disabled in the render for non-owner/admin.
- [ ] 4.3 Feature-flag rate limiter: count `feature_flag_toggled` rows in the trailing hour, `FEATURE_FLAG_TOGGLE_CAP = 5`; at/over cap → reject with no publish + no audit (spec R6, design D3). Distinct bucket from the 20/hour destructive cap.
...
- [ ] 6.3 Gate tests — blank-reason reject, no-op reject (R3); CSRF missing/mismatch (R4); moderator reject + owner/admin allow (R5); 6th-in-hour rate-limit reject + bucket independence from destructive cap (R6).
- [ ] 6.4 Validation tests — threshold 0/10001/non-integer reject, 50 accept (R7); `attestation_mode` unknown value reject (D7).
...
- [ ] 8.1 Operator task (out-of-band, NOT a code blocker — staging first): grant the admin Cloud Run runtime SA a Firebase role with Remote Config write scope on the bound project; document the slot/role in `docs/07` § Data Access Pattern / Secret Management Runbook. Until provisioned, the panel is read-only via graceful degradation.
