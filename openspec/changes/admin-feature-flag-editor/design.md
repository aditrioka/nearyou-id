...
Add a publish method (e.g. `publishServerParameter(key, value, expectedVersion): PublishResult`) to the `:infra:remote-config` public surface, implemented with the Firebase Admin SDK Remote Config management API (`getServerTemplate()` → set the one parameter → `publishTemplate(...)`). All Firebase vendor types (`Template`, `Parameter`, `TemplateResponse`) stay inside the module; `:backend:ktor` sees plain Kotlin types and a sealed `PublishResult` (`Published(newVersion)` / `StaleVersion` / `WriteUnavailable` / `Failed`). **Alternative rejected:** publishing from `:backend:ktor` directly — violates the "no vendor SDK import outside `:infra:*`" invariant and duplicates the read-seam pattern.
...
`GET` captures the Server-template `version` (etag) and embeds it in each write form. The publish passes it as `expectedVersion`; the Admin SDK's template-version check (or a re-read-and-compare) rejects a publish whose base version has advanced, yielding `StaleVersion` → a "reload and retry" prompt. This prevents a second admin's render-then-publish from silently clobbering a concurrent change. No audit row is written on a stale rejection.
...
- **Operator step (out-of-band, staging first):** grant the admin Cloud Run runtime service account a Firebase role with Remote Config write scope on the bound project; until then the panel is read-only via D5.
...
- **Role breadth (D4):** owner/admin for *all* writes is the safe default; the mockup only explicitly pins `attestation_mode`. If the operator wants `moderator` to flip a "safe" subset (e.g. `search_enabled`), that's a deliberate widening for a later change — flagged for reviewer confirmation.
