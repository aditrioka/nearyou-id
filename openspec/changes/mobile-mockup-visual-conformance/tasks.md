...
- [ ] 3.5 Update `SignInScreenTest`: replace the title-node assertion with the disclosure-present + title-absent pair per the modified scenario (note: the test hardcodes `TITLE = "Masuk ke NearYouID"` as a constant rather than referencing the key — a key-name grep misses it)

## 4. Composer (frame 6)

- [ ] 4.1 `PostCreationScreen`: add the location chip — `locationPinContainer` container, `onLocationPinContainer` label (`post_create_location_chip`), `ic_post_location` glyph tinted `locationPin`; no coordinate/city/location-derived value rendered
...
- [ ] 4.4 Update `PostCreationScreenTest`: chip + privacy-note presence (static copy, no coordinate), counter bottom-bar placement via bounds comparison (the `AppShellScreenTest` bounds-math idiom), no-attachment-toolbar negative guard; existing CTA scenarios unchanged
