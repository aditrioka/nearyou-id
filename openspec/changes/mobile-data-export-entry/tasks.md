...
- [ ] 5.2 Robolectric `SettingsScreenTest` additions — row → dialog → `POST` recorded; cancel → nothing; `processing` → in-progress + no second `POST`; `ready` → banner with deadline + download affordance; `failed` → re-request issues a new `POST`; `401` on **both** the `POST` and the `GET` read → sign-in. Ensure `SettingsScreenTest` stays in the Release-variant test-exclude list in `mobile/app/build.gradle.kts`.
