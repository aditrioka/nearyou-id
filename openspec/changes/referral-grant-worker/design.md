...
**Stacking is computed by us, not delegated to RC's native behavior** (more robust than relying on RC's near-duplicate heuristic): the new entitlement end is `max(current_entitlement_end, NOW()) + INTERVAL '7 days'` — extend-if-active, fresh-1-week-if-free/lapsed (docs/01 §139–140) — passed as the grant's absolute `end_time_ms`. The recipient's `current_entitlement_end` is the `GREATEST` of their latest `subscription_events` window AND the worker's own already-written `granted_entitlements` rows — including the latter so a user who earns two grants in one run (an invitee who also hits their 5th-referral inviter milestone) stacks the second off the first's ledger row, before its `GRANT` echo (which writes `subscription_events`) arrives. A `dedup_key` (`<referral_ticket_id>:<grant_role>`) is the our-side idempotency token (the v1 endpoint has no dedup param).
...
_Both resolved at apply time (2026-06-20):_

- **Exact RevenueCat v1 promotional-grant request shape** — RESOLVED: the v1 endpoint `POST /v1/subscribers/{app_user_id}/entitlements/{entitlement_id}/promotional` takes an absolute `end_time_ms` (dated re-check, tasks 1.1); our computed extend-by-7-days absolute end is passed there. `dedup_key` is our-side idempotency only (no v1 param).
- **`subscription_events` entitlement window on the `GRANT` echo** — RESOLVED: the webhook GRANT handler records the event's `entitlement_start`/`end` via the existing `insertEventIfNew` path (same as the paid events); the worker stacks off `GREATEST(subscription_events, granted_entitlements)`, so the stacking does not depend on echo timing.
