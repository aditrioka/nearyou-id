...
**Deferred (negative-guard requirements):** the ≥ 3-login-days and ≥ 5-app-sessions engagement legs (docs/01 §212) and the IP-subnet (/24) + recently-seen-identifier anti-collision legs (docs/01 §213 item 3). Rationale: no durable server-side source today (docs/05 §1186). The residual abuse exposure is bounded by the **already-shipped** signup-time `device_fingerprint_hash` collision check + the per-inviter ≤ 3/7-day burst limit, and each grant is worth only 1 week of Premium. Captured as spec requirements so a follow-up has a positive target to MODIFY (the `feedback_defer_as_explicit_requirement` pattern).
