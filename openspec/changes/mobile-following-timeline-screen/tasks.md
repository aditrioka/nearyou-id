...
- [ ] 1.2 Create `timeline/FollowingTimelineRepository.kt` + sealed `FollowingTimelineOutcome` (`Loaded(posts, nextCursor, upsell)` / `SessionExpired` / `Error` / `NetworkError`) — HTTP-status-driven mapping (200→Loaded; terminal 401→SessionExpired never NetworkError; 400 invalid_cursor→retryable Error + log; 5xx/IO→NetworkError; any other non-2xx→defined NetworkError fallback). No generic "load failed" copy; do not reimplement 401 refresh/retry. Map DTOs → the domain post model (no PII: no UUID, no raw coords surfaced to the card)
- [ ] 1.3 Create `timeline/FollowingTimelineFlow.kt` interface (the testable seam `FakeFollowingTimelineFlow` implements), mirroring `GlobalTimelineFlow`
...
- [ ] 3.2 Implement the **directive Empty state**: `timeline_following_placeholder` copy + a `cta_see_global` ("Lihat Global") control invoking `onSwitchToGlobal`, rendered inside a scrollable (so pull-to-refresh works from it) — NOT the loading-skeleton copy. Hard-cap state uses `timeline_limit_hard` (distinct); soft uses the `timeline_limit_soft` banner over the list
- [ ] 3.3 Wire the inline like affordance + reply shortcut through the ViewModel's shared-controller delegation; render `mobile-cap-upsell-dialog` with `post_detail_likes_cap_upsell` + countdown while the cap state is set; SessionExpired → `timeline_session_redirect` (no retry); NetworkError/Error → `signin_error_network` + `cta_retry`
- [ ] 3.4 DELETE `screens/timeline/FollowingPlaceholderScreen.kt`

## 4. Mobile — Koin wiring

- [ ] 4.1 In `di/MobileModule.kt`: register `single { FollowingTimelineApiClient(...) }` + `single { FollowingTimelineRepository(...) }` + bind `single<FollowingTimelineFlow> { get<FollowingTimelineRepository>() }`; resolve the EXISTING `SessionIdProvider` single (no second registration)

## 5. Mobile — home-tab-host integration

- [ ] 5.1 Repoint the Following pager page (page 1) in `screens/home/HomeScreen.kt` from `FollowingPlaceholderScreen` to `FollowingTimelineScreen`, hoisting `onOpenPost` / `onOpenPostReply` into it (identical to Nearby/Global) and wiring `onSwitchToGlobal = { scope.launch { pagerState.animateScrollToPage(<Global index>) } }`
- [ ] 5.2 In `screens/routing/AppEntryProvider.kt` / `screens/shell/AppShellScreen.kt`: forward `onOpenPost` / `onOpenPostReply` to the Following page exactly as for Nearby/Global (Following cards carry `distanceM = null`); no new `NavKey`, no per-tab `NavDisplay`

## 6. Tests

- [ ] 6.1 commonTest `FollowingTimelineUiStateTest` — the pure projection across all six states incl. the Following-specific directive empty state (deterministic, no wall-clock/platform dependency)
- [ ] 6.2 commonTest `FollowingTimelineViewModelTest` — load-once-on-construction, reload toggles `isRefreshing` not `isInitialLoad` + retains outcome, load failure → NetworkError, inline-like delegation to the shared controller (optimistic flip / RateLimited revert + cap state / PostGone revert + reload / NetworkError silent revert)
- [ ] 6.3 MockEngine `FollowingTimelineApiClient` / `FollowingTimelineRepository` tests — endpoint path with no spatial params + no first-page cursor, shipped mixed-case + distance-less wire parse, snake_case-only negative-regression guard, "no `distanceM`" assertion, reused `X-Session-Id` (equals the shared provider), `upsell` parse, full status→outcome mapping (incl. terminal-401→SessionExpired)
- [ ] 6.4 Robolectric `FollowingTimelineScreenTest` (androidUnitTest) — initial render + each of the six states via `FakeFollowingTimelineFlow` (incl. directive empty + "Lihat Global" CTA firing `onSwitchToGlobal`; no-UUID/no-coords assertion; SessionExpired redirect ≠ connectivity copy); ADD `**/FollowingTimelineScreenTest*` to the `mobile/app/build.gradle.kts` Release-variant exclude
- [ ] 6.5 iosTest Following-feed flow test mirroring `NearbyTimelineFlowIosTest` (K/N-legal names), exercising the load path on the simulator
- [ ] 6.6 Update the home-tab-host tests: REMOVE `FollowingTabNoFetchScanTest`; extend the no-re-fetch commonTest to include `FakeFollowingTimelineFlow` (Following fetch count stays 1 across swipe/section/composer round-trips); extend the Robolectric shell/host test so the Following page asserts the live feed (renders `FollowingTimelineScreen`, issues its fetch) instead of the placeholder; add the "Lihat Global" CTA → pager-to-Global scenario

## 7. Verification gates

- [ ] 7.1 Mobile gate: `./gradlew :mobile:app:testDevDebugUnitTest :mobile:app:testDevReleaseUnitTest` green (new `FollowingTimelineScreenTest` in the Release exclude) + `:mobile:app:iosSimulatorArm64Test` for the commonTest + iosTest additions
- [ ] 7.2 Lint gate: `./gradlew ktlintCheck detekt :lint:detekt-rules:test` green (no hardcoded-UI-string violations; no vendor-SDK-leak)
...
- [ ] 7.4 No staging deploy/smoke needed (no backend/runtime change) — mark Section 7.3 evidence as the UI DoD; note "no backend change" in the archive commit body
