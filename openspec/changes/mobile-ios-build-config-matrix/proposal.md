...
- **No icon assets change.** `AppIcon-Staging.appiconset`, `AppIcon.appiconset`, and the Android flavor `colors.xml` files from #155 are untouched — this change only reworks how iOS *configurations* resolve the existing icons.
...
- **Specs:** `openspec/specs/mobile-auth-signin/spec.md` (env-aware-config requirement).
- **No** `gradle/libs.versions.toml` change, no new dependencies, no backend/schema/API/security surface. Production end-user experience unchanged (the production icon + assets are byte-identical).
