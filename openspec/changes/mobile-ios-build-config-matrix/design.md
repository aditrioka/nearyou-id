...
**Apply-time discoveries (both fixed, verified by a real build 2026-06-06):**
1. **CocoaPods clobbers the anchor/relativePath base.** `pod install` could not read the objectVersion-77 `baseConfigurationReferenceAnchor`/`RelativePath` base (same blind spot as the `xcodeproj` gem) — it treated each target config as having NO base and **set the base directly to the Pods xcconfig**, dropping the per-config xcconfig (and its env values → every config resolved staging). Fix: after `pod install`, set each target config's `baseConfigurationReference` to a **real `PBXFileReference`** for its per-config xcconfig (done via the `xcodeproj` gem). CocoaPods then *detects* the custom base and respects it on future installs (it warns instead of overriding). The per-config xcconfig `#include`s the Pods xcconfig, so Pods still link.
2. **KMP cocoapods plugin can't auto-detect the build type from custom config names.** The `ComposeApp` framework sync failed with `Could not identify build type ... CONFIGURATION=Prod Release`. Fix (canonical, per the [Kotlin cocoapods DSL reference](https://kotlinlang.org/docs/native-cocoapods-dsl-reference.html)): add `xcodeConfigurationToNativeBuildType["<config>"] = NativeBuildType.DEBUG|RELEASE` for all 4 matrix configs in `mobile/app/build.gradle.kts`'s cocoapods block. This is a `build.gradle.kts` change (no `libs.versions.toml` pin, so no pre-impl library re-check).
