...
**D6 — Wiring.** `appleS2SRoutes(...)` gains `AccountDeletionRepository` (or a thin `AppleDeletionService`) + `AccountHardDeleteWorker` parameters, passed from `Application.kt` (both already constructed there: worker at `Application.kt:458`). Keep the route signature change minimal and the existing `InMemoryDedup` default intact.
...
- **Session-kick vs. restore-banner interaction for consent-revoke** → bumping `token_version` logs the user out, so the in-app restore banner is unreachable until they re-authenticate with Apple (which they just revoked). This is faithful to [`docs/06`](../../../docs/06-Security-Privacy.md):137 (both "cancellable" and "sessions kicked" are stated). Cancellation during grace therefore happens via re-auth or support, not a live in-app session. Flagged in Open Questions; documented, not silently resolved.
...
- **Re-bump `token_version` on a duplicate `consent-revoked`?** If a user already has a pending `consent-revoked` row and Apple re-sends (past the dedup window), the insert is a no-op — should `token_version` still increment? Default: **yes, bump on every consent-revoked receipt** (cheap, strictly safer for session revocation), but confirm at apply. Resolve before `/opsx:apply` finalizes the SQL.
- **Should `consent-revoked` also reuse the *existing* `requestDeletion` row if a `source='user'` pending row exists** (i.e., upgrade-in-place), or insert a distinct `apple_s2s_consent_revoked` row? Default: treat the existing pending-row guard as sufficient (no second row, keep the existing schedule) to honor the idempotent-request contract; the source of the surviving row is whichever was first. Confirm at apply.
