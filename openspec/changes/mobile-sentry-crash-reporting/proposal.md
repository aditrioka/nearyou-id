...
- **Scrub PII** via a `beforeSend` hook composing with the existing `CoordinateMaskingLogger` / coordinate-free discipline (mirrors `:infra:otel`'s `ForbiddenAttributeStripper`): no coordinates, tokens, or post bodies in crash payloads.
