...
`crash` defaults ON per docs/06 § Defaults. The settings toggle (`ConsentSettingsViewModel`) applies immediately: decline → `CrashReporter.close()` for the session; re-consent → re-init. On cold start, init ON (opt-out), then honor the last-known `crash` value read from the app's consent state holder (the source the consent settings screen renders), calling `close()` if declined. *Alternative:* gate init itself (don't init until consent confirmed) — rejected: opt-out semantics mean ON is the expected default, and gating init would lose early-startup crashes for the majority who never decline. The brief init-then-close window for decliners is acceptable and carries no event (nothing crashed in those milliseconds).

### Decision 7 — PII scrubbing via `beforeSend` composing with `CoordinateMaskingLogger`
A `beforeSend` hook strips/redacts coordinates, tokens, and free-text post/chat bodies from event payloads (message, breadcrumbs, extras), reusing the established coordinate-free discipline rather than a parallel redactor. Breadcrumbs sourced from `DiagnosticSink` are already coordinate-free by contract; `beforeSend` is the backstop for SDK-captured native context.
...
1. **Local consent persistence for cold-start gating** — confirm at apply time whether the app already persists the last-known `crash` consent locally (for the cold-start `close()` decision) or whether a lightweight local cache is needed. Does not change the spec; affects one wiring detail in Decision 6.
