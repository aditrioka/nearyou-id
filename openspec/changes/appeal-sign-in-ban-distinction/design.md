...
- **`token_version` mechanism claim corrected.** The MODIFIED `mobile-appeal` "Sign-in banned-state appeal entry" requirement inherited a claim that suspension "bumps `token_version`, revoking existing tokens." Review found this false for shipped code — `UserModerationRepository` suspends WITHOUT bumping `token_version`; the actual mid-session enforcement is the per-request `is_banned` gate in `AuthPlugin`. The requirement now keys its rationale on the per-request gate, and the `docs/06` § Suspension `token_version`-bump divergence is pointed at the #377 reconciliation (this change does not fix docs/06). Behavior is unaffected — the per-request gate 403s a suspended user regardless of `token_version`.
