...
- [ ] 1.1 Add `V19__reports_created_idx.sql` creating `CREATE INDEX reports_created_idx ON reports(created_at DESC, id DESC);` (plain `CREATE INDEX`, mirroring `V17__admin_actions_log_created_idx.sql` — no `CONCURRENTLY`). Confirm V19 is still the next free version at apply time (re-check `find … db/migration/V*.sql | sort`).
...
- [ ] 3.2 Parse query params (`cursor`, `status`, `target_type`, `reason_category`, `trigger`, `from`, `to`); malformed/absent cursor → first page (never error); wire to the repository; compute the "older" cursor only when a further page exists.
- [ ] 3.3 Wire the route into the admin Koin module + the admin route subtree; retire/replace the 31-LOC `moderation` admin-queue stub if it is now superseded.
...
- [ ] 5.6 Filters: `status` / `target_type` / `reason_category` each return only matching rows; date range bounds the whole "to" day; filters compose with AND.
...
- [ ] 5.9 `moderation_queue` context: report WITH a queue row shows trigger/priority; report WITHOUT one renders fine (no crash); a target with two triggers yields exactly ONE display row (no fan-out).
...
- [ ] 8.1 At archive: update `docs/07-Operations.md` §Core Features "Report Queue" + the top-of-file Status note from DESIGN → partially-shipped (read-only viewer at `GET /admin/reports`; resolution write-back still DESIGN).
