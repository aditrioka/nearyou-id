...
`Authorization: Bearer <revenuecat-webhook-secret>` is mandatory and compared with `MessageDigest.isEqual` (constant-time). `X-RevenueCat-Signature` HMAC-SHA256 over the raw body is verified against `revenuecat-webhook-hmac-secret` **when the header is present** (RevenueCat signs only if signing is enabled in its dashboard). Rationale: Bearer is the always-on gate; HMAC is defense-in-depth that activates when the operator enables signing. Both secret **slot names** come from the `secretKey(env, name)` env-namespacing helper, their **values** from the secret resolver — never a direct secret read. **Alternative rejected**: the OIDC `InternalEndpointAuth` gate — RevenueCat cannot mint a Google-issued OIDC token, so OIDC is structurally wrong for this caller; the spec already carves the route out. Auth failures log a WARN security event — Bearer-failure and HMAC-signature-failure are logged distinguishably so the `docs/05` § Anomaly Detection Metrics "signature fail rate" alert keys on real signature failures, not ordinary `/internal/*` internet scans — and never echo secret material. **Conditional HMAC is a known Bearer-only-fallback tradeoff**: making signing mandatory in production is deferred (see the spec's "Mandatory production webhook signing is deferred" requirement) to avoid rejecting all live webhooks before the operator enables dashboard signing.

### D2 — Idempotency via DB `UNIQUE`, not in-memory dedup
Idempotency uses `subscription_events.revenuecat_event_id UNIQUE` (`INSERT … ON CONFLICT (revenuecat_event_id) DO NOTHING`; a 0-row insert means "already processed" → `200` duplicate, skip the state transition). The DB resolves the concurrent-delivery race (two identical webhooks arriving at once) — only one insert wins, the other reads 0 rows and is treated as a duplicate (covered by the "Concurrent duplicate deliveries" spec scenario). **This durable idempotency is also the replay defense**: HMAC authenticates integrity but not freshness, so a captured `(body, signature)` pair could be replayed at the auth layer; `UNIQUE(revenuecat_event_id)` makes any such replay a no-op duplicate rather than a repeated state change. **Alternative rejected**: the in-memory `InMemoryDedup` ring buffer that `appleS2SRoutes` uses — acceptable for Apple's idempotent relay-email toggles, but billing events are financial and must stay exactly-once across instance restarts and horizontal scale-out, which only a durable DB constraint guarantees.
...
### D6 — User resolution by `app_user_id = users.id`; prefer a read-free `UPDATE … RETURNING`; orphan events are acked
The RevenueCat `app_user_id` is the `users.id` UUID (set at SDK init in the future mobile change). Because every transition is an unconditional status set keyed by event type, the handler SHOULD resolve-and-write in a single `UPDATE users SET subscription_status = ? WHERE id = ? RETURNING …` — a 0-row result is the orphan signal, avoiding a separate `SELECT … FROM users` entirely. On an orphan (no matching user — should be rare), the handler logs WARN and returns `200` (acknowledged) **without** a DB write: the `subscription_events.user_id` FK would reject an orphan row anyway, and a non-2xx would make RevenueCat retry the unresolvable event forever.

**Lint mechanics** (verified against the rule + precedent): `UPDATE users … WHERE id = ?` does NOT match `BlockExclusionJoinRule` (its pattern is `FROM|JOIN users`, not `UPDATE`), so the read-free path needs no annotation. IF implementation nonetheless adds a `SELECT … FROM users` (e.g. to read prior state for a future conditional), that read WILL trip the rule in this non-admin code — the suspension/unban worker's raw `FROM users` passes only via the admin-package path exemption, which this billing code does not get — and MUST carry `@AllowMissingBlockJoin` (annotated on the SQL-holding property, PR #207 pattern) with the privileged-by-id-billing-write rationale (not a viewer-scoped feed read). The shadow-ban `RawFromPostsRule` is inert here (it matches `FROM posts` only).

### D7 — No RevenueCat SDK on the backend
The inbound webhook deserializes the JSON envelope directly with `AppJson` — no RevenueCat server SDK is added, so the `:infra:revenuecat` module stays DESIGN. The vendor-SDK-leakage invariant is satisfied trivially (no SDK import). The mobile RevenueCat KMP SDK is a separate future change. **DTO-shape note**: RevenueCat's webhook payload nests the event under a top-level `event` object (`event.type`, `event.app_user_id`, `event.id`, `event.expiration_reason`, …); the `@Serializable` request DTO must mirror that nesting — verify against RevenueCat's current webhook docs at apply time rather than assuming flat fields.
