...
- **User-initiated logout** UI (Settings "Keluar") — stays with `mobile-settings-screen` (FOLLOW_UPS `mobile-auth-signin-logout-wire-up`); only the shared `SessionInvalidator` seam is noted here.
...
- **Load-bearing detail:** "exactly ONE refresh" holds **only if** the second caller *awaits and reuses* the in-flight refresh's result, rather than re-reading the store and issuing its own `POST`. The `Mutex` + a shared `Deferred`/result-cache (not just a lock around independent round-trips) is the implementation contract; the 9.2 single-flight test asserts exactly one `POST /api/v1/auth/refresh` across an overlapping proactive+reactive pair.
...
Both the cold-start-wake (item #4) and foreground-return (item #5) triggers collapse into **one** rule: on the app-root `Lifecycle.Event.ON_RESUME`, if `accessExpiresAtEpochMillis - now < 5 min`, call `TokenRefresher.refresh()` async (fire-and-forget; never blocks composition or the first frame). The `<5-min` window **is** the idle gate — a 3-second app-switch leaves >5 min on the token and does nothing; a resume after the token lapsed refreshes once. Implemented in **commonMain** via `lifecycle-runtime-compose`'s `LifecycleResumeEffect` at the app root (the CMP common `LifecycleOwner` is wired to the iOS UIViewController lifetime and the Android process/activity lifecycle — verified 2026-06-08 per [proandroiddev / kotlinlang docs](https://kotlinlang.org/docs/multiplatform/compose-lifecycle.html): `org.jetbrains.androidx.lifecycle:lifecycle-runtime-compose` already pinned at 2.10.0 and consumed by `:mobile:app`, no new dependency, no expect/actual).
- **Coroutine scope:** the fire-and-forget refresh launches on a **lifecycle-bound** scope — an app-root `rememberCoroutineScope()` (cancels with the host) or a Koin-`single` `CoroutineScope(SupervisorJob() + Dispatchers.Main)`. **NOT `GlobalScope`** (Detekt-discouraged; an orphaned token-touching coroutine that outlives the composition surface is exactly what to avoid).
- **Clock seam:** the expiry comparison uses the project's existing injectable `nowMillis: () -> Long` precedent (`AuthApiClient.kt:93`), **not** `TimeSource.Monotonic` (monotonic time can't be compared against an absolute `accessExpiresAtEpochMillis`). This makes the `<5-min` boundary deterministically unit-testable.
- **Supersedes the spec's "in `loadTokens`" aside:** the committed `mobile-auth-signin` spec (§ RootRouterScreen, ~line 329) reserved preemptive refresh as "a future optimization in `loadTokens`." This design instead uses the `ON_RESUME` async hook, because `docs/05-Implementation.md` line 38 specifies an *async, on-wake* refresh that a synchronous per-request `loadTokens` callback cannot satisfy. The residual case the wake-trigger misses (a >15-min foreground session crossing the boundary without backgrounding) is covered transparently by the unchanged reactive 401 refresh — so no `loadTokens` proactive check is added (avoiding a per-request blocking refresh).
...
Add `NearbyTimelineOutcome.SessionExpired` (and `GlobalTimelineOutcome.SessionExpired`). The repositories add an explicit `HttpError(401)` → `SessionExpired` branch **ahead of** the existing fallback; a transport failure (`NetworkError(cause)` from a caught `IOException`/timeout) keeps mapping to the existing `NetworkError` outcome (connectivity copy retained). **Note on the `when`:** the mapping is over an `Int` status, so a defined fallback for any *unenumerated* non-2xx (e.g. an unexpected 403/404) MUST remain — it maps to `NetworkError` (retryable). The spec's "no generic fallthrough" bans a generic *"load failed" copy*, not a `when` `else`; the fix is to branch 401 explicitly, **not** to delete the fallback (which would be a non-exhaustive-`Int` compile error). The screen renders `SessionExpired` as a **neutral redirect placeholder** (a short "Mengalihkan ke halaman masuk…" line via a CMP string, **no** retry button, **no** connectivity copy) — a brief, honest state for the sub-second before D1's now-reliable re-route whisks the user to `SignInScreen`.
...
Wire `NearbyTimelineRepository`'s `diagnosticLog` in `MobileModule` to a coordinate-free sink (currently the no-op default — `single { NearbyTimelineRepository(get(), get(), get()) }`). `GlobalTimelineRepository` defaults its sink to no-op too (`single { GlobalTimelineRepository(get(), get()) }`), so wire it identically. The sink carries no coordinate/token (signature is `(String) -> Unit` with pre-redacted messages; the only inputs are `HttpError.status: Int` and `NetworkError.cause.message` — neither is a token or coordinate) — the existing PII discipline and the `CoordinateMaskingLogger` on the HTTP path are untouched.
