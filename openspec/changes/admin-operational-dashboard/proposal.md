...
- **Add live-data operational widgets** (every widget below reads a table/function that exists in the current schema today — no Flyway migration):
  ...
  - Top 10 active cities — posts grouped by the denormalized `posts.city_name` (set at write time by the `posts_set_city_tg` trigger, V11; no read-time `admin_regions` join or `ST_Contains`, per the `global-timeline` canonical pattern). The V12 region-polygon seed (552 regions) is shipped, so in-polygon posts carry a populated `city_name` and the widget shows live city rankings; posts matching no polygon have NULL `city_name` and are excluded.
...
- **Invariants:** the raw `FROM posts` / `FROM users` aggregate reads live in the admin module, which is **path-exempt** from `RawFromPostsRule` + `BlockExclusionJoinRule` (project.md § Coding Conventions; the shipped `ReportResolutionRepository` reads raw `posts` un-annotated) — so **no lint annotation is required**. The reads carry a justification comment documenting the deliberate operator-volume semantics (the count includes posts by shadow-banned authors + auto-hidden posts — deliberately not the `visible_posts` subset). `admin_app` role stays read-only; `pg_database_size()` callability is confirmed in design (else DB-size defers).
