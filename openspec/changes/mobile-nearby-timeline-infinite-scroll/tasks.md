...
- [ ] 3.4 Tests: VM (append below page 1, cursor advance, end-reached, non-destructive error, anchor reused + never logged); MockEngine `NearbyTimelineApiClient`/`Repository` (follow-up carries `cursor` + the page-1 `lat`/`lng`/`radius_m`); extend `NearbyTimelineScreenTest` (scroll-end triggers load-more; footer states).
...
- [ ] 4.4 Tests: VM + MockEngine client (follow-up carries `cursor`, no `lat`/`lng`) + extend `FollowingTimelineScreenTest`.
...
- [ ] 5.4 Tests: VM + MockEngine client + extend `GlobalTimelineScreenTest`.
...
- [ ] 6.4 Tests: VM (append; mark-read on an appended row; mark-all-read over all loaded rows; end-reached; non-destructive error); MockEngine client (follow-up carries `cursor`); extend `NotificationsScreenTest`.
...
- [ ] 7.4 Tests: replies-paging in the VM (append below page 1; cursor advance; end-reached; non-destructive error; new-reply prepend leaves appended pages undisturbed); MockEngine reply client (follow-up carries `cursor`); extend `PostDetailScreenTest` (scroll-end on replies triggers load-more; footer states; header/like-row unaffected) + `PostDetailUiStateTest` if the replies projection changes.
...
- [ ] 9.1 `./gradlew :mobile:app:testStagingDebugUnitTest` (+ `:mobile:app:testDevReleaseUnitTest` to confirm `*ScreenTest` Release-variant excludes still hold) — all green.
