...
- [ ] 1.1 Create `infra/otel/src/main/kotlin/id/nearyou/app/infra/otel/IpHasher.kt` modeled on the sibling `UserIdHasher.kt` — `object IpHasher { fun hash(ip: String): String }` returning the first 16 hex characters of `SHA-256(ip.toByteArray(StandardCharsets.UTF_8))`. Use `java.security.MessageDigest.getInstance("SHA-256")` and `String.format("%02x", ...)` per byte for the hex encoding (mirror `UserIdHasher`'s exact style).
- [ ] 1.2 Create `infra/otel/src/test/kotlin/id/nearyou/app/infra/otel/IpHasherTest.kt` mirroring `UserIdHasherTest` (4 scenarios). Cover: deterministic, distinct-IPs-distinct-output, exact 16-hex shape, 1000-random-IPv4-shape sanity. Add a 5th scenario for IPv6 input (`2001:db8::1`) producing valid 16-hex output.
- [ ] 1.3 Run `./gradlew :infra:otel:test` and confirm 5 new `IpHasherTest` scenarios pass alongside the 4 existing `UserIdHasherTest` scenarios.

## 2. Wire `IpHasher` into the `/health/*` rate-limit call site

- [ ] 2.1 Locate the `/health/*` rate-limit Redis-key construction in `:backend:ktor` — likely `backend/ktor/src/main/kotlin/id/nearyou/app/health/HealthRoutes.kt` or wherever `tryAcquireByKey(key = "{scope:health}:{ip:...}", ...)` is invoked. Grep for the literal `{scope:health}` to find it.
- [ ] 2.2 Replace the raw `clientIp` interpolation in the key with `IpHasher.hash(call.clientIp)`. Add `import id.nearyou.app.infra.otel.IpHasher` to the file. Verify the call site still passes `capacity = 60` and `ttl = Duration.ofSeconds(60)` unchanged.
- [ ] 2.3 Update the corresponding test in `:backend:ktor` (likely `HealthRoutesTest` or similar — find it by grepping for the raw-IP assertion `1.2.3.4` near a `/health/*` test fixture) to assert the 16-hex hashed form in the captured Redis key, not the raw IP. Use `IpHasher.hash("1.2.3.4")` to compute the expected value rather than hardcoding the hash literal (more readable + survives a future hash-shape change with a single `IpHasher.kt` edit).
