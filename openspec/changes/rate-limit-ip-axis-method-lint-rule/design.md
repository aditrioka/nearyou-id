...
### Decision 9: Adopt the canonical `id.nearyou.test.fixtures.*` package-FQN allowlist fallback

**Choice:** The path-based allowlist (`/src/test/`) is supplemented by a package-FQN fallback: any `KtFile` whose `packageFqName` equals `id.nearyou.test.fixtures` OR begins with `id.nearyou.test.fixtures.` is also allowlisted. Test fixtures for this rule MUST be authored with `package id.nearyou.test.fixtures.iprule` (or any sub-package) for the harness scenarios to either fire (negative path: fixture in canonical production package) or pass (positive path: fixture in test-fixtures package).

**Rationale:**

- Detekt's test harness `lint(String)` overload synthesises files with no `virtualFilePath` populated — the `/src/test/` path check returns false on synthetic test fixtures. Without a package-FQN fallback the rule cannot fire in the test harness at all.
- **Canonical project precedent (3 rules):** `RedisHashTagRule.isAllowedPath()` at [`RedisHashTagRule.kt:115-120`](../../../lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/RedisHashTagRule.kt), `RateLimitTtlRule.isAllowedPath()` at [`RateLimitTtlRule.kt:92-100`](../../../lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/RateLimitTtlRule.kt), and `BlockExclusionJoinRule.isAllowedPath()` at [`BlockExclusionJoinRule.kt:176-186`](../../../lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/BlockExclusionJoinRule.kt) — all three implement the dual check: path-based test allowlist + package-FQN fallback for `id.nearyou.test.fixtures.*`. This is the majority pattern.
- **Outlier (1 rule):** `OtelForbiddenAttributeRule.isAllowedPath()` at [`OtelForbiddenAttributeRule.kt:179-187`](../../../lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/OtelForbiddenAttributeRule.kt) uses `id.nearyou.lint.detekt.*` instead. The reason: `OtelForbiddenAttributeRule`'s KDoc embeds raw IP literals (illustrative examples of what the rule forbids); without the `id.nearyou.lint.detekt.*` allowlist the rule would self-trigger on its own production source. The IP-axis-method rule has no analogous self-trigger concern (the KDoc references the regex literal only, not Kotlin `tryAcquire(...)` syntax) — adopting the majority `id.nearyou.test.fixtures.*` pattern is the correct precedent to follow here.
- The dual check is symmetric: real `**/src/test/**` files (e.g., the lint-test class itself) AND Detekt-test-harness synthetic fixtures (whose package is set explicitly via `package id.nearyou.test.fixtures.iprule` at the top of the inline-Kotlin string) both pass the allowlist; production code in `:backend:ktor` (whose packages start with `id.nearyou.app.*`) does NOT.
