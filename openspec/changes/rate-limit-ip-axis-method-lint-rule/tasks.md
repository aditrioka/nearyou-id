...
- [ ] 1.4 Implement the dual allowlist: short-circuit `visitCallExpression` if (a) `containingKtFile.virtualFilePath?.replace('\\', '/')?.contains("/src/test/") == true` OR (b) the `KtFile`'s `packageFqName.asString() == "id.nearyou.lint.detekt"` OR `startsWith("id.nearyou.lint.detekt.")`. Mirror precedent from [`OtelForbiddenAttributeRule.kt:179-187`](../../../lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/OtelForbiddenAttributeRule.kt) `isAllowedPath()` exactly. The package-FQN fallback is required because Detekt's test harness `lint(String)` overload synthesises files with `virtualFilePath = null` — without the fallback every positive-case fixture would silently pass.
...
- [ ] 3.6 Implement scenario 13 (block-form template `${...}` fires): fixture `RateLimiter.tryAcquire(userId, "{scope:foo}:{ip:${'$'}{IpHasher.hash(clientIp)}}", 10, ttl)` → exactly one finding. Locks the rule's method-choice-not-value-shape semantic.
- [ ] 3.7 Implement scenario 14 (triple-quoted string fires): fixture `RateLimiter.tryAcquire(userId, """{scope:foo}:{ip:abc123def4567890}""", 10, ttl)` → exactly one finding. Confirms the rule handles `removeSurrounding("\"\"\"")` for triple-quoted KtStringTemplateExpression flat-content extraction.
- [ ] 3.8 Implement scenario 15 (chained-call shape fires): fixture `rateLimiter.tryAcquire(userId, "{scope:foo}:{ip:abc123def4567890}", 10, ttl)` (where `rateLimiter` is a `val rateLimiter: RateLimiter` — the canonical production form) → exactly one finding. Confirms the inner `KtCallExpression.calleeExpression` lookup works through the outer `KtDotQualifiedExpression`.
- [ ] 3.9 Implement scenario 16 (3-rule composition): fixture `RateLimiter.tryAcquire(userId, "rate:health:{ip:1.2.3.4}", 10, ttl)` run against THREE rules together (`IpAxisMustUseTryAcquireByKeyRule` + `OtelForbiddenAttributeRule` + `RedisHashTagRule`) — assert all three finding-IDs appear, no cross-suppression.
- [ ] 3.10 Implement scenario 17 (path-allowlist non-allowlisted companion): fixture identical to (1) but explicitly placed under simulated production path `backend/ktor/src/main/kotlin/id/nearyou/app/SomeService.kt` (NON-allowlisted) → exactly one finding. Pairs with the `/src/test/`-allowlisted-suppresses scenario to lock both sides of the allowlist.

## 4. Provider registration block (co-located in lint-test file)

- [ ] 4.1 Add a Kotest block `"rule registered in NearYouRuleSetProvider"` co-located inside `IpAxisMustUseTryAcquireByKeyLintTest.kt` (mirror the actual just-shipped pattern at [`OtelForbiddenAttributeLintTest.kt:807-812`](../../../lint/detekt-rules/src/test/kotlin/id/nearyou/lint/detekt/OtelForbiddenAttributeLintTest.kt) — and earlier `RedisHashTagRuleTest.kt:244` + `RateLimitTtlRuleTest.kt:242`). Pattern (copy-paste-friendly): construct `NearYouRuleSetProvider()`, materialise via `provider.instance(io.gitlab.arturbosch.detekt.api.Config.empty)` (note: `Config.empty` is a property, no parens), assert `ruleSet.rules.map { it::class.simpleName }.contains("IpAxisMustUseTryAcquireByKeyRule")`. **Do NOT create a separate `NearYouRuleSetProviderTest.kt` file** — that pattern does not exist in the project; an earlier draft of this task list erroneously cited it.
