...
>
> **IMPLEMENTATION STATUS (checkpoint 2026-06-13):** Backend code + Pebble templates are **implemented and verified in-sandbox** (local Postgres via `scripts/setup_backend_db.sh`): `compileKotlin` + `compileTestKotlin` green, `ktlintMainSourceSetCheck` green, `detekt` **0 code smells**, and the **existing** admin moderation + report-queue suites pass (no regression). **STILL PENDING — NOT a silent deferral:** the NEW §7 test classes (one per spec scenario), the full `:backend:ktor:test` run incl. those tests, and the §8 `verify-loop` admin bring-up. The PR stays **draft** until those land. Done so far: §1, §2, §3, §4, §5, §6.2–§6.5 (6.1 measurement-annex redline NOT generated — templates mirror the shipped `users.peb`/audit-log styling functionally, a fidelity polish gap to close at §8).
