...
### Requirement: Replaces wizard stub

The wizard-generated empty `Application.kt` (only declaring `fun main(args: Array<String>) = io.ktor.server.netty.EngineMain.main(args)` with no `module` function) SHALL be replaced. The new `Application.kt` MUST contain a `fun Application.module()` that wires all required plugins above.
