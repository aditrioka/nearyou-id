...
- **GIVEN** the Following feed with a first page plus at least one appended load-more page
...
- **GIVEN** the Following feed whose latest page returned `nextCursor = null`
- **WHEN** the user scrolls to the end again
- **THEN** no further `GET /api/v1/timeline/following` request is issued AND no load-more footer spinner is shown (end-reached)

#### Scenario: A load-more failure keeps the loaded posts and offers retry

- **GIVEN** the Following feed with a loaded first page AND a load-more fetch that fails (network/5xx)
