# content-moderation-appeal Specification

## Purpose
The user-facing recourse for account-level moderation actions (7-day suspension / permanent ban): the `appeals` ledger (data model + `pending → approved | rejected` lifecycle) and the ban-exempt submission endpoint (eligibility = `is_banned`, shadow-ban exclusion, one-pending guard, per-user daily rate-limit, 1000-char length guard, server-derived `action_type`) plus the own-appeal-status read — so an actioned user can contest a mistake and learn the outcome.
## Requirements
