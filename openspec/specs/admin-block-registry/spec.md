# admin-block-registry Specification

## Purpose

Provide moderators a read-only admin surface (`GET /admin/blocks`) over the `user_blocks` block-pair table for dispute-resolution and support-ticket context — looking up which accounts a given user has blocked or been blocked by. The viewer renders a newest-first, keyset-paginated table (blocker → blocked, `created_at` in UTC, and a bidirectional-block indicator) with either-side search by username or user ID, deep-linking each username to the shipped `/admin/users` lookup. It is strictly read-only: it writes no `admin_actions_log` row, notifies neither party, and creates or removes no block — block enforcement stays in the product path via the bidirectional NOT-IN join (the `BlockExclusionJoinRule` invariant). The view is accessible to any authenticated admin role and surfaces only the two usernames plus the block timestamp (no other PII).
