# admin-chat-message-redaction Specification

## Purpose

The `admin-chat-message-redaction` capability is the admin write surface for redacting a reported chat message that carries a severe violation (PII leak, doxxing) — the message-level moderation action the report-queue resolution actions cannot perform (they act on the offending user or Hide a post/reply, but not a chat message). It serves `GET /admin/chat-messages/{id}` (a confirmation page rendering the target message plus ±2 surrounding messages as limited-disclosure context, a required reason field, and the Redact action) and an owner/admin-tier, CSRF-gated, destructive-action-rate-limited `POST /admin/chat-messages/{id}/redact` that — in one transaction — idempotently sets `redacted_at`/`redacted_by`/`redaction_reason`, writes a `chat_message_redacted` notification to both active conversation participants, and writes one immutable `admin_actions_log` row. It is reachable as a deep-link from a `chat_message` report row; the chat data plane already NULL-masks redacted content, so this capability is the write half. Phase 3.5 ([`docs/07-Operations.md`](../../../docs/07-Operations.md) § Core Features "Chat Message Redaction"); mirrors the shipped report-resolution write pattern (no new pattern).
