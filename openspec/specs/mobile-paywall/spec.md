...
## Purpose
The in-app conversion surface that turns the Premium upsell CTAs from dead-ends into a working subscribe flow. It defines the `PaywallRoute` / `PaywallScreen` / `PaywallViewModel` (Compose Multiplatform / Material 3, mockup frame 17) — a Premium hero tailored to the entry-context, the Month-1 benefit set (features available now, no image upload), RevenueCat-Offerings-derived pricing cards, and a subscribe → confirm-entitlement → return-and-re-evaluate flow — backed by the vendor-free `PurchaseController` seam in `:infra:revenuecat` (the SDK is `implementation`-fenced there per invariant #16). It is reachable from the daily-cap upsell dialog (`LIKE_CAP`), the search 403 gate (`SEARCH_GATE`), and the premium-username gate (`USERNAME`), and degrades to a fail-soft `Unconfigured` state until RevenueCat billing is provisioned. The subscription contract is owned by the `subscription-billing-webhook` change (`subscription_status` is webhook-driven; the client premium signal is the RevenueCat `CustomerInfo` entitlement).
## Requirements
### Requirement: PaywallRoute is a payload-carrying serializable NavKey registered for the iOS-saveable back stack

The change SHALL introduce a `PaywallRoute` `NavKey` (in `mobile/app/src/commonMain/kotlin/id/nearyou/app/screens/routing/NavKeys.kt`) that is a `@Serializable data class` carrying a single non-PII `entry: PaywallEntry` property — an enum of the gated surfaces that can open the paywall. This change WIRES `LIKE_CAP` (cap dialog) and `SEARCH_GATE` (search 403 gate); it also supplies the `USERNAME` entry-context for `premium-username-customization` ([#322](https://github.com/aditrioka/nearyou-id/pull/322)), whose merged spec (§ "The Premium gate … routes to the paywall") delegates its username-gate call-site `PaywallRoute` push to this change — so on merge the `appEntryProvider` `UsernameCustomizationRoute` call site is wired to `PaywallRoute(USERNAME)` (fulfilling that change's `TODO(#309)`). The route MUST NOT carry any PII, token, coordinate, or user identifier. It SHALL be registered in the `navSavedStateConfiguration` polymorphic `SerializersModule` (`mobile/app/src/commonMain/kotlin/id/nearyou/app/screens/routing/AppNavSerialization.kt`) via an explicit `subclass(PaywallRoute::class, PaywallRoute.serializer())` entry so the back stack is saveable on Kotlin/Native (iOS), mirroring `PostDetailRoute`. `PaywallRoute` SHALL be appended to the **root** back stack (overlaying the section `NavigationBar`), the same mechanism `SearchRoute` / `PostDetailRoute` / the composer FAB use — deliberately NOT a per-tab back stack.
