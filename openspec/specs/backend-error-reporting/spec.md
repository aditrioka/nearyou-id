# backend-error-reporting Specification

## Purpose
Sentry error aggregation for `:backend:ktor`, so a production 500 or an operator-actionable `ERROR` log becomes an alertable event instead of a Cloud Logging line nobody watches. The Sentry Java SDK is fenced in the JVM module `:infra:sentry-jvm` (invariant #16) behind a vendor-free `SentryBootstrap.start(env, dsn, release)`; a logback `SentryAppender` at `ERROR` on the root logger turns the StatusPages 500 handler and every existing `log.error` into an event with no per-call-site code, and the SDK's uncaught-exception handler covers non-request threads. A blank DSN is a safe no-op. The PII posture matches mobile (`sendDefaultPii=false`, no host name, a `PiiScrubber` `beforeSend`) but is stricter on identity: no user is attached and log breadcrumbs are off. Correlation goes through the request `call_id` tag into Cloud Logging, and tracing stays with OpenTelemetry. The mobile counterpart is `mobile-crash-reporting`.
## Requirements
