# mobile-fcm-token-registration Specification

## Purpose

The mobile FCM push-token acquisition + backend-registration lifecycle in `:mobile:app` — the device-side consumer of the shipped `POST /api/v1/user/fcm-token` endpoint + `user_fcm_tokens` table (`docs/04-Architecture.md` § Push Notification Infrastructure / `docs/05-Implementation.md` § FCM Token Registration). A platform-free commonMain `FcmTokenProvider` interface (Android `firebase-messaging` actual + iOS `FirebaseMessaging` Pod actual — the vendor SDK confined to the platform source sets) acquires the device registration token; a `FcmTokenApiClient` over the shared `Auth`-plugin `HttpClient` registers it via the `{token, platform, app_version}` body, mapping responses to a closed `FcmRegistrationOutcome` vocabulary (`Registered` / `Rejected(code)` / `Unauthorized` / `TransportError` / `NoTokenAvailable`) with client-side guards mirroring the backend validators; and a `FcmTokenRegistrar` orchestrates acquire→register, fired once from the authenticated app shell (`AppShellScreen`, where cold-start / sign-in / signup converge) and re-registering on SDK token rotation for the shell's lifetime — so registration is structurally never attempted while unauthenticated. The raw token is never logged (a device-addressed credential). iOS requests only the minimal notification authorization that token acquisition structurally requires (APNs registration); Android requests no `POST_NOTIFICATIONS` (token issuance needs none). Push message display/handling is implemented by the separate `mobile-push-message-handling` capability (#256); the contextual notification-permission prompt UX is the consuming feature's (#257, shipped with the chat screen); the Firebase client config + APNs key are operator setup (#258) — the code builds, unit-tests, and runs without them (the providers return `null` when no `FirebaseApp` is configured).
