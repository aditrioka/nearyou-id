# auth-login-anomaly-detection Specification

## Purpose
Consume the `login_events` history (V35, `login-history-tracking`) for the anti-abuse anomaly leg it was built to feed: a periodic, Cloud Scheduler-driven sweep that flags any user whose logins span more than 5 distinct `/24` IP subnets within a trailing 1-hour window — the "credentials shared / account takeover / bot farm" signal from `docs/05-Implementation.md` § Anomaly Detection Metrics. Flagged users are recorded idempotently as `moderation_queue` rows (`trigger='anomaly_detection'`, `target_type='user'` — the value the schema already reserved) for human moderator review; no automated enforcement, no user-visible effect, no PII in notes or logs. The detector runs behind the OIDC-gated internal-worker pattern (`POST /internal/login-anomaly-check`), inert until the operator's Cloud Scheduler job invokes it.
