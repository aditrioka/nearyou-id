...
- **AND** if the webhook handler ALSO bumps `users.token_version` (per the `auth-jwt-spec-debt-userprincipal-subscription-status` follow-up, action item 3), the existing JWT additionally fails token-version validation at the auth plugin and returns `401 token_revoked`, forcing a fresh sign-in cycle — but the subscription_status downgrade is already enforced even WITHOUT the token-version bump because of the per-request `findById` read
