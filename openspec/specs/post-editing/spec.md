...
## Purpose
Post editing is a Premium entitlement: a subscriber may edit a post's text content within 30 minutes of creation, author-only, with an append-only temporal edit-history audit trail (`post_edits`). Edits are race-safe (`SELECT … FOR UPDATE` + a `(post_id, edited_at)` unique index), re-moderated (an edit cannot launder clean→toxic content past create-time moderation), per-user rate-limited, and surfaced through a visibility-respecting history read that never exposes raw location (spatial-fuzzing invariant). This capability covers the backend endpoints `PATCH /api/v1/posts/{post_id}` + `GET /api/v1/posts/{post_id}/edits`; the mobile "Riwayat edit" UI and the admin edit-history filter are separate follow-ups.
