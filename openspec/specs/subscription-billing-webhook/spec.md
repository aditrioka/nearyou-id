...
## Purpose
The subscription-billing-webhook capability turns RevenueCat billing events into the authoritative writer of `users.subscription_status`. It exposes the vendor-authenticated (`Authorization: Bearer` + optional HMAC) `POST /internal/revenuecat-webhook` endpoint — mounted outside the OIDC `/internal` gate — which idempotently records every event in the `subscription_events` ledger (keyed by `revenuecat_event_id UNIQUE`) and drives the paid-path 3-state status machine (`free` / `premium_active` / `premium_billing_retry`), writing the billing-issue and expiry notifications. Premium-gated features (e.g. premium search) read the status this capability owns. Referral `GRANT` entitlement stacking, the 72h privacy-flip coupling, the time-based grace-elapse worker, the cancellation confirmation notification, and mandatory production signing are deferred to their owning changes.
...
### Requirement: 72h privacy-flip scheduling is deferred

This change SHALL NOT set `users.privacy_flip_scheduled_at` on an `EXPIRATION` downgrade and SHALL NOT clear it on a Premium re-activation — the 72h privacy-flip window and its acting worker are owned by a future privacy-flip-worker change. The `EXPIRATION` handler downgrades `subscription_status` to `free` and writes the `subscription_expired` notification only. The future privacy-flip change will MODIFY the `EXPIRATION` and purchase/renewal transitions to add the scheduling (on downgrade) and clearing (on re-subscribe).

#### Scenario: Expiration downgrades without scheduling a privacy flip
- **WHEN** an `EXPIRATION` event downgrades a user who has `private_profile_opt_in = TRUE`
- **THEN** that user's `subscription_status` becomes `free` AND `users.privacy_flip_scheduled_at` remains `NULL` (no flip is scheduled by this change)
