# admin-hard-delete-queue Specification

## Purpose

Provide operators an oversight surface (`GET /admin/deletion-requests`, admin board frame 15) over the `deletion_requests` rows pending hard-delete (`executed_at IS NULL AND cancelled_at IS NULL`) — the account right-to-erasure (UU PDP) population produced by the user-initiated `account-deletion` lifecycle and consumed by the daily `account-hard-delete-worker` — so the deletion pipeline can be triaged without raw SQL. The list is keyset-paginated **soonest-deadline-first** with composable `q` (username/UUID) + `source` filters, a pending-count summary, and a per-row countdown; it is served by the existing `deletion_requests_scheduled_idx` partial index (no migration) under identity-only PII discipline (the `users` JOIN is FK-guaranteed present via `ON DELETE CASCADE`, tolerating a soft-deleted target). It additionally exposes a manual **expedite** write that advances a pending request's `scheduled_hard_delete_at` to `NOW()` so the existing worker erases the account on its next run (the admin *schedules*; the worker *executes* — no synchronous cascade in the route): `owner`/`admin`- + CSRF-gated, reason-required, rate-limited on a distinct **10/admin/hour** counter independent of the destructive budget, recording exactly one immutable `deletion_request_expedited` audit row atomically with the deadline advance, and rejecting any already-executed / cancelled / already-due / unknown target. The 6th read-only-admin-viewer instance, extended with one genuinely mutating, irreversible-accelerating action.
