# referral-read Specification

## Purpose

The `referral-read` capability is the JWT-authenticated `GET /api/v1/user/referral` endpoint that returns the caller's own shareable invite code (`users.invite_code_prefix`, returned verbatim — the same value redemption resolves via `findInviterByInviteCodePrefix`) plus their referral progress: the count of `granted` referral tickets, the 5-referral inviter-reward milestone (`ReferralActivityCheckWorker.INVITER_MILESTONE`), and whether the inviter lifetime reward is already claimed (`users.inviter_reward_claimed_at IS NOT NULL`). It is principal-only — resolved from the verified JWT `sub` with no `user_id` parameter, so it has no IDOR surface — and a pure read that writes nothing and grants nothing (the referral activity-check worker remains the sole authority for promoting tickets and claiming rewards). It fails soft (`500` on a repository error) with PII-safe logging (event name + exception class only). It exists to make the already-built referral growth/revenue loop reachable from the mobile app: every user is assigned an invite code and the ticket-creation + grant-worker machinery shipped previously, but no surface could read a user's own code and progress until this endpoint.

## Requirements
### Requirement: GET /api/v1/user/referral returns the caller's invite code and referral progress

The backend SHALL expose `GET /api/v1/user/referral` authenticated via `AUTH_PROVIDER_USER` (Bearer JWT). The handler SHALL resolve the caller from the verified principal (`principal.userId`) ONLY — it accepts NO `user_id` path or query parameter, so it has NO IDOR surface — and SHALL respond `200` with a JSON body `{ "inviteCode": <string>, "grantedReferrals": <int>, "milestone": <int>, "inviterRewardClaimed": <boolean> }` (bare camelCase keys, mirroring the `hide-distance` / timeline wire convention). `inviteCode` SHALL be the caller's `users.invite_code_prefix` returned verbatim (the same value redemption resolves via `findInviterByInviteCodePrefix`) — no second derivation and no transform. An unauthenticated request SHALL be rejected with `401`. The endpoint is a pure read — it writes nothing and grants nothing. The `invite_code_prefix` read requires no username/privacy-flag *write* allowlist annotation (it is a read, not a `username` or `private_profile_opt_in` write); however, because it `SELECT`s `FROM users` (a `BlockExclusionJoinRule`-protected table), the SQL-holding property SHALL carry `@AllowMissingBlockJoin` declaring the own-row self-read (scoped to `id = caller`; a user cannot block themselves; not a feed/visibility read), mirroring the `HideDistanceRepository` precedent — the annotation on the SQL-bearing property, not the function. The `referral_tickets` granted-count query is not annotated (that table is outside the protected set). PII discipline: the handler SHALL log only a content-free event name (and, on failure, the exception class) — never the bearer token, the JWT `sub`, the invite code, or the response body.
