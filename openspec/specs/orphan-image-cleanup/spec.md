...
Reclaims uploaded-but-never-attached images: every successful `POST /api/v1/images` inserts an `image_uploads` ledger row at `status = 'uploaded'`, and only a post-attach flips it to `'attached'` — without this capability, orphaned uploads accumulate forever as paid Cloudflare Images storage and dead ledger rows. A daily Cloud-Scheduler-invoked worker (`POST /internal/cleanup-orphan-images`, OIDC-gated) sweeps `'uploaded'` rows older than 24 hours, deleting the Cloudflare image and the ledger row per-row transactionally so a concurrent attach always wins the race and a vendor failure leaves the row for the next run. Discharges the orphan-cleanup deferral of the `premium-image-upload` capability (issue #340).
