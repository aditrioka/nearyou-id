...
## Purpose
The account-deletion capability provides user-initiated account deletion (the UU PDP right-to-erasure): `POST` / `DELETE` / `GET /api/v1/account/deletion-request` — schedule a deletion with a 30-day cancellable grace, cancel/restore within the window, and read the pending status (for the mobile restore banner). The `deletion_requests` schema enumerates all four `source` values; a pending-deletion account stays fully functional during grace (NOT session-terminated — it is not a suspension). This lays the `deletion_requests` foundation the downstream admin Hard Delete Queue + Apple S2S delete paths build on. The hard-delete execution lives in the companion `account-hard-delete-worker` capability.
## Requirements
### Requirement: deletion_requests schema

Migration `V27__deletion_requests.sql` SHALL create the `deletion_requests` table **exactly** per the canonical `docs/05-Implementation.md` § Deletion Requests Schema (the migration number MAY be renumbered above V23 at rebase if a concurrent change lands first):
