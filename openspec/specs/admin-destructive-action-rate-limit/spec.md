# admin-destructive-action-rate-limit Specification

## Purpose

The `admin-destructive-action-rate-limit` capability enforces a per-admin abuse-prevention cap of **20 destructive admin actions per acting admin per trailing one-hour window** (pulling forward `docs/08-Roadmap-Risk.md` § Pre-Launch #9). The **destructive set** is the user-punitive account-state actions — a **warning** (`user_warned`), a **suspend** (`user_suspended`), and the report-queue **permanent-ban** / **shadow-ban** resolutions; restorative and non-punitive actions (unban, content keep/hide, report decision bookkeeping, reads/logins) are never counted or capped. The count is sourced from the immutable `admin_actions_log` audit trail itself (the audit trail IS the rate-limit ledger — no second source of truth to drift), checked inside the same JDBC transaction as the gated action for read-consistency; it deliberately does NOT take a `FOR UPDATE` lock on the ledger, so it is an abuse-prevention SOFT cap with an accepted ±1 concurrency tolerance, not a hard authorization boundary. A destructive action at or over the cap is rejected with no mutation and no audit row, surfaced as an inline "quota exceeded" state (never a 5xx). The trailing-hour count is also exposed read-only so the `admin-user-management` profile page can render a quota chip. It is a shared cross-capability guard consumed by `admin-user-moderation` (suspend, warn) and `admin-report-queue` (the destructive moderation-queue resolutions).
## Requirements
