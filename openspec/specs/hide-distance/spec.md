# hide-distance Specification

## Purpose
The hide-distance capability is the Premium "Hide Distance" feature (`docs/01-Business.md` § Hide Distance Mechanics; Phase 4 #15). A per-user `users.hide_distance_opt_in` flag — **effective only while the user is Premium** (`subscription_status IN ('premium_active','premium_billing_retry')`) — **symmetrically** suppresses the distance NUMBER on the Nearby feed: it is hidden for a (post-author, viewer) pair when EITHER side's preference is effective, while `city_name`, the global 5km floor, the radius filter, and time ordering are all unaffected (no ordering leak). The capability owns the JWT-authenticated `PATCH` + `GET /api/v1/user/hide-distance` endpoints (write-anytime + read-gated; GET returns the stored flag plus the caller's effective-Premium status so the mobile Settings toggle can seed its state and gate interactive-vs-upsell). Cross-surface scope: the Nearby timeline is the only surface that emits a viewer-relative distance number, so the rule is a verified no-op everywhere else (Following/Global feeds, post-detail, profile, search, chat-embedded card, and push bodies all remain distance-free). Suppression is **server-side field omission upstream of the pure `:shared:distance` `DistanceRenderer.render`** — the renderer is never given a hidden mode.
