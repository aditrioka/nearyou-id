# system-actor Specification

## Purpose

The `system-actor` capability defines the deterministic `system` sentinel admin user — a non-human `admin_users` row that machine-initiated (worker / scheduled-job) actions attribute to, since `admin_actions_log.admin_id` is `NOT NULL REFERENCES admin_users(id)` and a background job has no human admin to record. The sentinel has a fixed UUID (`UUID.nameUUIDFromBytes("system")` = `54b53072-540e-3eb8-b8e9-343e71f28176`), is seeded inactive (`is_active = FALSE`) with a well-formed-but-non-matching Argon2id `password_hash` so it can never authenticate, and holds the least-privilege `read_only` role. Because `admin_actions_log.admin_id` carries no `ON DELETE` action, the sentinel cannot be hard-deleted while it owns audit rows. It is the canonical attribution actor for every machine-written `admin_actions_log` row — the first consumer being `suspension-unban-worker`.
