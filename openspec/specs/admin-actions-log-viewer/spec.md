# admin-actions-log-viewer Specification

## Purpose

The `admin-actions-log-viewer` capability owns the read-only, in-app surface for the `admin_actions_log` audit trail — the `GET /admin/actions-log` route, wired inside the `admin-login` session + CSRF gate. It renders a newest-first (`created_at DESC, id DESC`) paginated table via a keyset cursor over `(created_at, id)` (no SQL `OFFSET`), with composable, index-aligned filters (`action_type` / `admin_id` / `target_type`+`target_id` / `from`–`to` date range), an `admin_users` join for human-readable actor display, HTML-escaped rendering of every row value (including the client-controlled `user_agent` and the `before_state` / `after_state` JSONB shown in a per-row detail region), and an HTMX partial-swap with a plain-`GET` progressive-enhancement fallback. It is strictly read-only — it adds no mutation route and writes no audit row on read — and ships a single performance-only Flyway migration (`V17`) creating the `(created_at DESC, id DESC)` keyset index. DB-level immutability of the log remains the operational `REVOKE UPDATE, DELETE … FROM admin_app` enforcement (out of scope here).

See [`docs/07-Operations.md` § Core Features + § Admin Actions Log (Reference)](../../../docs/07-Operations.md) for the canonical "Moderation Actions Log" contract, [`docs/05-Implementation.md` § Admin Actions Log Schema](../../../docs/05-Implementation.md) for the table shape, and the `admin-schema` / `admin-login` / `admin-panel-scaffold` capabilities for the schema, auth gate, and route subtree this builds on.
