...
- **THEN** a single `FloatingActionButton` node is present AND activating it appends `PostCreationRoute` to the **root** back stack (the composer surface becomes the current entry, overlaying the tab bar)
