# admin-csam-detection-log Specification

## Purpose
The Admin Panel operator front-end for the CSAM (child-safety) takedown subsystem: a read-only detection-log viewer over `csam_detection_archive`, the admin-triggered fixed-policy takedown (the MVP "paste matched URL/image_id from Cloudflare's email" path, invoked in-process), Kominfo-filing tracking that releases a row to the retention purge, and audit-logged on-demand metadata decryption. Every destructive/sensitive action is `owner`/`admin`-gated, CSRF-protected, and audited; the surface is image-content-free by construction (only `image_hash` + on-demand decrypted scalar metadata are ever shown). Makes the <24h Kominfo reporting obligation operable end-to-end.
