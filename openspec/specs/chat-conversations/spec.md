...
#### Scenario: embedded_post_author_id FK is live post-V38
- **WHEN** the integration-test Kotest spec queries `pg_constraint` for `chat_messages` with `contype = 'f'` looking for a FK on `embedded_post_author_id`
- **THEN** exactly one row is returned with `confrelid` referencing `users` AND `confdeltype = 'n'` (`ON DELETE SET NULL`) AND `convalidated = true`
...
- **WHEN** A calls `GET /api/v1/conversations`
- **THEN** X IS in the response list — list-view does not enforce block exclusion (consistent with `docs/02-Product.md` "Existing conversations remain visible in history" and the list-messages history-readable-after-block contract)
...
- **WHEN** A calls `GET /api/v1/chat/X/messages`
- **THEN** the response is `200 OK` with M1, M2, M3 still readable — the block-aware-chat contract at `docs/02-Product.md` requires history visibility for both parties post-block
