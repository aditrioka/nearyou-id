# private-profile Specification

## Purpose

The user-facing side of the Premium private-profile perk: `PATCH` / `GET /api/v1/user/private-profile` — the sanctioned `@allow-privacy-write: user_settings` writer of `users.private_profile_opt_in` (the reserved Settings-flow entry on the privacy-flag-write lint allowlist) and the state read that seeds the mobile Settings "Profil privat" toggle. The write is permitted for any tier ("write-anytime + read-gated", mirroring `hide-distance`); effectiveness is governed by the canonical effective-private formula, which this capability references but does not alter. An opt-out additionally clears any pending `privacy_flip_scheduled_at` — the docs/03 "confirm switch public" action — so toggling public mid-grace takes effect immediately. Broader read-hiding (timelines, profile page, post-detail, chat, notifications) is explicitly out of scope: the read effect remains exactly the shipped `premium-search` follower-only gate.
