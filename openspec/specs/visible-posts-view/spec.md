...
#### Scenario: Post-V24 definition keeps shadow-ban + post-soft-delete but not the author-deletion exclusion
- **WHEN** querying `pg_views WHERE viewname = 'visible_posts'` on a database migrated to V24 or later
...
- **WHEN** querying `pg_views WHERE viewname = 'visible_posts'` on a database at exactly V20–V23 (before V24)
- **THEN** the `definition` contained both `p.deleted_at IS NULL` and `u.deleted_at IS NULL` (V24 later drops the author-side one)
...
- **THEN** that row does not appear in `visible_posts` (post-side soft-delete is unchanged by V24)

#### Scenario: View stays viewer-agnostic after the tombstone-surfacing change
- **WHEN** querying `pg_views WHERE viewname = 'visible_posts'` after V24 ships
