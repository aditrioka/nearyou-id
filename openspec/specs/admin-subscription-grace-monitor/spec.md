# admin-subscription-grace-monitor Specification

## Purpose

Provide support-desk operators a read surface (`GET /admin/subscriptions/grace`, admin board frame 18) over the `users` rows in `subscription_status = 'premium_billing_retry'` — the RevenueCat billing-retry grace population written by the `subscription-billing-webhook` capability — so a "I was charged but premium stopped / the webhook is late" ticket can be triaged without raw SQL. Each row surfaces the store/platform, the current-streak retry-since, the latest webhook event, and an already-expedited indicator; it is keyset-paginated with composable `q` (username/UUID) + `store` filters and an in-window count summary, served by the existing V2 `users_subscription_idx` partial index (no migration). It additionally exposes a manual **expedite** bookkeeping write that records exactly one immutable `admin_actions_log` row (CSRF- + `owner`/`admin`-gated, capped at 20/hour per admin via a distinct counter, requiring a support-ticket reference) — explicitly **without** mutating entitlement: RevenueCat remains the sole source of truth (only an `EXPIRATION` event downgrades), so the audit row's `before_state`/`after_state` carry an identical `subscription_status`. The fifth instance of the read-only-admin-viewer pattern, extended with one non-punitive write.
