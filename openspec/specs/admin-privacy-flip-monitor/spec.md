# admin-privacy-flip-monitor Specification

## Purpose

Provide moderators a read-only admin surface (`GET /admin/privacy-flips`, admin board frame 17) over the `users` rows with a pending privacy flip (`privacy_flip_scheduled_at IS NOT NULL`) — the 72h private↔public downgrade-grace population. Each row is classified, against a single request-scoped evaluation instant, as **IN_WINDOW** (still mid-grace — the canonical `docs/07-Operations.md` population) or **OVERDUE** (deadline passed but the worker has not cleared the row — a stuck-row / webhook-handler-bug signal surfaced deliberately beyond the canonical in-window predicate). It serves support tickets ("why is my profile still private / went public?") and anomaly detection (mass-scheduling events, stuck rows past the deadline). The monitor renders an ascending keyset-paginated table (most-overdue / soonest first) with composable `status` (in_window/overdue) and `q` (UUID / exact case-insensitive username) filters and an in-window-vs-overdue count summary whose OVERDUE total is the anomaly signal. It is index-served by the existing V2 partial index `users_privacy_flip_idx` (no migration), enforces identity-only PII discipline (username/display_name/id + flip state, no location/email/DOB; usernames deep-link to the shipped `/admin/users?q=` lookup), HTML-escapes all user-controlled output, and is strictly read-only — it writes no `admin_actions_log` row, mutates nothing, and exposes no clear/expedite control (anomalies are escalated to an out-of-band worker fix). The fourth instance of the read-only-admin-viewer pattern.
