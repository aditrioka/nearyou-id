# timeline-read-rate-limit Specification

## Purpose

Defines the per-user Layer 2 read-side rate limit shared across the three authenticated timeline endpoints (`GET /api/v1/timeline/{nearby,following,global}`). Enforces a 150-posts/hour rolling hard cap aggregated across all three endpoints into a single per-user bucket plus a 50-posts/session soft cap that drives a non-blocking `upsell.soft` UX nudge. Premium tier (`premium_active`, `premium_billing_retry`) is unconditionally exempt — both buckets are skipped entirely. The cap counts post-deliveries (not requests) so a 30-post-per-page client hits the cap on the 6th request, not the 31st.

The capability owns: the bucket-key contract (`{scope:rate_timeline_rolling}:{user:U}` and `{scope:rate_timeline_session}:{session:U__SID}` — both conforming to the `RedisHashTagRule` strict regex without modification), the cap-hit response shape (HTTP 200 with `posts: []` + `upsell.hard = true`, NOT 429), the limiter ordering (rolling pre-check → session pre-check → DB query → post-increment, with cap-hit short-circuiting before the timeline DB query), the `X-Session-Id` validation contract (`^[A-Za-z0-9-]{1,64}$` with `no-session` fallback to prevent Redis-key injection), and the over-admission tolerance bound (per-request `N - 1 ≤ 29`; concurrent same-user `K × N` bounded by the per-IP authenticated baseline).

Guest read rate limiting (`10/session + 30/hour` keyed by guest JWT `jti`) is explicitly out of scope and lands in a follow-up change once the Layer 1 attestation-gated guest-session JWT issuance work (Phase 1 item 25) ships. See `docs/05-Implementation.md:799-806` + `:1159-1160` for the canonical cap values and informal key-shape references; `docs/08-Roadmap-Risk.md` Phase 1 item 30 for cap rationale; the `rate-limit-infrastructure` capability spec for the underlying `RateLimiter` interface and Detekt-rule enforcement of the hash-tag standard.
