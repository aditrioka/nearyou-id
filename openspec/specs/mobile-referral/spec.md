# mobile-referral Specification

## Purpose

The `mobile-referral` capability is the mobile app's referral surface: a parameterless `ReferralRoute` mapping to a `ReferralScreen` + `ReferralViewModel` that displays the user's shareable invite code with a copy-to-clipboard action, the "X dari 5" progress toward the 5-referral inviter reward plus a distinct reward-unlocked state, and explanatory reward copy — reached from a Settings "Undang teman" entry row. It is open to all tiers (no Premium gate; `docs/01-Business.md` § Referral System makes participation universal — the rewards are Premium grants, but sharing and progress are not gated), backed by a `ReferralApiClient` + `ReferralRepository` data layer over the `referral-read` endpoint, with all copy sourced from `:shared:resources`. The v1 sharing affordance is copy-to-clipboard; a native system share-sheet is an explicit deferred requirement tracked by a follow-up issue. It exists to give users a way to find, copy, and track their referral code so the open-signup virality + Premium-grant loop has a usable mobile entry point.
