# privacy-flip-worker Specification

## Purpose
The privacy-flip-worker capability is the hourly internal worker (`POST /internal/privacy-flip-worker`, OIDC-gated on its own route subtree) that completes the 72h Premium→Free privacy-downgrade flow. Once a private profile's grace deadline (`users.privacy_flip_scheduled_at`, set by the `subscription-billing-webhook` on an `EXPIRATION`) has elapsed, it flips `private_profile_opt_in` to `FALSE`, clears the deadline, and writes one immutable `system_privacy_flip_applied` audit row per flip attributed to the seeded `system` sentinel actor — all in a single data-modifying CTE, atomic and idempotent (a retry flips nothing and writes no audit row). It is invoked by Cloud Scheduler and reuses the `suspension-unban-worker` internal-worker pattern (own-subtree OIDC gate + system-actor audit CTE); it is what gives the shipped admin `privacy-flip-monitor` real data.
