...
Defines the user-blocking contract: the V5 `user_blocks` table (schema, constraints, dual cascade FKs to `users`), the `POST/DELETE/GET /api/v1/blocks/*` HTTP endpoints (auth, idempotency, self-block prevention, pagination), and the documented future-cascade hook for follow-style relationships. Block exclusion at read time is enforced by `block-exclusion-lint` and exercised by `nearby-timeline`. As of `social-list-profile-summaries`, `GET /blocks` rows embed the blocked user's profile summary via the chat-conversations partner pattern (`LEFT JOIN visible_users` + `akun_dihapus` placeholders — rows survive hidden targets so unblock stays possible), while the block ACTIONS deliberately keep existence-based semantics (a hidden harasser must stay blockable; they are spec'd exemptions from the constant-404 visibility contract).
