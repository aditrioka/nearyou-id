...
- **THEN** there is NO `ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon` (or any value) assignment in the target `buildSettings` — the launcher icon resolves only from each configuration's xcconfig (`Production.xcconfig` → `AppIcon`, `Staging.xcconfig` → `AppIcon-Staging`, `Dev.xcconfig` → `AppIcon-Dev`)
...
- **HTTP 403 with `error = "account_banned"`**: the 403 body carries a `suspended_until` field (per `auth-signin`: an ISO-8601 timestamp for a suspension, `null` for a permanent ban) that the `AuthRepository` SHALL parse and branch on, emitting one of two banned sub-states. The discriminator is **null-ness** (non-null ⇒ suspension, including a non-null value already in the past; `null` ⇒ permanent); if the field is absent or unparseable (e.g. an older backend), the client SHALL default to the **permanent** sub-state (the safe degrade — support copy, no appeal entry). The limited `appeal_token` is captured into the in-memory appeal-session holder **before** the sub-state branch, so capture is independent of which sub-state is emitted. Both sub-states remain on `SignInScreen` with the CTA disabled (visually disabled AND tap-rejected per the dedicated scenario below) to prevent retry (per `mobile-appeal`):
...
Adding the field when attestation lands is deferred per `docs/08-Roadmap-Risk.md` § Phase 3 (attestation) / `docs/06-Security-Privacy.md` § Attestation.
...
#### Scenario: canonical docs track the attestation fingerprint deferral

- **WHEN** inspecting `docs/08-Roadmap-Risk.md` § Phase 3 (attestation) and `docs/06-Security-Privacy.md` § Attestation after this change is applied
- **THEN** the deferral of `device_fingerprint_hash` (identifier `mobile-auth-signin-attestation-fingerprint-hash`, referencing this Mobile #3 omission as the trigger) is tracked there as the landing context for adding the field when attestation lands
