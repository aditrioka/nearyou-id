...
2. **Compile-time** (added by the `ContentWriteRequiresModerationRule` requirement above): a Detekt rule that fires on candidate content-write call sites when the enclosing function does NOT also invoke `TextModerator.moderate(...)` BEFORE the write call. The rule's allowlist mechanism (`@AllowContentWriteWithoutModeration("<reason>")` annotation with reasons `tombstone` / `admin_redaction` / `seed` / `embedded_snapshot` / `service_layer_moderated`) provides documented bypass for legitimate carve-outs. **Effective coverage scope** (extended 2026-06-11): the rule fires on Kotlin sources scanned by the project Detekt invocation — `:backend:ktor`'s `src/main/kotlin` (per `nearyou.ktor.gradle.kts`) AND the `:infra:*` / `:core:*` JVM modules (per the `nearyou.detekt` convention plugin + `config/detekt/invariants.yml`). Repository-layer SQL INSERTs in `:infra:supabase` (`JdbcPostRepository.kt`, `JdbcPostReplyRepository.kt`) ARE now scanned; as structurally-moderation-free sinks they carry `@AllowContentWriteWithoutModeration("service_layer_moderated")`, and the 3 integration tests remain the end-to-end call-order defence.

The two layers are independent — neither replaces the other. Compile-time enforcement provides faster developer feedback (IDE squiggles) and catches regressions in `:backend:ktor`-scoped service-layer call sites; test-time enforcement provides end-to-end behavioural assertion (HTTP handler → moderator → INSERT) across all 3 paths regardless of source-set scope.
...
- **THEN** `layer3DispatcherScope.dispatch(coroutineContext) { ... }` IS invoked AFTER the INSERT commit (Layer 3 runs independently of Layer 2's outcome — both can fire on the same row, producing two queue rows with distinct triggers per [`docs/05-Implementation.md:545`](../../../../../docs/05-Implementation.md))
...
- **THEN** `layer3DispatcherScope.dispatch(coroutineContext) { ... }` is NOT invoked (chat-message Layer 3 is explicitly deferred per the `text-moderation-perspective-api-layer` capability design Open Question 1; chat ships Layer 1+2 only as of this change)
...
2. **Repository-method pattern**: `KtCallExpression`s whose callee short-name + receiver-expression text matches one of the verified canonical write-method shapes (production-source-verified at proposal time):
   - `posts.create(conn, ...)` — receiver `posts` (the `PostRepository` constructor-injected parameter name in [`CreatePostService.kt:41,97`](../../../../../backend/ktor/src/main/kotlin/id/nearyou/app/post/CreatePostService.kt))
   - `replies.insertInTx(conn, ...)` — receiver `replies` (the `PostReplyRepository` constructor-injected parameter name in [`ReplyService.kt:66,155`](../../../../../backend/ktor/src/main/kotlin/id/nearyou/app/engagement/ReplyService.kt))
   - `repository.sendMessage(conversationId, senderId, content, ...)` — receiver `repository` (the `ChatRepository` constructor-injected parameter name in [`ChatService.kt:69,256`](../../../../../backend/ktor/src/main/kotlin/id/nearyou/app/chat/ChatService.kt))

   These receiver names are intentionally generic (Kotlin idiom for constructor-injected dependencies). The match-set MUST be exact text — matching just the short-name (`create`/`insertInTx`/`sendMessage`) without the receiver-text gate would produce false positives on unrelated classes. Future additions to the match-set go through the spec amendment process.
...
**Annotation-bypass**: a function annotated with `@AllowContentWriteWithoutModeration("<reason>")` (where `<reason>` is non-blank and one of `tombstone` / `admin_redaction` / `seed` / `embedded_snapshot` / `service_layer_moderated` — the last two added 2026-06-11: `embedded_snapshot` for writes copying ALREADY-MODERATED content verbatim with `content` itself NULL, `service_layer_moderated` for repository-layer INSERT sinks whose `moderate()` call lives at the service call site per docs/11 layering) SHALL pass the rule. Empty-string reasons MUST fail the rule (mirror `RateLimitTtlRule`'s `@AllowDailyTtlOverride` enforcement). Reasons not in the enumerated set MUST fail the rule with an error message naming the allowed reasons. The annotation lives alongside the existing `@AllowDailyTtlOverride` / `@AllowRawRedisKey` / `@AllowForbiddenSpanAttribute` annotations (the canonical 3 in-project bypass annotations as of this change — verified by `grep -rn "AllowDailyTtlOverride\|AllowRawRedisKey\|AllowForbiddenSpanAttribute" core/ lint/`).
...
The rule SHALL be registered in `lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/NearYouRuleSetProvider.kt` AND listed with `active: true` in every project Detekt config that runs the ruleset (`backend/ktor/config/detekt/detekt.yml`, `config/detekt/invariants.yml`). The original "default-active pattern" claim here was WRONG — detekt 1.23 treats a rule absent from a provided config as INACTIVE (`valueOrDefault(ACTIVE_KEY, false)`); rules default to active only under `Config.empty`, i.e. in `:lint:detekt-rules`' own unit tests. That asymmetry left 7 of 10 rules (including this one) dormant in every project Detekt run until the 2026-06-11 backend review activated them; `DetektConfigActivationTest` now pins every provider rule against both config files.

**Effective coverage scope**: the rule's effective enforcement scope MATCHES the project Detekt source-set configuration. Since 2026-06-11 that scope is `:backend:ktor`'s `src/main/kotlin` (per `nearyou.ktor.gradle.kts`) PLUS the `:infra:*` and `:core:*` JVM modules' `src/main/kotlin` (per the `nearyou.detekt` convention plugin). The original `:backend:ktor`-only profile below is retained as the historical record of the asymmetry that motivated the extension:

- **`chat_messages`**: arm (a) SQL-literal regex catches the inlined INSERTs at [`ChatRepository.kt:529, 727`](../../../../../backend/ktor/src/main/kotlin/id/nearyou/app/chat/ChatRepository.kt) (which lives in `:backend:ktor` per the chat package's in-tree Repository placement); arm (b) catches the `repository.sendMessage(...)` call site at `ChatService.kt:256`. Both arms cover the chat path.
- **`posts`**: arm (a) does NOT catch the `INSERT INTO posts (...)` SQL literal — it lives at [`infra/supabase/.../JdbcPostRepository.kt:15`](../../../../../infra/supabase/src/main/kotlin/id/nearyou/app/infra/repo/JdbcPostRepository.kt) which is NOT in Detekt's source-set scope. Arm (b) DOES catch the `posts.create(conn, ...)` call site at `CreatePostService.kt:97` (in `:backend:ktor`).
- **`post_replies`**: arm (a) does NOT catch the `INSERT INTO post_replies (...)` SQL literal — it lives at [`infra/supabase/.../JdbcPostReplyRepository.kt:37, 166`](../../../../../infra/supabase/src/main/kotlin/id/nearyou/app/infra/repo/JdbcPostReplyRepository.kt) (NOT in Detekt's scope). Arm (b) DOES catch the `replies.insertInTx(conn, ...)` call site at `ReplyService.kt:155` (in `:backend:ktor`).

Net (historical): arm (b) was the load-bearing arm for `posts` and `post_replies` defence while arm (a) was functionally chat-only. The 2026-06-11 extension closed that asymmetry: arm (a) now fires on the `:infra:supabase` SQL literals too, and the repository sink functions carry `@AllowContentWriteWithoutModeration("service_layer_moderated")` so the contract is visible at the SQL site while arm (b) + the integration tests keep enforcing the actual call order.

This requirement complements — and does NOT replace — the existing test-time enforcement scenarios on the parent capability. Together the two layers form defense-in-depth on the call-order contract: compile-time call-site mechanical (this rule, backend + infra/core source-set scope) + test-time end-to-end behaviour (the 3 existing integration tests, full call-graph including `:infra:supabase` Repository writes).
...
- **THEN** the rule fires AND the error message names the allowed reasons (`tombstone` / `admin_redaction` / `seed` / `embedded_snapshot` / `service_layer_moderated`)
