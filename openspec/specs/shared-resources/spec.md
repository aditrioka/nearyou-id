...
#### Scenario: Follow-up issue tracks the Detekt rule upgrade with retargeted accessor

- **WHEN** inspecting the project's open GitHub issues (label `follow-up`) after this change is applied
- **THEN** GitHub issue [#184](https://github.com/aditrioka/nearyou-id/issues/184) (label `follow-up`) tracks `mobile-negative-requirement-ci-grep` (proposing future OpenSpec change `mobile-negative-requirement-detekt-rule`), noting that the grep-based verification in this change should eventually be replaced by a `:lint:detekt-rules` rule modeled on the existing `RawFromPostsRule` / `BlockExclusionJoinRule` precedent — AND the issue's example accessor pattern references `Res.string.X` (CMP Resources), NOT the legacy `MR.strings.X` (Moko Resources) wording Mobile #2 originally used
...
- **THEN** vector-drawable assets for the bottom-nav (Home / Notifications / Profile), the composer action (add), the privacy-note shield (`ic_privacy_shield`), and the post-card affordances (location / like / reply) are present AND each is accessible from `:mobile:app` commonMain via `painterResource(Res.drawable.<name>)` AND no feed-tab icon drawable is required (tabs are text-only)
