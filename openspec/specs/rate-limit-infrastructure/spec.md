...
This dual-allowlist pattern mirrors the canonical project shape used by `RedisHashTagRule.isAllowedPath()` at [`RedisHashTagRule.kt:115-120`](../../../../../lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/RedisHashTagRule.kt), `RateLimitTtlRule.isAllowedPath()` at [`RateLimitTtlRule.kt:92-100`](../../../../../lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/RateLimitTtlRule.kt), and `BlockExclusionJoinRule.isAllowedPath()` at [`BlockExclusionJoinRule.kt:176-186`](../../../../../lint/detekt-rules/src/main/kotlin/id/nearyou/lint/detekt/BlockExclusionJoinRule.kt) — all three use `id.nearyou.test.fixtures.*` as the fallback FQN. (Note: `OtelForbiddenAttributeRule` uses `id.nearyou.lint.detekt.*` instead — it is the outlier because its KDoc examples embed raw IP literals that would otherwise self-trigger; the IP-axis-method rule has no analogous self-fixture concern.) Without the package-FQN fallback every positive-case fixture in `IpAxisMustUseTryAcquireByKeyLintTest` would silently pass (the `lint(String)` test overload synthesises files with `virtualFilePath = null`).
