# admin-schema Specification

## Purpose

The admin-schema capability owns the five admin tables (`admin_users`, `admin_webauthn_credentials`, `admin_sessions`, `admin_webauthn_challenges`, `admin_actions_log`) plus the three FK backfills on operational tables (`reports.reviewed_by`, `moderation_queue.resolved_by`, `chat_messages.redacted_by`) that close the V9/V15-era "deferred to the Phase 3.5 admin-users migration" markers. It ships the schema-layer enforcement of the `csrf_token_hash` invariant via `NOT NULL`, the "admin-user FK ON DELETE SET NULL" invariant via the three validated backfills, and the audit-trail-preservation contract on `admin_actions_log` via `NO ACTION` default (rejecting orphaning admin hard-delete). Role-level access control (`admin_app` REVOKE statements), RLS policies, and the sentinel `system` admin user seed are explicitly out of scope and tracked as operational / follow-up work; this capability is schema-only and ships zero application code, zero seed rows, and zero `GRANT`/`REVOKE` statements.

See [`docs/05-Implementation.md` § Admin Users Schema (lines 636-703)](../../../docs/05-Implementation.md) and [§ Admin Actions Log Schema (lines 1184-1208)](../../../docs/05-Implementation.md) for the canonical schema source; [`docs/07-Operations.md` § Data Access Pattern](../../../docs/07-Operations.md) for the operational `admin_app` REVOKE / GRANT contract; and [`openspec/project.md` § Coding Conventions](../../project.md) for the `csrf_token_hash` and "Admin-user FKs on operational tables must use `ON DELETE SET NULL`" invariants that the V16 migration enforces at the schema layer.
## Requirements
