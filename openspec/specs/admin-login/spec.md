...
The `admin-login` capability owns the admin panel's authentication path — the first end-to-end admin login for the `/admin/` route subtree scaffolded by `admin-panel-scaffold`. It verifies an Argon2id password + a mandatory RFC 6238 TOTP second factor (solo-admin period; WebAuthn deferred), establishes an opaque `__Host-admin_session` cookie (256-bit token, SHA-256 at rest) with an HMAC-derived per-session CSRF token, and gates every authenticated `/admin/` request behind fail-closed session-validation + CSRF middleware. It audit-logs every login attempt + CSRF violation to `admin_actions_log`, returns a no-enumeration response on every failure path (with timing equalization), decrypts `admin_users.totp_secret_encrypted` via AES-256-GCM (admin-UUID AAD), and replaces the `admin-panel-scaffold` unauthenticated-mount posture (the `KTOR_ENV` guard is removed — this auth gate is what makes the subtree production-safe). It unblocks every operationally-load-bearing admin write (audit-log viewer, suspend/unban, etc.).

See [`docs/08-Roadmap-Risk.md`](../../../docs/08-Roadmap-Risk.md) Phase 3.5 §3 (the roadmap source), [`docs/05-Implementation.md` § Admin Session Cookie + WebAuthn](../../../docs/05-Implementation.md) + [`docs/07-Operations.md` § Admin Login / Security](../../../docs/07-Operations.md) for the canonical session-cookie + Argon2id + TOTP contract, and the `admin-schema` capability for the `admin_users` / `admin_sessions` / `admin_actions_log` tables this capability writes.
## Requirements
