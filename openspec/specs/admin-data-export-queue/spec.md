# admin-data-export-queue Specification

## Purpose
The admin-data-export-queue capability is the operator-facing surface of the UU-PDP data-portability loop (the producer half is `account-data-export`). It gives admins a read view of every `data_export_requests` job's identity + lifecycle state — `GET /admin/data-exports`, keyset-paginated newest-`requested_at`-first, `status` + `q` (username/UUID)-filterable, joined to `users` for the requester username, with the five DB statuses surfaced as QUEUED / RUNNING / DELIVERED / EXPIRED / FAILED — and a manual recovery action, `POST /admin/data-exports/{id}/trigger`, that re-runs a stalled or failed export through the shipped producer's single-request pipeline (one export path, two callers; not a forked second path). The panel is strictly status orchestration: it surfaces request status and requester identity only, never the archive contents, `r2_object_key`, or signed download URL (the archive is the requester's own personal data, delivered only to them via a short-lived signed link). The trigger is owner/admin + CSRF-gated (CSRF checked before the role gate), requires a reason, is rate-limited on a distinct 10/admin/hour counter independent of the destructive-action budget, writes exactly one immutable `data_export_triggered` audit row per state-transitioning trigger, and is a benign idempotent no-op on a non-triggerable target. For UU-PDP 7-day-SLA accountability and day-to-day support triage; the canonical visual is admin mockup frame 16. No schema change ships (the `data_export_requests` table + indexes exist at V30; `admin_actions_log.action_type` is a free `VARCHAR(64)`).
