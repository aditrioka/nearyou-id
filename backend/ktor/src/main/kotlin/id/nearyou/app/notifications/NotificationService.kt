 ...
 * idempotent already-read branch on `markRead`. All JDBC hops to the
 * pool-bounded [dbDispatcher] (docs/11 §3.2); the default keeps
 * direct-construction tests working — production passes `DbDispatchers.db`.
 */
class NotificationService(
    ...
    suspend fun unreadCount(userId: UUID): Long = withContext(dbDispatcher) { notifications.countUnread(userId) }
    ...
    suspend fun markRead(
        ...
        withContext(dbDispatcher) {
            ...
            if (affected > 0) return@withContext MarkReadOutcome.Acknowledged
            // 0 rows: either already-read (exists for user) or not found (does not exist / other user).
            if (notifications.existsForUser(userId, notificationId)) {
    ...
    suspend fun markAllRead(userId: UUID): Int = withContext(dbDispatcher) { notifications.markAllRead(userId) }
