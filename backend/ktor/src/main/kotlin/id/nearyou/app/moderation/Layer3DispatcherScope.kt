...
/**
 * Long-lived supervised coroutine scope owning the async Layer 3 (toxicity
 * classifier) dispatch lifecycle for `text-moderation-perspective-api-layer`.
 * Single class owning BOTH the lifecycle (long-lived `SupervisorJob`-rooted scope,
 * bounded shutdown drain) AND the dispatch-after-shutdown WARN behavior —
 * deliberate divergence from the `:infra:fcm` `FcmDispatcherScope` + `FcmDispatcher`
 * split per design.md Decision 4 (FCM has substantial per-dispatch logic; Layer 3
 * dispatch is a single timed HTTP call so collapsing both responsibilities keeps
 * the surface lean).
 *
 * Internal shape:
 *  - `scope = CoroutineScope(SupervisorJob() + Dispatchers.IO + CoroutineName("layer3-dispatch"))`
 ...
 * `traceparent` header on the outbound HTTP call) inherits per design.md Decision 13.
 * The Layer 3 dispatch span is parented under the originating request span (e.g.,
 * `POST /api/v1/posts`).
 *
 * Post-shutdown semantics:
 *  - `dispatch(...)` after shutdown emits `event=layer3_dispatch_after_shutdown`
 ...
 *    and a `event=layer3_dispatch_drain_exceeded cancelled_count=<N>` WARN is
 *    emitted.
 *
 * **Vendor history**: the original `text-moderation-perspective-api-layer` change
 * planned Google Perspective API. Mid-implementation Perspective announced sunset
 * (end-of-2026) so the vendor pivoted to OpenAI Moderation. Sentry event names
 * + CoroutineName + cache scope use the vendor-agnostic `layer3_*` prefix.
 */
class Layer3DispatcherScope private constructor(
     ...
     *
     * Emits `event=layer3_dispatch_drain_exceeded cancelled_count=<N>` WARN if
    ...
    fun shutdown(drainMillis: Long = DEFAULT_DRAIN_MILLIS) {
        ...
        log.info(
            "event=layer3_dispatcher_scope_shutdown drain_ms={} in_flight_before={}",
            drainMillis,
            inFlightBefore,
        )
    }
    ...
    companion object {
        ...
        const val EVENT_DISPATCH_AFTER_SHUTDOWN: String = "layer3_dispatch_after_shutdown"
        const val EVENT_DISPATCH_DRAIN_EXCEEDED: String = "layer3_dispatch_drain_exceeded"
        const val EVENT_DISPATCH_UNHANDLED_EXCEPTION: String = "layer3_dispatch_unhandled_exception"

        private val log = LoggerFactory.getLogger(Layer3DispatcherScope::class.java)
         ...
         * Dispatchers.IO + CoroutineName("layer3-dispatch") + [UNHANDLED_HANDLER]`.
         * Caller wires `shutdown(...)` into the JVM shutdown hook.
         */
        fun production(): Layer3DispatcherScope {
            val scope =
                CoroutineScope(
                    SupervisorJob() +
                        Dispatchers.IO +
                        CoroutineName("layer3-dispatch") +
                        UNHANDLED_HANDLER,
                )
            return Layer3DispatcherScope(scope = scope)
        }
         ...
         */
        fun forTest(scope: CoroutineScope): Layer3DispatcherScope = Layer3DispatcherScope(scope = scope)
         ...
         */
        fun forTestWithDefensiveHandler(scope: CoroutineScope): Layer3DispatcherScope {
            val wrappedScope = CoroutineScope(scope.coroutineContext + UNHANDLED_HANDLER)
            return Layer3DispatcherScope(scope = wrappedScope)
        }
    }
}
