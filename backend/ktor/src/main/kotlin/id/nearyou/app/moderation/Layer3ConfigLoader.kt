...
/**
 * Cache layer for the Layer 3 kill-switch + threshold flags. Lives in
 ...
 *
 * 3-tier resolution ladder per flag (mirrors the `CachingModerationListLoader` pattern):
 *
 *  1. **Redis cache** (key `{scope:layer3_config}:{flag:<flag-name>}`, TTL 5 min).
 ...
 *     `event = "layer3_kill_switch_unavailable"` per design.md Decision 12.
 *
 ...
 * WARN `event = "layer3_threshold_fallback"`.
 *
 * Cross-flag misconfiguration (`flag_threshold > high_score_threshold`) is detected
 * at the orchestrator ([Layer3Moderator]), NOT here — the loader returns thresholds
 * independently; the orchestrator checks the cross-flag invariant.
 *
 * **Naming note**: Firebase Remote Config parameter names (`perspective_api_enabled`,
 * `perspective_api_high_score_threshold`, `perspective_api_flag_threshold`) retain
 * historical "perspective" branding from the original spec — they're operator-facing
 * Firebase Console parameter names; renaming would require parallel updates in
 * Firebase + Setup Checklist + the `:infra:remote-config` seed checks. The Kotlin
 * class names + Sentry event names + Redis cache key scope are vendor-agnostic
 * (`layer3_*`) per the vendor-swap amendment in proposal.md.
 */
interface Layer3ConfigLoader {
...
/**
 * Production binding for [Layer3ConfigLoader]. Suspending on Redis I/O via
 * the calling coroutine context (Lettuce sync API; the orchestrator is invoked from
 * the dispatcher scope's `Dispatchers.IO` per `Layer3DispatcherScope`).
 *
 ...
 */
class CachingLayer3ConfigLoader(
    ...
    private val log = LoggerFactory.getLogger(CachingLayer3ConfigLoader::class.java)
    ...
    companion object {
        // Firebase Remote Config parameter names: retain historical "perspective_api_*"
        // names since they're operator-facing in the Firebase Console + already seeded
        // in the staging Server template per docs/10-Setup-Checklist.md:152. Renaming
        // would require parallel Firebase Console updates + setup-checklist edits +
        // RC seed migration.
        ...
        // Redis cache key scope: vendor-agnostic `layer3_config` (clean break from the
        // historical `perspective_config` scope since no operator dashboards depend
        // on it; pre-launch state allows the rename).
        ...
        // `perspective_*` events; no operator dashboards exist yet at pre-launch).
        const val EVENT_KILL_SWITCH_UNAVAILABLE: String = "layer3_kill_switch_unavailable"
        const val EVENT_THRESHOLD_FALLBACK: String = "layer3_threshold_fallback"

        internal fun thresholdCacheKey(param: String): String = "{scope:layer3_config}:{flag:$param}"
    }
}
