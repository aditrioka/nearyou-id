 ...
 */
class Layer3IntegrationTest : StringSpec({

    val dataSource = hikari()
    val perspectiveLogAppender = ListAppender<ILoggingEvent>().apply { start() }
    val perspectiveLogger =
        LoggerFactory.getLogger(DefaultLayer3Moderator::class.java) as Logger
    perspectiveLogger.addAppender(perspectiveLogAppender)
    val configLoaderLogger =
        LoggerFactory.getLogger(CachingLayer3ConfigLoader::class.java) as Logger
    ...
    "10.5 + 10.6 Failure path: HTTP 5xx → no DB writes, sentinel content NOT in logs" {
        ...
        val client =
            RecordingModerationClient().apply {
                stageException(
                    id.nearyou.app.infra.openaimoderation.ModerationHttpException(
        ...
        try {
            ...
            val capturedJson = capturedEventsAsJsonLike(perspectiveLogAppender)
            capturedJson shouldContain "layer3_dispatch_failed"
    ...
    "10.7 Kill-switch OFF: ModerationClient never invoked, no DB writes" {
        perspectiveLogAppender.list.clear()
        val client =
            RecordingModerationClient().apply {
                stageMaxScore(0.92) // would auto-hide if invoked
            }
    ...
    "11.3 At most one Sentry-equivalent event per moderate(...) invocation" {
        ...
        try {
            ...
            // the `layer3_dispatcher_scope_shutdown` lifecycle log.
            val perspectiveOutcomeEvents =
                perspectiveLogAppender.list.filter { event ->
                    val msg = event.formattedMessage ?: ""
                    msg.contains("event=layer3_high_score_applied") ||
                        msg.contains("event=layer3_flag_applied") ||
                        msg.contains("event=layer3_dispatch_failed") ||
                        msg.contains("event=layer3_kill_switch_unavailable") ||
                        msg.contains("event=layer3_threshold_misconfigured")
                }
...
private data class TestRig(
    ...
    val dispatcherScope: Layer3DispatcherScope,
    val authorId: UUID,
)
...
// ---- DB seed / inspect / cleanup helpers (mirror JdbcLayer3ModerationWriterIntegrationTest) ----
