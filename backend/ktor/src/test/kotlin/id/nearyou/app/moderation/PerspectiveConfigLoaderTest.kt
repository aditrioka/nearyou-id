...
/**
 * Unit tests for [CachingPerspectiveConfigLoader] per
 ...
 */
class PerspectiveConfigLoaderTest : StringSpec({

    "Tier 1 kill-switch cache hit short-circuits — no Remote Config call" {
        val cache = FakeCache()
        cache.put(CachingPerspectiveConfigLoader.KILL_SWITCH_CACHE_KEY, "false")
        val rc = SpyRemoteConfigClient()
        val loader = CachingPerspectiveConfigLoader(cache, rc)
    ...
    "Tier 1 cache miss → Tier 2 success caches and returns" {
        val cache = FakeCache()
        val rc = SpyRemoteConfigClient(booleans = mapOf(CachingPerspectiveConfigLoader.KILL_SWITCH_PARAM to false))
        val loader = CachingPerspectiveConfigLoader(cache, rc, cacheTtl = Duration.ofMinutes(5))

        runBlocking { loader.isEnabled() } shouldBe false

        cache.entries[CachingPerspectiveConfigLoader.KILL_SWITCH_CACHE_KEY] shouldBe "false"
    ...
    "Tier 2 throw → fail-OPEN to true (kill-switch ERROR path)" {
        ...
        cache.entries[CachingPerspectiveConfigLoader.KILL_SWITCH_CACHE_KEY] shouldBe null
    }
    ...
    "Tier 2 returns null → static default true; cached for the TTL window" {
        ...
        runBlocking { loader.isEnabled() } shouldBe true
        cache.entries[CachingPerspectiveConfigLoader.KILL_SWITCH_CACHE_KEY] shouldBe "true"
    }

    "Tier 1 high-score-threshold cache hit returns parsed double" {
        val cache = FakeCache()
        cache.put(
            CachingPerspectiveConfigLoader.thresholdCacheKey(
                CachingPerspectiveConfigLoader.HIGH_SCORE_PARAM,
            ),
            "0.85",
        )
        val rc = SpyRemoteConfigClient()
    ...
    "Tier 2 high-score-threshold positive out-of-range falls back to default 0.8" {
        ...
        val rc =
            SpyRemoteConfigClient(
                doubles = mapOf(CachingPerspectiveConfigLoader.HIGH_SCORE_PARAM to 1.5),
            )
        val loader = CachingPerspectiveConfigLoader(cache, rc)
    ...
    "Tier 2 high-score-threshold negative falls back to default 0.8" {
        ...
        val rc =
            SpyRemoteConfigClient(
                doubles = mapOf(CachingPerspectiveConfigLoader.HIGH_SCORE_PARAM to -0.5),
            )
        val loader = CachingPerspectiveConfigLoader(cache, rc)
    ...
    "Tier 1 cached out-of-range value falls back to Tier 2 (poisoning prevention)" {
        ...
        val rc =
            SpyRemoteConfigClient(
                doubles = mapOf(CachingPerspectiveConfigLoader.HIGH_SCORE_PARAM to 0.85),
            )
        val loader = CachingPerspectiveConfigLoader(cache, rc)
    ...
    "Flag threshold respects in-range Remote Config values" {
        ...
        val rc =
            SpyRemoteConfigClient(
                doubles = mapOf(CachingPerspectiveConfigLoader.FLAG_PARAM to 0.45),
            )
        val loader = CachingPerspectiveConfigLoader(cache, rc)
    ...
    "Cache key uses canonical hash-tag format for cluster safety" {
        ...
        cache.entries.keys.first() shouldBe "{scope:perspective_config}:{flag:perspective_api_high_score_threshold}"
        // And kill-switch
        cache.put(CachingPerspectiveConfigLoader.KILL_SWITCH_CACHE_KEY, "true")
        cache.entries[CachingPerspectiveConfigLoader.KILL_SWITCH_CACHE_KEY] shouldNotBe null
        CachingPerspectiveConfigLoader.KILL_SWITCH_CACHE_KEY shouldBe
