 ...
 * with real Postgres + `RecordingPerspectiveClient` + a fresh
 * `PerspectiveDispatcherScope` per scenario.
 *
 * Each scenario:
 ...
 *
 * Scope vs. `PerspectiveLayer3IntegrationTest`: this file covers the HTTP-boundary
 ...
 */
@Tags("database")
class PerspectiveLayer3RouteTest : StringSpec({
    ...
    val perspectiveWriter = JdbcPerspectiveModerationWriter(dataSource)
    val jitterSecret =
        Base64
            .getDecoder()
     ...
     * [PerspectiveDispatcherScope] per scenario (so `shutdown(5000)` drains
     * deterministically without leaking into peer scenarios).
     */
    suspend fun withPostApp(
        recordingClient: RecordingPerspectiveClient,
        ...
        val createPostService =
            CreatePostService(
                ...
                perspectiveDispatcherScope = dispatcher,
    ...
    "10.5 POST /api/v1/posts with HTTP 503 → 201 + no Layer 3 DB writes (fail-open)" {
        ...
        val client =
            RecordingPerspectiveClient().apply {
                stageException(PerspectiveHttpException(HttpStatusCode.ServiceUnavailable, body = ""))
            }
    ...
    "10.6 POST /api/v1/posts with 600ms Perspective delay → 201 + timeout fail-open" {
        ...
        val client = RecordingPerspectiveClient().apply { stageDelay(delayMillis = 600L) }
...
private class HttpScenarioContext(
    ...
    private val dispatcher: PerspectiveDispatcherScope,
) {
    /**
