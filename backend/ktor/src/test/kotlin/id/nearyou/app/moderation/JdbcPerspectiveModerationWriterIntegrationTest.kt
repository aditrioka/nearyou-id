...
/**
 * Integration tests for [JdbcPerspectiveModerationWriter] per
 ...
 */
class JdbcPerspectiveModerationWriterIntegrationTest : StringSpec({
    ...
    "Concurrent applyAutoHideAndQueue collapses to one queue row via UNIQUE constraint" {
        ...
        try {
            ...
            val results: MutableList<PerspectiveWriteResult> =
            ...
            results.all { it is PerspectiveWriteResult.Applied || it is PerspectiveWriteResult.QueueConflictNoOp } shouldBe true
        } finally {
            cleanupPost(dataSource, postId)
            cleanupUser(dataSource, author)
        }
    ...
    "Atomicity: queue INSERT failure rolls back the UPDATE (is_auto_hidden remains FALSE)" {
        ...
        try {
            ...
            val interceptingWriter = JdbcPerspectiveModerationWriter(interceptingDataSource)
