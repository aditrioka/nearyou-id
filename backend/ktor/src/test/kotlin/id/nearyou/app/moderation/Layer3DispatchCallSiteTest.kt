 ...
 * Three assertions:
 *
 *  - `CreatePostService.kt` invokes `layer3DispatcherScope.dispatch` AFTER
 *    the canonical `posts.create(` INSERT call.
 *  - `ReplyService.kt` invokes `layer3DispatcherScope.dispatch` AFTER the
 *    canonical `replies.insertInTx(` INSERT call.
 *  - Chat send service files do NOT contain `layer3DispatcherScope.dispatch`
 *    OR `layer3Moderator.moderate` invocations — chat-message Layer 3 is
 ...
 */
class Layer3DispatchCallSiteTest : StringSpec({
    ...
    "CreatePostService dispatches Perspective Layer 3 AFTER posts.create INSERT" {
        ...
        val dispatchMarker = source.indexOf("layer3DispatcherScope.dispatch") // intentional: substring matched by lint scan
        val moderateMarker = source.indexOf("layer3Moderator")

        require(insertMarker > 0) { "posts.create(...) not found in CreatePostService.kt" }
        require(dispatchMarker > 0) { "layer3DispatcherScope.dispatch not found in CreatePostService.kt" }
        require(moderateMarker > 0) { "layer3Moderator reference not found in CreatePostService.kt" }
    ...
    "ReplyService dispatches Perspective Layer 3 AFTER replies.insertInTx INSERT" {
        ...
        require(dispatchMarker > 0) { "layer3DispatcherScope.dispatch not found in ReplyService.kt" }
        require(moderateMarker > 0) { "layer3Moderator reference not found in ReplyService.kt" }
    ...
    "Chat send service files do NOT carry the Layer 3 dispatch (deferred per Open Question 1)" {
        ...
        for (f in serviceFiles) {
            val src = f.readText()
            check(!src.contains("layer3DispatcherScope.dispatch")) {
                "${f.name} unexpectedly contains 'layer3DispatcherScope.dispatch' " +
                    "— chat-message Layer 3 is deferred per `text-moderation-perspective-api-layer` " +
                    "design Open Question 1"
            }
            check(!src.contains("layer3Moderator.moderate")) {
                "${f.name} unexpectedly contains 'layer3Moderator.moderate' " +
                    "— chat-message Layer 3 is deferred per `text-moderation-perspective-api-layer` " +
                    "design Open Question 1"
            }
        }
