...
- `:infra:perspective` — Google Perspective API HTTP client (Ktor CIO + kotlinx.serialization). Owns the `PerspectiveClient` interface and the `KtorPerspectiveClient` Layer-3 toxicity-classifier binding consumed by `:backend:ktor`'s `PerspectiveModerator`.
